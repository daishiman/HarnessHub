...
- ヘッダ: 上段に Workspace 文脈、下段にブラウザ履歴の戻る/進む (各 44px、アイコンのみ) + 現在 route の画面タイトル (最小可読幅 48px) + 対象がある画面だけの検索アイコン + 通知 + アバター。履歴操作とタイトルは共通 `ShellHeader` に 1 回だけ置き、本文の `ScreenHeader` やボトムタブへ複製しない。サイドバーはモバイルで描画しない。
...
| P10 | ウィザード (横長 step 表示) | 1 step = 1 画面・上部に進捗バー・ウィザード内の「戻る」で前 step。共通ヘッダーの戻るは route 履歴なので混同しない | S10・公開ウィザード |
