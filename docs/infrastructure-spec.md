...
- **GitHub Actions secret / variable 台帳 (2026-07-29 実装反映)**: 正本は [`scripts/ci/actions-secrets-registry.json`](../scripts/ci/actions-secrets-registry.json)。手動投入が必須なのは secret 5 件 (`CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_R2_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` / `TURSO_DATABASE_URL` / `TURSO_AUTH_TOKEN`) と variable 2 件 (`HUB_HEALTH_URL` / `HUB_PUBLIC_URL`) の計 7 件。任意は secret 2 件 (`BACKUP_HEARTBEAT_URL` / `NOTION_TOKEN`) と variable 3 件 (`NOTION_DB_SKILL_LIST` / `INTAKE_NOTION_DATABASE_ID` / `NOTION_DB_IMPROVEMENT_REQUEST`)、`GITHUB_TOKEN` は Actions 自動注入である。値や投入済み判定を文書へ複製せず、`node scripts/ci/check-actions-secrets.mjs --live` の結果を現在状態の正とする。
