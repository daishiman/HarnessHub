...
- **後続 (landed)**: 上記 2 件は **HarnessHub-1y6** で処理した。契約 version は 1.2.0 へ進み、`qa_semantic_violations()` は `validate-system-plan.py` へ戻さず新規 module `validate-qa-semantic-coverage.py` (255 行) として分離したため patch 衝突は発生していない (`validate-system-plan.py` は 438 行)。3 skill の live-trial も再取得済み。判定は [system-dev-planner-qa-semantic-coverage](system-dev-planner-qa-semantic-coverage.md) を参照。
- **本書の dangling 参照**: 本 PR (#60) の diff が参照する `docs/features/feat-validator-contract-version/spec-reflection-receipt.md` が不在。8vx 側の残件として追跡する。
