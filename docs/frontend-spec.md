...
- **モバイル (< md)**: §6.2 のボトムタブ+その他シート。ヘッダは WorkspaceSwitcher+画面タイトル+検索アイコン+アバター。
- **Workspace 切替**: desktop / mobile で同じ server component を使い、所属 1 件では現在値だけ、2 件以上では `<details>` と素のリンクで切替候補を出す。現在値はリンクにしない。切替リンクは安全な同一 origin の `returnTo` を持ち、server が旧 scope 情報を含まない中間文書を先に返してから新 scope の画面へ進める。
...
- **S01 公開ウィザード**: Web 手動取込は `skill` ZIP に限定する。新規 Project 作成または自分が owner の既存 Project 指定→PublishRequest 作成→package upload/submit を 1 UI フローに束ね、API の全 status と公開要求 IDを隠さない。途中失敗は段階別 Idempotency-Key で checkpoint から再開し、Needs Fix は同じ request を Draft へ戻して findings と共通文言を表示したまま修正版 ZIP を再投入する。自動進行中だけ S03 の polling 規約で追跡する。`web_app` は作者 local session の wrangler と疎通確認が必須なので選択肢にせず Publisher CLI / S08 Device 承認へ案内する。H7 が未成立の間、Published は Hub の公開記録完了として表示する一方、実導入可能とは表現せず、利用不能な catalog/install リンクや推測した導入コマンドを成功終端にしない。単一テナント/単一 Project を定数にしない。
