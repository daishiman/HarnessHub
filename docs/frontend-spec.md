...
- **S10 ヒアリングウィザード (FormData 30 項目 = backend-spec §4.3)**: system-spec の4大工程 **基本情報 → 業務詳細 → 要件 → 確認** を、入力負荷を下げる8画面 **基本情報 → 現状 → 用途プロファイル → よくある要望パターン → 参考URL・添付 → 要望 → 整理・まとめ → 確認** に分割する。用途プロファイルは `usagePurpose, expertise, role, context, motivation, sharingIntent, constraintTags, shareTarget, knowledgeAssets` の必須 9 軸と、`informationSources, trueProblem` の任意 2 軸 (未回答は `null`、複数入力の回答済み 0 件は `[]`) を扱う。依頼パターンは `requestPatterns` と条件付きの `integrationTools, integrationToolsOther, automationDescription, existingDataSources, existingDataSourcesOther`、参考情報は `referenceUrls` (最大 10 件) を扱う。選択式の初期値は具体的な回答を捏造せず「不明・わからない」とし、親のパターンを選ばない条件付き項目は送らない。整理・まとめで全入力と削減時間の参考値を見せ、確認で送信意思を確定する。金額は出さず、提出後はサーバ snapshot (`estimate_json`) だけを正として S12 に表示する。途中状態はテナント/作業場所別の sessionStorage に保持し、誤離脱には確認を挟む。提出成功後は HS コード・`生成中`・次の導線を表示する。
