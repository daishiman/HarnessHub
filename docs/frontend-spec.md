...
- **ナビゲーションの段階表示**: サイドバー 9 項目 (§3.0)・ボトムタブ (§6.2) は未実装 phase の項目を**表示しない** (グレーアウトでなく非表示 — 「押せるのに動かない」を作らない qa-018 整合)。ボトムタブのダッシュボード slot は、上記 appr-034 により**着地 `/dashboard` を先頭 slot とする** (2026-07-18 時点の「S09 完成まで『シート (S11)』を先頭にする暫定」を置き換える)。S09 の KPI 部分の確定は従来どおり feat-metrics-tracking の P02 で行い、着地 `/dashboard` の存在とは独立に進む。
...
- **RSC の session scope フォールバックと PrimaryNav 最小シェル (2026-08-08)**: 既定着地の実装定数は当面 `DEFAULT_POST_SIGNIN_LANDING = '/sheets'` (クエリ無し) のまま。`(dashboard)` / `(workspace)` の各 page は `resolveDashboardScope()` で session から tenant/workspace を補完し、`query ?? session` の順で API ヘッダーへ渡す。layout の `PrimaryNav` は到達済み画面への scope 付きリンクだけを出す最小シェルであり、§3.0 サイドバー 9 項目の段階表示契約を置き換えない。詳細は [frontend-scope-reference](features/feat-post-signin-scope-routing/frontend-scope-reference.md) と [spec-reflection-receipt 追補 2026-08-08](features/feat-post-signin-scope-routing/spec-reflection-receipt.md) を参照する。
