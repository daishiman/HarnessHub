...
- **PrimaryNav シェル (2026-08-08)**: ログイン直後にブラウザ操作で他画面へ移れるよう、layout が `PrimaryNav` を描画する。各リンクは解決済み scope をクエリへ引き継ぐ。これはサイドバー 9 項目の段階表示契約 (qa-018) の本実装ではなく、到達性を埋める最小シェルである。
