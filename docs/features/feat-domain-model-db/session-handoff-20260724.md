...
| HEAD（feature/merge tip） | `815c0e2`（`Merge branch 'main' into devgraph/feat-domain-model-db`）— 機械受領書が束縛する tip。本引き継ぎ書を以後 `63ca3cd` で追加し branch tip は前進（doc 追加は spec 影響なし） |
...
- ※本引き継ぎ書 `session-handoff-20260724.md` 自身は commit `63ca3cd` で **PR #53 に追加済み**（PR は計 76 files に更新）。以降 `git status` に本書は現れず、揮発 2 ファイルのみが未 commit で残る。
...
git -C "$R" status --porcelain             # 揮発 2 ファイルのみ（本引き継ぎ書は 63ca3cd で commit 済み）
...
本書は **2026-07-24 セッション時点の記録**として確定している。その後の作業（CI 赤の解消・`origin/main` の再取り込み・follow-up 起票）は後続の引き継ぎ書に分離した。

→ **[session-handoff-20260725.md](./session-handoff-20260725.md)**（次に読むべき文書）

> **なぜ分けたか**: 本リポジトリは qa-070 により **1 文書 300 行が上限**（`scripts/lint-doc-line-limit.py`・CI `change-category-guard` で fail-closed）。追補を本書へ足すと 334 行で上限を超えるため、セッション単位で責務分割した。allowlist への登録は `--ratchet-base origin/main` で新規追加が遮断されるうえ、新規文書を例外に逃がすのは規約の意図に反するため採らない。
