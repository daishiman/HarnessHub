...
| 13 | `publisher_tokens` | id PK, tenant_id, user_id, device_name, refresh_token_hash, scopes_json, family_id, last_used_at, expires_at, revoked_at, created_at | refresh は SHA-256 ハッシュ保存。scopes_json 現行値域は publish:write/metrics:write/feedback:write/aijob:process/docs:write |
