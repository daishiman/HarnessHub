...
source: .dev-graph/plans/feature-package-feat-domain-model-db/goal-spec.json
feature_context_digest: sha256:ed9bbe22a2887731c84470adb8e4f8df41fd7eec46efc58b4dc8e5095b5b5fd5
architecture_refs: [arch-harness-hub-data, arch-harness-hub-infrastructure]
---
...
|---|---|---|
| A1 | スキーマが SQLite 方言互換で D1 接続テストが通る | Drizzle スキーマの migration が libSQL と D1 の両方の接続テスト (CI) で適用成功すること |
| A2 | Release が immutable として強制される | releases への UPDATE が status 列以外で拒否されること (リポジトリ層テスト + DB 制約) |
| A3 | バックアップ export と復元手順が検証済み | 日次 export cron の成果物から staging へ restore し行数・整合検査が通ること (restore drill 記録) |

## 5. 品質制約 (quality_constraints — goal-spec 9 件の確定転記)
...
1. **User 基底テーブルの owner feature 未確定**: quality_constraints の `user-base-table-schema-owner-unresolved-p02` を参照。P02 で本 feature が users 基底スキーマの owner かを確定し、矛盾する記述 (feat-domain-model-db.md 上流未解決節か docs/backend-spec.md §2.2) のいずれかを訂正する。
2. **qa-045 (tenant_data_objects / C4 改訂) は本 digest スコープ外**: system-spec/database.md qa-045 (2026-07-18 確定。C4 データ保持境界改訂による tenant_data_objects テーブル新設と業務ナレッジ/実行入出力データの R2 テナント別封筒暗号化保存) は、本 feature の feature_context_digest (sha256:ed9bbe22a2887731c84470adb8e4f8df41fd7eec46efc58b4dc8e5095b5b5fd5) が拘束する goal-spec の quality_constraints/lineage に含まれない後発確定事項のため、本 baseline のスコープに含めない。P02 で改めて「本 digest スコープ外」と確定させ、dev-graph への follow-up feature candidate として返す。

## 7. 転記元と検証

- 転記元: `.dev-graph/plans/feature-package-feat-domain-model-db/goal-spec.json` (promoted。feature_context_digest = sha256:ed9bbe22a2887731c84470adb8e4f8df41fd7eec46efc58b4dc8e5095b5b5fd5)
- 本文書の受入条件 (P01 acceptance): goal-spec の acceptance 3 件 (§4) と quality_constraints 9 件 (§5) が過不足なく転記され、上流未解決 2 項目 (§6) が明記されていること
