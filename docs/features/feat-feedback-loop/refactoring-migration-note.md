...
- `packages/db/migrations/0007_feedback-loop-builds.sql`
- `packages/db/migrations/meta/0007_snapshot.json`
- `packages/db/migrations/meta/_journal.json`

`main` の documents migration (`0005_common_stepford_cuckoos`) と tenant-data-retention migration (`0006_tenant-data-retention`) を取り込んだ後、`pnpm exec drizzle-kit generate --name feedback-loop-builds` (schema barrel: `packages/db/schema/index.ts`, out: `./migrations`, single-migration-pipeline 契約どおり) で `0007` として生成した。追加したのは `feedbacks` と `builds` の 2 テーブル、各一意・検索用 index である。

## 互換性

- 先行する26テーブル (baseline / auth-tenancy device flow / hearing-intake ai-queue / 共通・顧客持ち込み Google OIDC / documents / tenant-data-retention) への `ALTER` / 削除 / 列変更は無い。migration SQL は `CREATE TABLE feedbacks` / `builds` と各 index のみで構成される。
...
- migration全体は28テーブルとなり、`pnpm exec drizzle-kit generate` のdry-run相当 (barrel再導出DDLとのSQL文字列比較) で差分ゼロを確認した。新規テーブルのみなので既存データのbackfillは不要。

## 検証

- `packages/db/__tests__/migration-lineage.test.ts` (DMDB-T07/T13): migration SQL 適用結果 (28テーブル) と schema barrel 導出 DDL が完全一致することを確認。ハードコードされたテーブル数期待値を26件から28件へ更新した。
- `packages/db/__tests__/backup-restore.test.ts` (P13 production migration CLI): `migrate-deploy.ts --dry-run` → 初回適用 → 再適用の冪等性テストの journal/pending/applied 件数期待値を7件から8件へ更新し、dry-run→初回適用→再適用の全経路が green であることを確認した。
...
- `packages/db/migrations/0007_feedback-loop-builds.sql` + `packages/db/migrations/meta/0007_snapshot.json` + `_journal.json` (`pnpm exec drizzle-kit generate --name feedback-loop-builds` で生成): `CREATE TABLE feedbacks` / `builds` と index のみで構成。main の `0006_tenant-data-retention` を保持し、番号衝突した Feedback/Build 並行生成物を正規連番 `0007` へ再生成した。先行26テーブルへの `ALTER`/削除/列変更は無い。
...
- `apps/hub/src/__tests__/feedback-loop/support/real-db.ts`: `0005_common_stepford_cuckoos.sql`、`0006_tenant-data-retention.sql`、`0007_feedback-loop-builds.sql` を canonical 順で適用し、`FeedbackDbHarness` に `buildsRepository` を追加。
