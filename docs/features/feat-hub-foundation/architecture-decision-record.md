...
- **local 再現 (R-18)**: required status checks と同一コマンドを root の `pnpm verify` で実行できるようにする（qa-039【2】）。追加ゲートは CI と local 入口を同時に用意すること（CI にしか無いゲートは着手前に気づけず、PR で初めて落ちる）。G13 は `pnpm check:client-bundle` を root に用意して同時結線した。**2026-07-25 時点で残っていた未結線 G7 / G7b / G9 は 2026-08-04 (`HarnessHub-yhc3`) に解消**し、`check:ddl` / `check:tenant-isolation-coverage` / `check:connection-isolation` / `check:a11y` を root へ追加して `verify` チェーンへ CI と同じ順序で組み込んだ。これで登録簿の未結線は 0 である（詳細は [docs/shared-layers.md](../../shared-layers.md) の「local からの実行」）。
- **G14 の位置づけ (2026-08-04 追記 / `HarnessHub-yhc3` の作業中に検出)**: `ci.yml` には G14 (OIDC / owner 認可 release contract) が以前から存在したが、`docs/shared-layers.md` の登録簿は G1〜G13 までしか列挙せず、root `verify` にも入口が無かった。逆に `docs/infrastructure-spec.md` §7 の要約表は G14 を持つ一方で G12 を落としており、**2 つの表が別々の項目を欠く双方向のドリフト**になっていた。登録簿 (`shared-layers.md`) を正本と定め G14 を追記、`infrastructure-spec.md` 側は正本を指す写しとして G12 を補い、`pnpm check:auth-release-contract` を root へ結線した。G14 は G4 の内訳を名指しへ引き上げるもので qa-038【2】の「8 種」を増減させないため、`spec-state.json` の改訂は要さない（G12・G13 と同じ扱い）。
