...
| backup 初回成功 | `backup.yml` は実装済みだが **3 回連続で失敗**しており（run 30321679596 / 30293639238 / 30213823182）、R2 に成果物が 1 つも無い。原因は secret ではなく「データ行 0 を不採用」とする判定で、稼働直後の本番 DB は 19 テーブルすべて 0 行のため恒常的に落ちていた（[actions-secrets-2026-07-28.json](actions-secrets-2026-07-28.json)） | 是正版（`verify-export-artifact` CLI への一本化）を main へ land 後、`gh workflow run backup.yml --ref main` で再実行し `db-export/<year>/<stamp>.jsonl.gz` の存在を確認する |
| restore drill | `backup.yml` と手順は実装済みだが、四半期 drill は未実行。**上記のとおり R2 に復元対象の成果物がまだ無い** | 一時 DB への復元と整合検査を実施 |
