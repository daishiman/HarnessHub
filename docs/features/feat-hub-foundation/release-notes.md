...
- **A3（SLO 99.5% 計測と /health 稼働）**: **`/health` の稼働は実測で確認済み**（200 / 全依存 ok）。P10 指摘 F-01 の是正が実環境で有効だったことの裏付けでもある（是正前の設計ならこの応答は 503 だった）。ただし SLO 99.5% の算定には**外形監視による 3 分間隔・1 ヶ月分の時系列**が必要で、これが未取得のため **blocked のまま**。runbook §1 の順序制約は満たしたので、外形監視は有効化してよい。
