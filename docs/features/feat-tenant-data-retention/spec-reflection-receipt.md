...
- **G7b 分離網羅 gate 是正**: `two-tenants` fixture はすでに tenant ごとの DEK、tenant_data object、tombstone を seed していたが、静的対応表に 3 テーブルが無く網羅性を誤判定していた。実際の seed シンボルを対応表へ登録し、fixture と検査の対応を一致させた。
