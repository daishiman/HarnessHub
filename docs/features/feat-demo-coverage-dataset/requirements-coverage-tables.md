...
A7 の機械検査は、全 150 セル (30 route × 5 状態) が「適用」か「非適用 (理由記号あり)」のどちらかで埋まっていることを判定する。空欄・保留は不可とする。
...
| 29 | `/settings/system` | 適用 | 適用 | 適用 | 適用 | 適用 |
| 30 | `/tracking` | 適用 | 適用 | 適用 | 適用 | 適用 |

集計: 適用 115 セル / 非適用 35 セル / 未記入 0 セル (合計 150 セル = 30 route × 5 状態)。
...
## 7. ドメイン enum 一覧 (実測 43 カラム / 135 値)

`packages/db/schema/**/*.ts` を実測した結果である。A2 の機械検査は、以下の全 135 値が seed 後の DB に最低 1 件ずつ存在することを判定する。
...
| `builds.risk_override` | none / warn / blocked (3) ※null は「上書きなし」で enum 値ではない |
...
正本 task spec (`phase-01-requirements.md`) の背景節は、enum の対象を `packages/db/schema/core/catalog.ts` と `packages/db/schema/core/publish.ts` の 9 カラムに限定して記述していた。現行の全 schema を実測すると、対象は 43 カラム / 135 値である。goal-spec の受入条件 A2 は「各ドメインモデルの enum ステータスが全値」と定めており、特定 2 ファイルへの限定を含まない。したがって本書は実測 43 カラム / 135 値を正本とし、後続 phase もこの範囲を対象とする。
...
seed 済み状態から、§6 で「適用」とした全 115 セルへ到達する手順を文書化する。各手順は次を含む。
...
| 引き継ぐもの | 引き継ぎ先の観点 |
|---|---|
| §5 の 30 route | fixture の設計単位 |
| §6 の 150 セル対応表 | 到達手順の作成対象と A7 の検査入力 |
