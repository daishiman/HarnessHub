...
| A2 | 各ドメインモデルの enum ステータスが全値、最低 1 件ずつ seed に含まれる (未使用値 0 件を機械検査) | §7 の 132 値それぞれについて seed 後の DB に 1 件以上存在することを検査スクリプトで判定する |
...
詳細は [requirements-coverage-tables.md](docs/features/feat-demo-coverage-dataset/requirements-coverage-tables.md) を正本とする。本書は目的・スコープ・受入と 28 route 一覧までを持つ。
