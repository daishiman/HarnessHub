...
| # | 手段 | 実施主体 | 備考 |
|---|---|---|---|
| (a) | リポジトリに `ANTHROPIC_API_KEY` secret を設定し workflow を再実行する | **作者** | 認証情報のため AI が代理できない。設定後は workflow 再実行だけで T-A3-03 まで通る見込み |
| (b) | 作者の Windows 実機で、通常のログイン済み環境に install して skill を実行する | **作者** | 最も忠実。ただし実環境に検証用 plugin が入る (事後に `claude plugin uninstall` で除去可能) |
