...
| Review | [final-review-receipt.md](../final-review-receipt.md) | 2026-08-03 再レビューの判定、仕様影響なしの理由、未達の受入条件 |
...
- **未達**: `coefficient.change` は語彙・参照実装のみに存在する。`PATCH /api/v1/tenant/coefficients` は `501` であり、係数変更の実測証跡は無い。
...
- API route: `apps/hub/src/app/api/v1/users/`
