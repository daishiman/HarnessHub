...
| 3 | **CWV 再計測** | 2026-08-02 の run `30736055772` は未認証 401 で失敗したが、現在は署名付き短命 `__cwv_probe` 経路がある。`hub-cwv` を `/catalog` 向けに再実行し、TBT ≤ 200ms を含む acceptance 2 を確定する | `HarnessHub-aqi`。fresh artifact が得られるまで未達を維持し、旧 run を現行の「計測不能」根拠に使わない |
