...
## 5. 品質制約 (quality_constraints — 現行 goal-spec 9 件の確定転記)
...
| rest-zod-single-source-authz-middleware-qa009 | Hub API は REST + OpenAPI とし、zod schema (packages/schemas) を単一ソースとして OpenAPI 仕様を生成する。認可は全 API で Tenant/Workspace スコープを強制する単一ミドルウェアに集約する (deny-by-default)。POST /publish は Idempotency-Key 必須で、直列化違反は POST /publish/:id/submit が 409 を返す。 | system-spec/spec-state.json qa-009 (REST + OpenAPI + zod を採用。Next.js Route Handlers で REST API を実装し、zod schema を単一ソースとして OpenAPI 仕様を生成する); qa-020 (認可は単一ミドルウェア層に集約する [散在させない]); qa-033 (認可は単一ミドルウェア deny-by-default の role×リソースマトリクス); docs/backend-spec-api-state.md §4.6 endpoint 一覧 (POST /api/v1/publish — Draft 作成・Idempotency-Key 必須、POST /api/v1/publish/:id/submit — 直列化違反は 409) |
| targetchannel-serialization-single-inflight-publishrequest | 同一 TargetChannel への PublishRequest は直列化する。Draft は複数作成できるが、先行 request が終端状態 (Published/Failed/Draft 差戻し) になるまで、後続 request の submit は 409 となり Draft に留まる。 | system-spec/spec-state.json qa-002 (不変条件: 同一 TargetChannel への PublishRequest は直列化); qa-009 (同一 TargetChannel への PublishRequest は直列化); docs/backend-spec-api-state.md §5.1 (Draft は channel を占有せず、先行が終端になるまで後続の POST /publish/:id/submit を 409 `channel_busy` とする); features/feat-publish-pipeline.context.json scope_in (『状態機械 + TargetChannel 直列化』) |
| publish-api-dual-principal-csrf-boundary-qa059 | publish API は endpoint ごとの所有者認可契約を固定する。作成・package・submit・cancel と GET list/id は session(Web)+Bearer(CLI)、session 変更系は Origin/CSRF 必須、Bearer は CSRF 非該当。approve は session/workspace-admin、deployment は Bearer/owner。cancel はどちらの credential でも owner・tenant/workspace scope・resource ownership を同一に保つ。Authorization がある invalid/expired Bearer は cookie session へ fallback しない。 | system-spec/spec-state.json qa-059 (dual-principal と role/scope 境界。cancel の session 対応は Web Needs Fix 再投入の additive follow-up); docs/backend-spec.md §4.6; docs/security-spec.md §6.3/§7.3; architecture/harness-hub-backend.md; architecture/harness-hub-security.md; release-record.md production dual-principal smoke |
...
- 本文書の受入条件 (P01 acceptance): 現行 goal-spec の acceptance 3 件 (§4) と quality_constraints 9 件 (§5) が過不足なく転記され、cross-feature 境界 3 項目 (§6) が明記されていること
