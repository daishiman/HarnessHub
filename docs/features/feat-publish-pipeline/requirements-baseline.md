...
| publish-api-dual-principal-csrf-boundary-qa059 | publish API は endpoint ごとの所有者認可契約を固定する。作成・package・submit と GET list/id は session(Web)+Bearer(CLI)、session 変更系は Origin/CSRF 必須、Bearer は CSRF 非該当。approve は session/workspace-admin、cancel と deployment は Bearer/owner。Authorization がある invalid/expired Bearer は cookie session へ fallback しない。 | system-spec/spec-state.json qa-059; docs/backend-spec.md §4.6; docs/security-spec.md §6.3/§7.3; architecture/harness-hub-backend.md; architecture/harness-hub-security.md; release-record.md production dual-principal smoke |
