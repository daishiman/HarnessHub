...
| 1.5 | サインイン完了後、自動的に業務画面へ着地する | S07 → S00.LANDING (`/dashboard`) | 遷移元 path があればそこへ、無ければ `/dashboard`。着地画面は自分が最後に触ったものを種別横断で示し、既存業務画面へ進める。通常時に KPI は出さない。同一 origin の相対 path のみ許可し、不正な戻り先は既定着地へフォールバック (open redirect 防止)。複数 Workspace 未選択時は `/` で選択してから進む。[運用 Runbook](features/feat-post-signin-scope-routing/operations-runbook.md) 参照 | qa-135, qa-137, qa-170, appr-034, appr-035, feat-post-signin-scope-routing |
...
| 1.7 | 認証基盤が使えないときも、自分の操作ミスだと誤解しない | 着地ダッシュボード / サインイン | 秘密の未解決などで認証が縮退した場合、「サインインしてください」と一律表示して無限の再試行へ誘導しない。権限不足 (403) では再サインインへ誘導しない (解決しないためループになる) | qa-162, qa-175, docs/features/feat-post-signin-landing-surface/landing-observability-investigation.md |
...
1. S10 ヒアリングウィザードに業務課題を入力 (4 大工程は基本情報→業務詳細→要件→確認。入力負荷を下げるため、実画面は基本情報/現状/用途プロファイル/要望パターン/参考URL・添付/要望/整理・まとめ/確認の8画面に分割。削減効果を自動試算) → HS 受付番号・「生成中」・「シートを見る」を表示
