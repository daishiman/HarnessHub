 ...
 * 「テストは通るが本番で落ちる」形になる。
 * 0005 は `encryption_keys` (core domain, `createCoreRepositories` が組む DEK 台帳) へ
 * `tenant_id` 列を足す (feat-tenant-data-retention AD-1)。tenant_data 関連の表を作らない用途でも、
 * `ColumnCipher` が発行する DEK 検索 query は列の存在を前提にするため必須。載せないと
 * 「no such column: encryption_keys.tenant_id」で全 real-db harness が落ちる。
 * 0006 は tenant_data 専用の tombstones 表のみを追加する (認証経路は触らないが、0005 と対になる
 * migration を分離させない意図で連番のまま載せる)。
 */
const MIGRATIONS = [
  ...
  '0005_tenant-data-retention-envelope-encryption.sql',
  '0006_tenant-data-tombstones.sql',
];
