 ...
 *
 * docs/backend-spec.md §3.8 が確定済みなのは 6 語 (`publish.request` / `publish.approve` /
 * `publish.reject` / `channel.promote` / `channel.rollback` / `release.suspend`)。
 * submit・package upload・cancel・deployment 登録に対応する語は §3.8 に無いため、
 * 既存語の命名規則 (`<資源>.<操作>`) に揃えてここで確定させ、§3.8 への追記を follow-up として記録する。
 */
...
export const PUBLISH_AUDIT_ACTIONS = {
  ...
  /** PUT /api/v1/publish/:id/package (§3.8 未確定 — 本 feature で確定) */
  packageUpload: 'publish.package_upload',
  /** POST /api/v1/publish/:id/submit (§3.8 未確定 — 本 feature で確定) */
  submit: 'publish.submit',
  /** POST /api/v1/publish/:id/approve (§3.8 確定語) */
  approve: 'publish.approve',
  /** POST /api/v1/publish/:id/cancel (§3.8 未確定 — 差戻し語 publish.reject とは別に持つ) */
  ...
  /** POST /api/v1/projects/:id/deployment (§3.8 未確定 — 本 feature で確定) */
