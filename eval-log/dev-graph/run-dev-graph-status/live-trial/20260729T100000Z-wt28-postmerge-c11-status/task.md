...
Skill({skill: "dev-graph:run-dev-graph-status", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-28/eval-log/dev-graph/live-trial-fixtures/wt28-postmerge-status"})
...
被験 fixture は `/Users/dm/orca/workspaces/HarnessHub/wt-28/eval-log/dev-graph/live-trial-fixtures/wt28-postmerge-status` にある dev-graph 初期化済みの独立 Git repository です。

## goal-seek 配線の必須履行 (省略禁止)

`run-dev-graph-status` は `goal_seek` を宣言します。SKILL.md の `## ゴールシーク実行` に従い、`/Users/dm/orca/workspaces/HarnessHub/wt-28/eval-log/dev-graph/live-trial-fixtures/wt28-postmerge-status/eval-log/` へ 3 点セット (`run-dev-graph-status-goal-spec.json` / `run-dev-graph-status-progress.json` / `run-dev-graph-status-intermediate.jsonl`、intermediate は必須 6 キー) をすべて書き出してください。
...
1. `/Users/dm/orca/workspaces/HarnessHub/wt-28/eval-log/dev-graph/run-dev-graph-status/live-trial/20260729T100000Z-wt28-postmerge-c11-status/out/status.json` に完了マーカーを 1 ファイルだけ Write する。内容:
