...
`Skill({skill: "dev-graph:run-dev-graph-schedule", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/live-trial-fixtures/20260802-wt32-schedule-final --max-parallel 2"})`
...
完了時は `/Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/run-dev-graph-schedule/live-trial/20260802T083500Z-wt32-schedule-final/out/status.json` だけを `{"status":"PASS|FAIL|ERROR","scenario":"C15-OUT1-positive-ready-set-r16"}` 形式で書き、最後に `DONE: <status>` と 1 行だけ報告してください。途中で人間に質問せず最後まで自走し、skill の手順に忠実に従い、人手の追加判断・省略をしないでください。fixture 以外の repository は変更しないでください。
