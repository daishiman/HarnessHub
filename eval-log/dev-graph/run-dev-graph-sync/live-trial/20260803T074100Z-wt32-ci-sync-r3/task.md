...
scenario `C03-OUT1-positive-second-sync-zero` を独立 fixture `/Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/live-trial-fixtures/20260803-wt32-ci-sync-r3` だけで検証する。途中で質問せず最後まで自走し、skill の手順を省略しない。
...
Skill({skill: "dev-graph:run-dev-graph-sync", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/live-trial-fixtures/20260803-wt32-ci-sync-r3 と続けて --dry-run / --apply / 確認 --dry-run の 3 パスを同じ入力で回す (決定論 remote は fixture 内 .dev-graph/remote.json)"})
...
HarnessHub 本体を変更せず、`out/` には最後の status 1 件だけを書く。全工程後、`/Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/run-dev-graph-sync/live-trial/20260803T074100Z-wt32-ci-sync-r3/out/status.json` へ `{"status":"PASS|FAIL|ERROR","scenario":"C03-OUT1-positive-second-sync-zero"}` を Write し、「DONE: <status>」と 1 行だけ報告する。
