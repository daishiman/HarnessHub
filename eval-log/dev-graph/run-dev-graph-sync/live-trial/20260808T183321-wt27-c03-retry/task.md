...
FIXTURE = `/Users/dm/orca/workspaces/HarnessHub/wt-27-3/eval-log/dev-graph/run-dev-graph-sync/live-trial/20260808T183321-wt27-c03-retry/fixture-repo`
...
外部依存ゼロは補助確認にも適用します。`gh-bridge.py` の `issue-fetch` 等を `--dry-run` で呼んでも
read は抑止されず実 `gh` を起動するため、**検証目的を含めて `gh-bridge.py` と `gh` を一度も実行しては
いけません**。IN1 は graph schema gate と、`--remote-state` adapter を通した sync report 内の
fixture receipt / operation validation で確認してください。transcript に `gh` subprocess の起動が
1 回でもあれば、write が 0 件でも FAIL としてください。
...
Skill({skill: "dev-graph:run-dev-graph-sync", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-27-3/eval-log/dev-graph/run-dev-graph-sync/live-trial/20260808T183321-wt27-c03-retry/fixture-repo --dry-run"})

パス 2 (適用):

Skill({skill: "dev-graph:run-dev-graph-sync", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-27-3/eval-log/dev-graph/run-dev-graph-sync/live-trial/20260808T183321-wt27-c03-retry/fixture-repo --apply"})
...
全検証後、
`/Users/dm/orca/workspaces/HarnessHub/wt-27-3/eval-log/dev-graph/run-dev-graph-sync/live-trial/20260808T183321-wt27-c03-retry/out/status.json`
