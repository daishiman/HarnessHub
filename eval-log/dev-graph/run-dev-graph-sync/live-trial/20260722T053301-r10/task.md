...
隔離 fixture repo `/Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260720-203452-wt-5/eval-log/dev-graph/live-trial-fixtures/20260722T053301-syncsched` だけを対象にしてください。外部 GitHub / Beads へ接続しません。

## 必須の開始操作

あなたの**最初の workflow tool call**は、ロード済みの `Skill` tool による次の対象 Skill 呼び出しでなければなりません。SKILL.md を検索して手作業で再現することは代替になりません。

`Skill({skill: "dev-graph:run-dev-graph-sync", args: "sync --repo-root /Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260720-203452-wt-5/eval-log/dev-graph/live-trial-fixtures/20260722T053301-syncsched --binding github --adapter-fixture /Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260720-203452-wt-5/eval-log/dev-graph/live-trial-fixtures/20260722T053301-syncsched/.dev-graph/remote.json --repeat 2"})`
...
- R3-sync と別の fresh `Agent` verifier に、上記 C24 receipt、pass1、pass2、最終 graph、remote、canonical sync-snapshot を**実際に読ませて**再計算させる。verifier の出力は fixture の `eval-log/run-dev-graph-sync-independent-verifier-r9.json` に保存し、`verified=true` でなければ FAIL とする。
- goal/progress/intermediate の hash 検査も実行する。

scenario ID は `C03-OUT1-positive-second-sync-zero`。

処理が終了（成功・失敗・中断のいずれでも）したら、`/Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260720-203452-wt-5/eval-log/dev-graph/run-dev-graph-sync/live-trial/20260722T053301-r10/out/status.json` だけを out/ に Write し、`{"status":"PASS|FAIL|ERROR","scenario":"sync-positive-two-pass-convergence"}` とする。最後は `DONE: <status>` と1行だけ報告する。
