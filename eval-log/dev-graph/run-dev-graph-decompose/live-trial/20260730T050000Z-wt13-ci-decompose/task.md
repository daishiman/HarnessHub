...
## retry で最初に解消する blocker

直前 run は none binding で2回目の target Skill を呼んだ後、監査で
`confirmation_evidence.evaluated_digest` の不一致を検出し、C02 upsert で後追い補正したため
FAIL になった。本 retry では beads / none の各系列で、**2回目 Skill より前に**次を順番どおり
完了すること。

1. 昇格後の full node を確定し、`confirmation_evidence` キーだけを完全に除外した値から、
   `json.dumps(payload, ensure_ascii=False, sort_keys=True, separators=(',', ':'))` の
   UTF-8 SHA-256 を計算する。
2. その digest を同じ full node の `confirmation_evidence.evaluated_digest` に入れて C02 へ
   1 回 upsert する。
3. persisted node を読み戻し、同じ方法で再計算して declared digest と一致することを assert
   する。不一致なら status=FAIL を書いて直ちに終了し、補正 upsert も2回目 Skill も行わない。
...
`/Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730`
...
`/Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-beads-20260730`
...
`b37619d2f1f732a076174644b8ceceac371fd798793cfdffe3f15bad36ec9a7b`
...
未達 responsibility の prompt を読み、`goal_seek.fork: subagent` 契約どおり
`Agent` ツールで分離 context の独立検証を少なくとも 1 回実行してください。
トップレベルの自己検証で代替してはいけません。

次に Git index 登録済み共通監査ヘルパーで pre-state を取ってください。

```bash
python3 /Users/dm/orca/workspaces/HarnessHub/wt-13/plugins/dev-graph/tests/fixtures/audit_decompose_live_trial.py snapshot \
  --repo-root /Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730 \
  --output /Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730/eval-log/pre-state.json
```
...
Skill({skill: "dev-graph:run-dev-graph-decompose", args: "ユーザー登録とログインができて、ログイン後にダッシュボードで自分の利用状況を見られる小規模 Web アプリを作りたい。登録完了時と重要な変更時には通知メールを送りたい。運営者向けには全ユーザーの利用状況を集計したレポート画面もほしい。 --repo-root /Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730"})
...
`/Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730/eval-log/macro-preview.json`

preview graph を C11 検証するときは、管理対象 repo へ一時ファイルを書かず **stdin 経路**を
使ってください (observation 5 の後半)。

```bash
python3 /Users/dm/orca/workspaces/HarnessHub/wt-13/plugins/dev-graph/scripts/validate-graph-schema.py --graph - --repo-root /Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730 < /Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730/eval-log/macro-preview.json
```
...
- `confirmation_evidence.evaluated_digest` は、昇格後の最終 node から
  `confirmation_evidence` だけを除き、Python の
  `json.dumps(payload, ensure_ascii=False, sort_keys=True, separators=(',', ':'))`
  で直列化した UTF-8 バイト列の SHA-256 としてください。`confirmation_status` /
  `evaluation_status` / `implementation_readiness` を含む全 patch 内容を先に確定し、
  その最終形から digest を計算して、同じ `upsert-node.py --input` patch の
  `confirmation_evidence` に入れてください。固定の 64 桁値や昇格前 node の digest は禁止です。
  digest 計算前に、patch の `updated_at` と
  `implementation_readiness.checked_at` を固定した同じ時刻文字列へ明示設定してください。
  C02 writer が `updated_at` を後から自動更新すると digest が変わるためです。upsert 直後、
  persisted node から digest を再計算して一致を確認し、不一致なら 2 回目の Skill 呼出しへ
  進まず、この run を FAIL としてください。
...
```bash
python3 /Users/dm/orca/workspaces/HarnessHub/wt-13/plugins/dev-graph/tests/fixtures/audit_decompose_live_trial.py audit \
  --repo-root /Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730 \
  --preview /Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730/eval-log/macro-preview.json \
  --scenario /Users/dm/orca/workspaces/HarnessHub/wt-13/plugins/dev-graph/tests/fixtures/live-trial-positive-scenarios.json \
  --pre-state /Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730/eval-log/pre-state.json \
  --plugin-dir /Users/dm/orca/workspaces/HarnessHub/wt-13/plugins/dev-graph \
  --run-mode apply \
  --run-binding none \
  --output /Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730/eval-log/decompose-audit.json
```
...
一致するかを fixture の git 差分 (`git -C /Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/live-trial-fixtures/wt13-ci-decompose-none-20260730 status --short`) からも確認してください。
...
`/Users/dm/orca/workspaces/HarnessHub/wt-13/eval-log/dev-graph/run-dev-graph-decompose/live-trial/20260730T050000Z-wt13-ci-decompose/out/status.json`
