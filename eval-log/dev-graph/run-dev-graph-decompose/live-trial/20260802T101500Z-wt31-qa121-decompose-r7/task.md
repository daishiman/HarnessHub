# C14 live trial qa-123 r7 — latest-main deterministic replay, paired beads/none, no nudge
...
- plugin: `/Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph`
- scenario file: `/Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/tests/fixtures/live-trial-positive-scenarios.json`
- audit helper: `/Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/tests/fixtures/audit_decompose_live_trial.py`
- run workdir: `/Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/run-dev-graph-decompose/live-trial/20260802T101500Z-wt31-qa121-decompose-r7`
- beads fixture: `/Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-beads-r7`
- none fixture: `/Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r7`
...
現在の r7 run workdir、r7 replay bundle、r7 beads/none fixture に対する読み取り専用の
存在・初期状態確認は step 1 の preflight に含めてよい。過去の live-trial run、過去
transcript、r7 以外の fixture は調査・参照してはならない。
...
4. **target Skill①** を呼ぶ。現行 Claude Code 2.1.62 は plugin namespaced 名を launcher で
   直接解決しないため、beads 系列の最初だけ compatibility probe として
   `Skill({skill: "dev-graph:run-dev-graph-decompose", args: "<want> --repo-root <fixture>"})`
   を正確に 1 回呼ぶ。結果が `Unknown skill` であることを確認し、これは target の成功起動数へ
   含めない。その直後に、実在 alias の
   `Skill({skill: "run-dev-graph-decompose", args: "<want> --repo-root <fixture>"})` を呼ぶ。
   以後の Skill①/② は全て短縮名を使い、`Launching skill` tool result を持つ成功起動が
   beads=2 / none=2 / 合計4、失敗 compatibility probe が beads=1 の状態にする。
...
10. 次の正規 helper だけで `feat-user-auth-001` の promotion patch を作り、C02 へ 1 回
    ...
    ```bash
    python3 /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/tests/fixtures/build_decompose_promotion_patch.py build --repo-root <fixture> --node-id feat-user-auth-001 --output eval-log/inputs/promote-feat-user-auth-001.json --checked-at 2026-08-02T07:30:00Z --evaluator run-dev-graph-decompose/live-trial --evidence-ref eval-log/macro-preview.json
    python3 /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/scripts/upsert-node.py --repo-root <fixture> --input eval-log/inputs/promote-feat-user-auth-001.json
    python3 /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/tests/fixtures/build_decompose_promotion_patch.py verify --repo-root <fixture> --node-id feat-user-auth-001
    ```
...
- evidence は Explore response の architecture 1 + feature 4 + task 0、3 dependency edge、
  ...
  Task tool（Read/Grep/Glob のみ）と file write 0 を含む
...
`Launching skill` 成功起動が beads=2 / none=2 / 合計4、namespaced compatibility probe の
`Unknown skill` が beads の最初に正確に1件、Task が beads=1 / none=1 / 合計2、Task の nested
...
`/Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/run-dev-graph-decompose/live-trial/20260802T101500Z-wt31-qa121-decompose-r7/out/status.json`
