# タスク: dev-graph:run-dev-graph-decompose の実走 (scenario C14-OUT1-positive-macro-decomposition-r7, paired bindings=beads/none, no-intervention final)

## 成功証跡で固定された no-intervention 順序

過去の PASS run と同じ no-intervention 契約を再現する。beads / none の各系列で、
**2回目 Skill より前に**次を順番どおり完了すること。

1. 下記の正規 helper で、昇格後の full node から `confirmation_evidence` だけを除いた
   `json.dumps(payload, ensure_ascii=False, sort_keys=True, separators=(',', ':'))` の
   UTF-8 SHA-256 を計算し、promotion patch を作る。手計算や shell 内 Python へ置換しない。
2. helper が作った patch を C02 `upsert-node.py --input` へ 1 回だけ渡す。
3. helper の `verify` で persisted node を読み戻し、declared digest と expected digest の一致を
   assert する。不一致なら status=FAIL を書いて直ちに終了し、補正 upsert も2回目 Skill も行わない。
4. 一致を確認した場合だけ2回目 Skill を呼ぶ。**2回目 Skill 後の node patch / upsert /
   digest 補正は理由を問わず禁止**。監査が落ちた場合も後追い修復せず FAIL で終了する。

この順序は transcript の timestamp で検査する。最終 audit が緑でも、2回目 Skill 後の
node mutation が1件あれば goal_fit=FAIL とする。

この paired run は scenario `C14-OUT1-positive-macro-decomposition-r7` の充足を確認するものです。
...
`/Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2`
...
## この run の binding は `none` — node 宣言で決まる (設定では決まらない)
...
**本 run で C02 へ登録する全ての node の `tracker_binding` を `"none"` にしてください。**
...
とくに本 retry は、初回だけでなく lifecycle 昇格後の冪等再実行も、必ず次の形の
**2 回目の Skill ツール呼出し**として transcript に残すことが合格条件です。

```text
Skill({skill: "dev-graph:run-dev-graph-decompose", args: "<同じ want> --repo-root <同じ fixture>"})
```

`upsert-node.py` を Bash から直接 5 回再送する操作は、2 回目の Skill 呼出しの代用になりません。
2 回目の Skill 内部で C02 writer を使う場合は、各 `graph_node_id` の最新 persisted node を
読み直し、全 5 node を `operation=noop` / `write_count=0` へ収束させてください。

## 同一 closure での paired beads run

observation 4 を同じ behavior closure で証明するため、次の独立 fixture でもこの task 内に
書かれた全手順を同じ順序で実行してください。

`/Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-beads-r2`

paired fixture では唯一の意図的な差として、全 node の `tracker_binding` を `"beads"` とし、
監査の `--run-binding` も `beads` にしてください。それ以外の want、target Skill、2 回の
Skill 呼出し、lifecycle 昇格、schema validation、goal-seek、監査、negative controls は
none fixture と同一です。paired fixture の監査出力は
`eval-log/decompose-audit.json`、preview は `eval-log/macro-preview.json`、
pre-state は `eval-log/pre-state.json` に保存してください。

実行順は `beads` fixture の全手順（初回 Skill → 昇格 → 2 回目 Skill → 監査）を完了してから、
`none` fixture の全手順を同様に行ってください。両 fixture は別 Git repository であり、
別 run として前後差分を測ります。各 fixture の transcript 上で target Skill 呼出しが正確に
2 回、合計 4 回あることを最後に確認してください。

behavior closure digest は target Skill 内で手計算しないでください。これは outer runner が
`live-trial-verdict.py` で機械計算し、現在値
`33a2988db3b8a9b5514d1a48ec61981caadced1cdf93c705ffe6c74b016dd45a`
へ束縛します。target run は両 audit の `audit_implementation.sha256` が一致することだけを
確認し、goal-seek と status marker の作成へ進んでください。
...
```bash
python3 /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/tests/fixtures/audit_decompose_live_trial.py snapshot \
  --repo-root /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2 \
  --output /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2/eval-log/pre-state.json
```
...
Skill({skill: "dev-graph:run-dev-graph-decompose", args: "ユーザー登録とログインができて、ログイン後にダッシュボードで自分の利用状況を見られる小規模 Web アプリを作りたい。登録完了時と重要な変更時には通知メールを送りたい。運営者向けには全ユーザーの利用状況を集計したレポート画面もほしい。 --repo-root /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2"})
...
`/Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2/eval-log/macro-preview.json`

preview graph を C11 検証するときは、管理対象 repo へ一時ファイルを書かず **stdin 経路**を
使ってください (observation 5 の後半)。

```bash
python3 /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/scripts/validate-graph-schema.py --graph - --repo-root /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2 < /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2/eval-log/macro-preview.json
```
...
- `confirmation_evidence.evaluated_digest` は、昇格後の最終 node から
  `confirmation_evidence` だけを除いた正準 JSON の SHA-256 とする。各 fixture で次を実行し、
  `<fixture>` は対象の beads / none fixture 絶対 path へ置換する。

  ```bash
  python3 /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/tests/fixtures/build_decompose_promotion_patch.py build \
    --repo-root <fixture> --node-id feat-user-auth-001 \
    --output eval-log/inputs/promote-feat-user-auth-001.json \
    --checked-at 2026-08-02T06:31:00Z \
    --evaluator run-dev-graph-decompose/live-trial \
    --evidence-ref eval-log/macro-preview.json
  python3 /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/scripts/upsert-node.py \
    --repo-root <fixture> --input eval-log/inputs/promote-feat-user-auth-001.json
  python3 /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/tests/fixtures/build_decompose_promotion_patch.py verify \
    --repo-root <fixture> --node-id feat-user-auth-001
  ```

  helper は `updated_at` と `implementation_readiness.checked_at` を同じ固定値にし、最終 patch
  shape から digest を導出する。固定 64 桁値、昇格前 node の digest、shell 内の再実装は禁止。
  `verify` が非0なら 2 回目の Skill 呼出しへ進まず、この run を FAIL とする。
- 昇格後、publication 経路を通すために被験 skill を同じ args でもう一度 Skill ツールで実行して
  ください。冪等性が要求されているので、二度目の実行で node が重複してはいけません。
  二度目の Skill 実行では、まず graph から同じ `graph_node_id` の最新 persisted node を読み直し、
  full node snapshot を C02 へ渡すなら `status` / `confirmation_status` /
  `evaluation_status` / `confirmation_evidence` / `implementation_readiness` を最新値のまま保持して
  ください。生成直後の draft before-image を再送して stale lifecycle guard に拒否された状態は
  冪等成功ではありません。全5ノードが `operation=noop` / `write_count=0` へ収束しない場合は
  FAIL とし、成功扱いにしないでください。
  2 回目の Skill 呼出し後は node を一切 patch しないでください。監査が fail した場合も
  後追い補正はせず、status=FAIL として終了してください。
...
```bash
python3 /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/tests/fixtures/audit_decompose_live_trial.py audit \
  --repo-root /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2 \
  --preview /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2/eval-log/macro-preview.json \
  --scenario /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph/tests/fixtures/live-trial-positive-scenarios.json \
  --pre-state /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2/eval-log/pre-state.json \
  --plugin-dir /Users/dm/orca/workspaces/HarnessHub/wt-31/plugins/dev-graph \
  --run-mode apply \
  --run-binding none \
  --output /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2/eval-log/decompose-audit.json
```

`--run-binding` は観測ではなく申告です。監査は graph に永続した `tracker_binding` を実測し、
申告と食い違えば `run_binding_attested: false` で落とします。宣言を `none` にし忘れたまま
申告だけ合わせても緑にはなりません。

監査結果の `pass` だけを根拠に成否を決めてはいけません。次の 7 点を
...
6. **証跡束縛 (observation 6)**: 昇格 feature の最終 persisted node から上記正準手順で
   digest を自分でも再計算し、`evidence_binding.all_bound=true` と
   `declared_digest == expected_digest` を報告してください。
7. **反証可能性 (observation 7)**: `gate_negative_controls.executed=true` /
   `all_rejected=true` と、2 control の `clause_fired=true` を報告してください。

write count は共通監査が前後状態の差分から導きますが、その値が実際に作成されたファイル数と
一致するかを fixture の git 差分 (`git -C /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-decompose-none-r2 status --short`) からも確認してください。
...
`/Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/run-dev-graph-decompose/live-trial/20260802T063000Z-wt31-qa121-decompose-r2/out/status.json`

内容:

`{"status": "PASS|FAIL|ERROR", "scenario": "C14-OUT1-positive-macro-decomposition-r7", "binding": "paired:beads+none"}`
