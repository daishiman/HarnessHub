...
```
/Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260726-011205-wt-2/eval-log/dev-graph/run-dev-graph-decompose/live-trial/20260726T031729Z-mfh7-decompose-gatefix/out/status.json
```
...
1. **1 件目の feature を「完全昇格」させる。** 次を**すべて**満たす必要があります。
   ...
   - `confirmation_evidence` の `evaluator` / `evidence_ref` / `evaluated_digest` を
     すべて非空にする。`evaluated_digest` は **64 桁の小文字 16 進**である必要があります。
...
- `publication.projection_targets` の `beads` と `github` が 0 件で、`none` に昇格 node
  だけが入ること (binding 解決が `none` へ倒れた結果として外部投影候補が空であること)

加えて `publication.discriminating` が true (両クラスが揃った) であることを記載して
ください。`publication.readiness_clause_note` は schema 由来の注記であり証拠ではないので
evidence に使わないでください。
...
```bash
ls -1 /Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260726-011205-wt-2/eval-log/dev-graph/run-dev-graph-decompose/live-trial/20260726T031729Z-mfh7-decompose-gatefix/out/
```
