...
被験 fixture は `/Users/dm/orca/workspaces/HarnessHub/wt-28/eval-log/dev-graph/live-trial-fixtures/wt28-postmerge-init-r3` にある、まだ dev-graph 初期化されていない独立 Git repository です。
...
```
Skill({skill: "dev-graph:run-dev-graph-init", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-28/eval-log/dev-graph/live-trial-fixtures/wt28-postmerge-init-r3 --hook-source plugin"})
```
...
`run-dev-graph-init` は `goal_seek` を宣言します。`/Users/dm/orca/workspaces/HarnessHub/wt-28/eval-log/dev-graph/live-trial-fixtures/wt28-postmerge-init-r3/eval-log/` に次の3点を出力してください。
...
全検証後、`/Users/dm/orca/workspaces/HarnessHub/wt-28/eval-log/dev-graph/run-dev-graph-init/live-trial/20260729T105300Z-wt28-postmerge-c11-init/out/status.json` に完了マーカーを1ファイルだけWriteしてください。
...
## retry safety（初回の失格原因を再発させない）

- fixture root には指定された正規ディレクトリ以外を作らない。`-la`、`-p`、`echo`、`ls`、`mkdir`、`Directories created` のようなコマンド断片名のディレクトリが生じたら、その run は FAIL とする。
- `eval-log/run-dev-graph-init-goal-spec.json` と`eval-log/run-dev-graph-init-intermediate.jsonl` の第 1 行は harness が trial 送信前に正しい内容で固定済み。goal spec と第 1 行を Write / Update / Edit してはならない。読み取って検証し、Skill 実行後の結果を intermediate の第 2 行として append するだけにする。
