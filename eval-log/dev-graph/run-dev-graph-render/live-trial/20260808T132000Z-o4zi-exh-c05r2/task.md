...
**最重要の計測契約:** この task.md を読み終えた直後の最初の execution tool_use は、下記の literal Skill(dev-graph:run-dev-graph-render) でなければなりません。fixture の Read/Search/Bash は Skill tool_use が成功した後にだけ実行してください。また、ロードされた SKILL.md の goal_seek.fork=subagent 契約を省略せず、必ず Agent tool を1回以上使って未充足責務を独立検証し、その結果を progress と最終判定へ反映してください。Skill より前の実行操作または Agent 呼出し0回なら、成果物が正しくても FAIL です。

この run は scenario `C05-OUT1-positive-feature-progress` の充足を確認するものです。

FIXTURE = `/private/tmp/harnesshub-o4zi.Gv5hFK/eval-log/dev-graph/run-dev-graph-render/live-trial/20260808T132000Z-o4zi-exh-c05r2/fixture-repo`
...
Skill({skill: "dev-graph:run-dev-graph-render", args: "--repo-root /private/tmp/harnesshub-o4zi.Gv5hFK/eval-log/dev-graph/run-dev-graph-render/live-trial/20260808T132000Z-o4zi-exh-c05r2/fixture-repo"})
...
全検証後、
`/private/tmp/harnesshub-o4zi.Gv5hFK/eval-log/dev-graph/run-dev-graph-render/live-trial/20260808T132000Z-o4zi-exh-c05r2/out/status.json`
