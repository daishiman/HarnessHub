...
`/Users/dm/orca/workspaces/HarnessHub/wt-26/eval-log/dev-graph/live-trial-fixtures/pr601-node-r4-092856`

管理対象 graph/config/content root を手で直接編集してはいけません。

以下を実行してください:

Skill({skill: "dev-graph:run-dev-graph-node", args: "add --repo-root /Users/dm/orca/workspaces/HarnessHub/wt-26/eval-log/dev-graph/live-trial-fixtures/pr601-node-r4-092856 --input /Users/dm/orca/workspaces/HarnessHub/wt-26/eval-log/dev-graph/live-trial-fixtures/pr601-node-r4-092856/mixed-artifacts.json"})
...
2. 原文本文は JSON ファイルから読み、shell source へ埋め込まない。各 original body が保存 Markdown
   ...
   各 node の writer 入力には、対応する `mixed-artifacts.json` の `body` を top-level `body`
   として一字も変えずに渡すこと。新規5件すべての receipt が `body_source=from_input` でなければ
   FAIL とする。5件それぞれについて `original_body in saved_markdown_body` を機械判定し、
   API token だけの部分検査で全文保持を代替してはならない。
...
`goal-spec.json` は immutable な original goal と契約だけを保持し、`pending` のような実行状態を
残さないでください。実行状態の正本は `progress.json` です。`progress.json` には少なくとも
`steps`、`checklist` (CL1〜CL5)、`criteria` (OUT1) を持たせ、完了時に全要素へ
`completed` または `PASS` と具体的な実測 `evidence` を設定してください。完了マーカーを書く前に
再帰的に `pending` 件数 0、`evidence: null` 件数 0 を機械検査し、どちらかが残れば FAIL としてください。

## 完了契約

処理が終了 (成功 / 失敗 / 中断いずれでも) したら:

1. `/Users/dm/orca/workspaces/HarnessHub/wt-26/eval-log/dev-graph/run-dev-graph-node/live-trial/20260729T092856Z-pr601-node-r4/out/status.json`
