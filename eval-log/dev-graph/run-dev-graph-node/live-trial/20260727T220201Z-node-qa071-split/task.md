...
被験 fixture は `/private/tmp/harnesshub-qa071-scope-clean.RwEmYZ/eval-log/dev-graph/live-trial-fixtures/node-qa071-split` にある dev-graph 初期化済みの独立 Git repository です。
`plugins/dev-graph/tests/fixtures/build_live_trial_fixture.py --kind node` が生成した正本 fixture であり、内容を書き換えてはいけません。

以下を実行してください:

Skill({skill: "dev-graph:run-dev-graph-node", args: "add --repo-root /private/tmp/harnesshub-qa071-scope-clean.RwEmYZ/eval-log/dev-graph/live-trial-fixtures/node-qa071-split --input /private/tmp/harnesshub-qa071-scope-clean.RwEmYZ/eval-log/dev-graph/live-trial-fixtures/node-qa071-split/mixed-artifacts.json"})
...
`run-dev-graph-node` は `goal_seek` を宣言します。SKILL.md の `## ゴールシーク実行` に従い、`/private/tmp/harnesshub-qa071-scope-clean.RwEmYZ/eval-log/dev-graph/live-trial-fixtures/node-qa071-split/eval-log/` へ 3 点セット (`run-dev-graph-node-goal-spec.json` / `run-dev-graph-node-progress.json` / `run-dev-graph-node-intermediate.jsonl`、intermediate は必須 6 キー) をすべて書き出してください。
...
1. `/private/tmp/harnesshub-qa071-scope-clean.RwEmYZ/eval-log/dev-graph/run-dev-graph-node/live-trial/20260727T220201Z-node-qa071-split/out/status.json` に完了マーカーを 1 ファイルだけ Write する。内容:
