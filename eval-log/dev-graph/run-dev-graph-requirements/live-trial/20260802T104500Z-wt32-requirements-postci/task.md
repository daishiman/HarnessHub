...
fixture: `/Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/live-trial-fixtures/20260802-wt32-requirements-postci`

必ず次の Skill ツールを 1 回起動してください。内部 script の直実行で skill 本体を代替してはいけません。

`Skill({skill: "dev-graph:run-dev-graph-requirements", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/live-trial-fixtures/20260802-wt32-requirements-postci --feature-id F-LIVE-001 (package は fixture 内 system-plan/F-LIVE-001/ を skill が解決する)"})`
...
完了時は `/Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/run-dev-graph-requirements/live-trial/20260802T104500Z-wt32-requirements-postci/out/status.json` だけを `{"status":"PASS|FAIL|ERROR","scenario":"C04-OUT1-positive-ready-handoff"}` 形式で書き、最後に `DONE: <status>` と 1 行だけ報告してください。fixture の `out/status.json` へ書いて代替してはいけません。途中で人間に質問せず最後まで自走し、skill の手順に忠実に従い、人手の追加判断・省略をしないでください。
