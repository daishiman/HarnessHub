...
verifier の出力は fixture の `eval-log/run-dev-graph-requirements-independent-verifier-r6.json` に保存し、`verified=true` でなければ FAIL とする。**True をハードコードせず実データから再計算すること。**

scenario ID は `C04-OUT1-positive-ready-handoff`。

処理が終了（成功・失敗・中断のいずれでも）したら、`/Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260720-203452-wt-5/eval-log/dev-graph/run-dev-graph-requirements/live-trial/20260722T061413c-r6/out/status.json` だけを out/ に Write し、`{"status":"PASS|FAIL|ERROR","scenario":"requirements-ready-handoff-positive"}` とする。最後は `DONE: <status>` と1行だけ報告する。
