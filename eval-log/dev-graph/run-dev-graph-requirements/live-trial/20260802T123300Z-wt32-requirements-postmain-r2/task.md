...
Skill({skill: "dev-graph:run-dev-graph-requirements", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/live-trial-fixtures/20260802-wt32-requirements-postmain-r2 --feature-id F-LIVE-001 (package は fixture 内 system-plan/F-LIVE-001/ を skill が解決する)"})
...
python3 "$SYSTEM_DEV_PLANNER/scripts/validate-system-plan.py" \
  --repo-root "/Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/live-trial-fixtures/20260802-wt32-requirements-postmain-r2" \
...
機械判定が読む canonical 名を必ず使う。別名の `.md` だけを作って代替してはならない。
...
`progress.json` は SKILL.md の goal-seek checklist 全項目を持ち、`pending=0`、
evidence の null が 0、`overall=pass` になった場合だけ PASS marker を書く。
...
1. `/Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/run-dev-graph-requirements/live-trial/20260802T123300Z-wt32-requirements-postmain-r2/out/status.json` に `{"status":"PASS|FAIL|ERROR","scenario":"C04-OUT1-positive-ready-handoff"}` を Write する。
