...
Skill({skill: "dev-graph:run-dev-graph-requirements", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-requirements-r2 --feature-id F-LIVE-001 (package は fixture 内 system-plan/F-LIVE-001/ を skill が解決する)"})
...
3. system plan は次の promotion 後 command で検証する。引数なしや `--staging` へ読み替えない。

```bash
python3 "$SYSTEM_DEV_PLANNER/scripts/validate-system-plan.py" \
  --repo-root "/Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/live-trial-fixtures/wt31-qa121-requirements-r2" \
  --feature-package "feature-package/F-LIVE-001"
```

## goal-seek 証跡

fixture の `eval-log/` に `run-dev-graph-requirements-goal-spec.json`、
`run-dev-graph-requirements-progress.json`、`run-dev-graph-requirements-intermediate.jsonl` を作る。
Skill 実行前に original_goal と正しい SHA-256 を持つ intermediate 1行目を作り、検証後は
結果を持つ行だけを append する。既存行を上書きしない。
...
1. `/Users/dm/orca/workspaces/HarnessHub/wt-31/eval-log/dev-graph/run-dev-graph-requirements/live-trial/20260802T041500Z-wt31-qa121-requirements-r2/out/status.json` に `{"status":"PASS|FAIL|ERROR","scenario":"C04-OUT1-positive-ready-handoff"}` を Write する。
