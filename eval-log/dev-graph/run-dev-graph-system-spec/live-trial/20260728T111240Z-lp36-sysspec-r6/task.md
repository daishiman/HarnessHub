# タスク: dev-graph:run-dev-graph-system-spec の実走 (scenario C19-OUT1-positive-system-spec-lineage-r6)
...
```
Skill({skill: "dev-graph:run-dev-graph-system-spec", args: "--repo-root /Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260728-082322-wt-11/eval-log/dev-graph/live-trial-fixtures/system-spec-lp36-merged-r6"})
```
...
r6 は requirements brief の記載を最初から具体的に反映し、可能なら最初の評価で次を満たしてください。
...
この r6 trial は、前回の fresh evaluator が検出した次の 3 blocker を解消できるかも測定します。
...
## r4 fresh evaluator の blocker も解消する r6 必須条件

前回 r2 では、上の 3 項目を満たした後にも次の blocker が残りました。r6 では以下を省略すると FAIL です。
...
後から noop を行ったため FAIL でした。r6 では R3 の入力準備と schema 検査をすべて終えてから、
...
被験 fixture は `/Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260728-082322-wt-11/eval-log/dev-graph/live-trial-fixtures/system-spec-lp36-merged-r6` にある dev-graph 初期化済みの独立 Git repository です。
...
`run-dev-graph-system-spec` は `goal_seek` を宣言します。SKILL.md の `## ゴールシーク実行` に従い、`/Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260728-082322-wt-11/eval-log/dev-graph/live-trial-fixtures/system-spec-lp36-merged-r6/eval-log/` へ 3 点セット (`run-dev-graph-system-spec-goal-spec.json` / `run-dev-graph-system-spec-progress.json` / `run-dev-graph-system-spec-intermediate.jsonl`、intermediate は必須 6 キー) をすべて書き出してください。
...
1. `/Users/dm/dev/dev/個人開発/HarnessHub/.worktrees/task-20260728-082322-wt-11/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260728T111240Z-lp36-sysspec-r6/out/status.json` に完了マーカーを 1 ファイルだけ Write する。内容:
   `{"status": "PASS|FAIL|ERROR", "scenario": "C19-OUT1-positive-system-spec-lineage-r6"}`
