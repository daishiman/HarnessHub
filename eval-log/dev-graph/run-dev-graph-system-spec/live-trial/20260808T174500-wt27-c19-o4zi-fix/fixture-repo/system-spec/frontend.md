...
**質問**: カテゴリ frontend × platform web の要件は何か (system-spec/requirements-brief.md §3 の確定回答を一次入力とする)
...
### 専用フロントエンドを持たない場合のクライアント契約

- project candidate: `openapi-as-client-contract` (`deepened`)
- 解決対象: フロントエンドを実装しない確定内容のもとで、動作確認と利用方法の伝達手段を仕様として固定する必要がある

#### 目的

確定内容 (専用フロントエンドを実装せず、OpenAPI 定義と HTTP クライアントで確認する) を、クライアント契約の所在として位置づける

#### 解決する問題

- UI が無いと利用方法が口伝になり、受入確認の手順が仕様から落ちる

#### 適用条件

- クライアント実装を成果物に含めない本システムの構成

#### 非適用条件

- GUI クライアントを提供する場合 (U7 で対象外と確定済み)

#### トレードオフ

- 画面を持たない代わりに、契約が機械可読で自動化しやすい

#### 失敗モード

- 定義が実装から乖離する。定義を実装から生成し手書きしないことで防ぐ

#### goalへの寄与

G4 (自作クライアントや shell script から組み合わせられる)
