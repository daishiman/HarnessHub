...
**質問**: カテゴリ auth × platform web の要件は何か (system-spec/requirements-brief.md §3 の確定回答を一次入力とする)
...
### ローカル bearer token 認証の適用範囲

- project candidate: `local-bearer-token-scope-fit` (`deepened`)
- 解決対象: 単一利用者の API で認証必須 (未認証 401) を、外部 IdP も外向き通信も持たずに満たす必要がある

#### 目的

確定内容 (初回起動時ローカル生成・ハッシュ化保存・全 TODO エンドポイント必須・未認証 401) が、標準の bearer 認証と OAuth2 のどちらに接地するかを決める

#### 解決する問題

- 第三者委譲の無い単一利用者へ認可サーバを持ち込むと運用対象と攻撃面が増える

#### 適用条件

- 利用者が 1 名で、token 発行者と資源所有者が同一 (本システムの確定内容そのもの)
- localhost バインド固定で token が端末外へ出ない

#### 非適用条件

- 第三者クライアントへ権限委譲する場合。その時のみ RFC 6749 系 (および更新 RFC 8252/8996/9700) へ移る

#### トレードオフ

- 標準化された委譲機構を持たない代わりに、認可サーバ相当の実装と運用が 0 になる

#### 失敗モード

- token 平文保存や失効手順の欠落。確定内容のハッシュ保存に加え、ローテーション手順を運用章へ残すことで緩和する

#### goalへの寄与

G2 (認証された利用者だけが自分の TODO を操作できる)

---

#### 本章での適用

- 上記原則は確定内容 qa-auth-web (対応セル: web) の判断へ適用する
- 資するゴール: G2
...
|---|---|---|---|---|---|
| fastapi-security | 0.141.1 (docs site 現行版・同 run の release-notes 取得で確認) | FastAPI (Sebastián Ramírez) (fastapi.tiangolo.com) | https://fastapi.tiangolo.com/tutorial/security/ | 2026-08-08T08:45:43.812Z | 2026-08-08T08:45:43.812Z |
| oauth2-rfc6749 | RFC 6749 (Proposed Standard; updated by RFC 8252 / 8996 / 9700) | IETF (RFC Editor / Datatracker) (datatracker.ietf.org) | https://datatracker.ietf.org/doc/html/rfc6749 | 2026-08-08T08:45:43.824Z | 2026-08-08T08:45:43.824Z |
