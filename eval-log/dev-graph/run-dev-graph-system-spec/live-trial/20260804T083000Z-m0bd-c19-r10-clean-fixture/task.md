# C19 live trial: run-dev-graph-system-spec (r10)
...
```
Skill({skill: "dev-graph:run-dev-graph-system-spec", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-21/eval-log/dev-graph/live-trial-fixtures/m0bd-c19-r9-ledger-finalization-20260804"})
```
...
被験 fixture は `/Users/dm/orca/workspaces/HarnessHub/wt-21/eval-log/dev-graph/live-trial-fixtures/m0bd-c19-r9-ledger-finalization-20260804` にある dev-graph 初期化済みの独立 Git repository です。
...
- boot が `--audit-fork-ledger` で渡した fixture 内の `SYSTEM_SPEC_AUDIT_FORK_LEDGER` を変更しない。監査台帳は PostToolUse hook にだけ書かせ、Write / Edit / Bash redirect で作成・改変しないこと。
...
正規4 entry point の PASS と confirmed 成果物だけを C02 `dev-graph:run-dev-graph-node` と `upsert-node.py` 経由で specification node と architecture node へ登録する。各 node の `source_lineage` 6フィールド、`confirmation_evidence`、evaluator evidence、source digest を実測し、`validate-source-digest.py` と `validate-evidence-refs.py` の exit 0 を確認する。architecture node の `source_lineage` が index を参照する場合は、それが全カテゴリ章を束ねる正本索引であることを artifact 本文または lineage note に明記する。graph.json や node Markdown を直接書いて登録を代替しない。dev-graph 内に elicitation / compile の同等ロジックを複製しないことを、harness 側の陽性対照と同じ検索語で確認する。

**fresh Agent を fork する前に**、全ての canonical flow と C02 import が完了した最終周回を `intermediate.jsonl` へ append し、`progress.json` の全 checklist・gates・evidence を実測結果で PASS に確定する。`validate-source-digest.py` と `validate-evidence-refs.py` の progress への書戻しもこの時点で済ませる。これらを独立検証と並行させたり、Agent の結果待ちに後置きしてはならない。
...
完了時は `/Users/dm/orca/workspaces/HarnessHub/wt-21/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260804T083000Z-m0bd-c19-r10-clean-fixture/out/status.json` だけへ `{"status":"PASS|FAIL|ERROR","scenario":"C19-OUT1-positive-system-spec-lineage-r2"}` を書く。最後は `DONE: <status>` の1行だけで報告する。
