...
```
Skill({skill: "dev-graph:run-dev-graph-system-spec", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/live-trial-fixtures/20260802-wt32-system-spec-r5"})
```
...
- **body source と source lineage は別物である。** `--body-file` は上記
  ...
  `source_digest` はその `system-spec/` 確定章を r5 fixture 内で sha256 再計算した値にする。
  `eval-log/node-bodies/*.md` を lineage source にした run は、6 field が埋まっていても FAIL。
- `confirmation_evidence.evaluated_digest` は r5 の
...
被験 fixture は `/Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/live-trial-fixtures/20260802-wt32-system-spec-r5` にある dev-graph 初期化済みの独立 Git repository です。
...
`run-dev-graph-system-spec` は `goal_seek` を宣言します。SKILL.md の `## ゴールシーク実行` に従い、`/Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/live-trial-fixtures/20260802-wt32-system-spec-r5/eval-log/` へ 3 点セット (`run-dev-graph-system-spec-goal-spec.json` / `run-dev-graph-system-spec-progress.json` / `run-dev-graph-system-spec-intermediate.jsonl`、intermediate は必須 6 キー) をすべて書き出してください。
...
1. `/Users/dm/orca/workspaces/HarnessHub/wt-32/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260802T082230Z-wt32-system-spec-r5/out/status.json` に完了マーカーを 1 ファイルだけ Write する。内容:
