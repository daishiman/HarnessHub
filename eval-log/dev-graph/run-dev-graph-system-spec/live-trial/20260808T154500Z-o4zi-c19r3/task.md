...
```
Skill({skill: "dev-graph:run-dev-graph-system-spec", args: "--repo-root /private/tmp/harnesshub-o4zi.Gv5hFK/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260808T154500Z-o4zi-c19r3/fixture-repo"})
```
...
FIXTURE = `/private/tmp/harnesshub-o4zi.Gv5hFK/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260808T154500Z-o4zi-c19r3/fixture-repo`
...
4. dev-graph 側が elicitation / compile の**実行ロジック**を複製せず、harness の確定成果物を
   `source_lineage` と `architecture_refs` で追跡していることを、実際の node Markdown と writer 実装で
   示してください。`build-system-spec-import.py` は監査可能な artifact として確定 source 本文を verbatim
   保存する正規契約です。この本文保存は executable logic の複製ではないため、確定 source と node body の
   この一致を理由に FAIL にしてはいけません。代わりに dev-graph node 本文へ elicitation state machine、compiler、
   coverage/citation validator の Python 実装が混入していないことを確認してください。
...
- 第 3 段: 本 run の書込み操作の全数検査で C02 以外の登録が 0 件、かつ writer 経路の静的確認が
  陽性対照つきで成立
- 第 4 段: executable logic の複製 0 件が陽性対照つきの対照実験として成立し、確定本文の verbatim
  保存と lineage 追跡が正規 import 契約どおりであることを node 本文と writer 実装で確認できている
...
```
/private/tmp/harnesshub-o4zi.Gv5hFK/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260808T154500Z-o4zi-c19r3/out/status.json
```
