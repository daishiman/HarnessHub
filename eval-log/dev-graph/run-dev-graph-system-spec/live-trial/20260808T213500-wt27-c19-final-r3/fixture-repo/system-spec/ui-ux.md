...
**質問**: 書面入力 system-spec/requirements-brief.md の section 「§3 カテゴリ別の確定回答 (web 列) / category=ui-ux」で、カテゴリ ui-ux × platform web の確定内容は何か

**回答**: | ui-ux | UI は持たない。API の使い勝手として、エラー応答を `{"error": {"code", "message"}}` に統一し、OpenAPI 定義を `/openapi.json` でローカル提供する。Apple HIG の consistency 原則は、GUI ではなく API 応答形式と OpenAPI 表現の一貫性として適用する。 | G2, G4 |
