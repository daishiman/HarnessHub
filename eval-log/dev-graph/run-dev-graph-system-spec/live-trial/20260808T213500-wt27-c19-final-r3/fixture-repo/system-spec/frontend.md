...
| プラットフォーム | 状態 | 根拠 |
|---|---|---|
| Web (web) | 確定 | 確定質疑: qa-frontend-web |
...
### qa-frontend-web (対応セル: web)

**質問**: 書面入力 system-spec/requirements-brief.md の section 「§3 カテゴリ別の確定回答 (web 列) / category=frontend」で、カテゴリ frontend × platform web の確定内容は何か

**回答**: | frontend | 専用フロントエンドを実装しない。動作確認は OpenAPI 定義と HTTP クライアント (curl 等) で行う。Clean Architecture の境界原則に従い、OpenAPI を外部クライアントと内部 API の単一契約境界にする。 | G4 |
...
- 上記原則は確定内容 qa-frontend-web (対応セル: web) の判断へ適用する
- 資するゴール: G4
