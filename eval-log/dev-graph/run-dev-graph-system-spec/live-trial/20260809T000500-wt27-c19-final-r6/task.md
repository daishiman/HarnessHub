...
```
Skill({skill: "dev-graph:run-dev-graph-system-spec", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-27-3/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260809T000500-wt27-c19-final-r6/fixture-repo"})
```
...
FIXTURE = `/Users/dm/orca/workspaces/HarnessHub/wt-27-3/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260809T000500-wt27-c19-final-r6/fixture-repo`
...
brief に直接書かれていない回答・判断は推論せず、該当セル/decision を未収集のまま fail-closed に
残してください。AI の判断を利用者回答や承認へ読み替えて新しい要求を足さないこと。

### 書面根拠と承認の provenance 契約（今回の回帰対象）

- `source.kind=written-requirements` の各 `answer` は、`source.path` / `source.section` に
  実在する利用者原文の逐語 excerpt にしてください。`source.sha256` はその `answer` の UTF-8 bytes
  から計算し、AI が生成した要約・判断・qa entry 自身の digest を利用者根拠として代用しないでください。
- この非対話 trial 中に新しい利用者入力は生じません。brief 第 7 節の既存一括承認だけを使用し、
  AI 自身を承認者とする新規 approval を作成しないでください。原文で直接支持できないセルは
  未収集として fail-closed に残してください。
- completeness evaluator の C07 / C06 / C08 監査は、1 回の assistant message で複数同時起動せず、
  **1 message = 1 foreground fork** で直列実行してください。1 件の完全 response と最終行
  `AUDIT_VERDICT`、PostToolUse 台帳行を回収してから次の 1 件を起動し、background/非同期 dispatch は
  使用しないでください。

### この実走で必須の evaluator 契約

- `assign-system-spec-completeness-evaluator` は `context: fork` のため、Skill 起動応答が background
  task の完全な `agentId` を返す。これは完了 receipt ではない。**完全な agentId と一致する native task-notification**
  (`status=completed`、fork 自身の完全 response あり) を受け取るまで待つこと。runtime が
  `TaskOutput` と互換の task id を明示した場合だけ `TaskOutput(block=true)` を使ってよい。
  subagent 一覧に出る短縮 ID を task id と推測して呼び出してはならない。
- native notification を処理できるよう、**待機は1回30秒以内の有限操作**にする。foreground で
  `until` / `while` / `for` + `sleep` や30秒超の sleep を使わず、存在しない sentinel を待つ
  loop で main tool boundary を塞がない。短い待機が戻るたびに native notification を確認する。
- **evaluator fork を TaskStop してはならない**。停止・失敗した場合は本 run を FAIL とし、
  **outer session が completeness-report.json を Write/Edit してはならない**。report は正規 evaluator
  fork 自身が指定出力へ書いたものだけを採用する。
- evaluator の Skill 呼出しは次の引数を明示する:
  `--spec-dir <FIXTURE>/system-spec --output <FIXTURE>/system-spec/completeness-report.json`。
  evaluator task が completed になる前に C02 import へ進んではならない。
- post-run harness は `validate-system-spec-evaluator-completion.py` で、完全 agentId の完了通知が
  最初の `upsert-node.py` より前であること、TaskStop と outer report 代筆が 0 件であることを
  transcript から機械検証する。したがって、この時系列と authority を実操作で守ること。
- doc-fetch は brief 第 4 節に列挙された公式 URL を入力とし、正規 skill の契約どおりに処理してください。
  **doc-fetch は公式ページを実取得して現行 version を記録する。既知の古い値を固定しない。**
  採択案だけでなく **比較候補6件すべて** (`sqlite` / `postgresql` / `fastapi` / `flask` /
  `local-bearer-token` / `oauth2-password-jwt`) を `targets[]` に入れ、同じ鮮度検証へ通す。
...
2. これらの実装に特徴的な検索語 (関数名・状態値・schema key・CLI サブコマンド名など) を**自分で選び**、
   ...
   使った検索語とヒット数を明記すること。
3. **同じ検索語**を実行可能な実装 root `plugins/dev-graph/skills/` と
   `plugins/dev-graph/scripts/` だけへ適用し、ヒット 0 件であることを示す。
   `tests/fixtures`・コメント・受領書は実装複製の有無を示さないため検索対象外とする。
...
```
/Users/dm/orca/workspaces/HarnessHub/wt-27-3/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260809T000500-wt27-c19-final-r6/out/status.json
```
