...
**質問**: 書面入力 system-spec/requirements-brief.md の section 「3. カテゴリ別の確定回答 (web 列)」で、カテゴリ security × platform web の確定内容と serves_goals は何か

**回答**: localhost (127.0.0.1) バインド固定、外向き通信なし、token はハッシュ保存、入力は schema 検証、SQL は必ずパラメータバインド。OWASP ASVS の入力検証と least privilege 原則を、ローカル公開面の最小化と永続化境界の検証に具体化する。
