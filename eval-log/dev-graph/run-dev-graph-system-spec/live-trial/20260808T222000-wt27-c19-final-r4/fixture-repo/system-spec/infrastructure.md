...
**質問**: 書面入力 system-spec/requirements-brief.md の section 「3. カテゴリ別の確定回答 (web 列)」で、カテゴリ infrastructure × platform web の確定内容と serves_goals は何か

**回答**: 常駐ミドルウェアなし。1 コマンド起動、データはファイル 1 個。バックアップはそのファイルのコピー。Google SRE の toil 削減と反復可能な復旧原則を、起動 1 コマンドと復旧 1 ファイルに具体化する。
