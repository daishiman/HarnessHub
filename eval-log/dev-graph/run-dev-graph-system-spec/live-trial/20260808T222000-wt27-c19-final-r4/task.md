# タスク: dev-graph:run-dev-graph-system-spec の実走 (scenario C19-OUT1-positive-system-spec-lineage-r3)
...
```
Skill({skill: "dev-graph:run-dev-graph-system-spec", args: "--repo-root /Users/dm/orca/workspaces/HarnessHub/wt-27-3/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260808T222000-wt27-c19-final-r4/fixture-repo"})
```
...
FIXTURE = `/Users/dm/orca/workspaces/HarnessHub/wt-27-3/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260808T222000-wt27-c19-final-r4/fixture-repo`
...
- `assign-system-spec-completeness-evaluator` は `context: fork` のため、Skill 起動応答が background
  task id を返す。これは完了 receipt ではない。返された id を `TaskOutput(block=true)` で待ち、
  timeout なら同じ id を再度待って、fork 自身の完全 response と task completed を回収すること。
...
```
/Users/dm/orca/workspaces/HarnessHub/wt-27-3/eval-log/dev-graph/run-dev-graph-system-spec/live-trial/20260808T222000-wt27-c19-final-r4/out/status.json
```
...
```json
{"status": "PASS|FAIL|ERROR", "scenario": "C19-OUT1-positive-system-spec-lineage-r3"}
```
...
scenario_id: `C19-OUT1-positive-system-spec-lineage-r3`
