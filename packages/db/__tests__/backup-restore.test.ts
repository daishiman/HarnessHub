...
describe('P13 production migration / smoke CLI', () => {
  ...
  it('migration は dry-run → 初回適用 → 再適用を台帳どおり冪等に処理する', () => {
    ...
    // 0004 顧客持ち込み OAuth client の lifecycle / 0005 tenant-data-retention 封筒暗号化拡張 /
    // 0006 tenant-data-retention tombstone 台帳
    ...
    expect(first).toMatchObject({ ok: true, appliedBefore: 0, appliedAfter: 7 });

    const second = JSON.parse(runCli('scripts/migrate-deploy.ts', ['--url', url]).trim());
    expect(second).toMatchObject({ ok: true, appliedBefore: 7, appliedAfter: 7 });
