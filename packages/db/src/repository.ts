...
export function normalizePageRequest(request?: PageRequest): PageRequest {
  ...
  return request.cursor === undefined
    ? { limit: clamped }
    : { limit: clamped, cursor: request.cursor };
}
...
export type ReadOnlyRepository<TEntity, TId = string> = Pick<
...
export type RepositoryFactory<
  ...
  TClient = unknown,
> = (adapter: DatabaseAdapter<TSchema, TClient>) => TRepository;

/**
 * factory を型推論つきで定義するための識別関数。
 * 実行時の振る舞いは持たない (境界の形を揃えるためだけの薄いヘルパ)。
 */
export function defineRepositoryFactory<
  TRepository,
  TSchema extends DrizzleSchema = DrizzleSchema,
  TClient = unknown,
>(
