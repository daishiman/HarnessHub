...
- Entry gate: goal-spec.json の feature_context_digest が sha256:68f274de9cd604964c4499897cc3bf2efc88d09bdaf730db7640c5f09c9caffc に一致し、confirmation_status=confirmed であること。P04 の docs/features/feat-domain-model-db/test-design.md が存在すること
...
- Produced artifacts: packages/db/schema/core/ (18 テーブル定義), packages/db/schema/index.ts (barrel), packages/db/connection/ (Turso/D1 ファクトリ), packages/db/repository/ (tenant scope・release immutable・audit・encryption を強制するリポジトリ関数群), packages/db/registry/ (R2 content-addressed registry), packages/db/backup/, packages/db/scripts/export-control-plane.ts, packages/db/scripts/restore-control-plane.ts, packages/db/__tests__/fixtures/two-tenants.ts, packages/db/__tests__/backup-restore.test.ts (normative implementation artifacts)
...
- Write scope/touches: packages/db/schema/core/, packages/db/schema/index.ts, packages/db/connection/, packages/db/repository/, packages/db/registry/ (packages/db/schema/{studio-feature}/ 配下は対象外), packages/db/backup/, packages/db/scripts/export-control-plane.ts, packages/db/scripts/restore-control-plane.ts, packages/db/__tests__/fixtures/two-tenants.ts, packages/db/__tests__/backup-restore.test.ts
...
- Required evidence: packages/db 配下に schema/core/, schema/index.ts, connection/, repository/, registry/ の 5 ディレクトリ/ファイルが実装され、P04 の test-design.md に列挙された全テストケースに対応する実装対象が揃っていること. Normative evidence: P05実装パス、P08 migration apply、2-tenant fixture、export→別DB restore round-trip、salary暗号化/マスク断面、単一lineage検査を証跡化する。
...
- Required responsibility: packages/db に日次 export job と検証可能な restore command/library を実装する。P12 の runbook は消費物であり、P05 実装の前提にしない。
