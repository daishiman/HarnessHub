...
- phase_ref: P11
- classification: confidence=0.9, reason="goal-spec.json を入力に P11 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p11.md}]
...
P06 のテスト実行ログと P09 の CI 品質ゲート実行結果、絵文字 lint の実行コマンドと出力を、第三者が再実行できる形式で集約する。
...
- Frontend: N/A: 本 task は証跡集約のみでコードを変更しない
- Backend: N/A: 本 feature は backend 実装物を変更しない
- API: N/A: 本 feature は公開 API を変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: N/A: 本 task は証跡集約のみで CI 構成を変更しない
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: 再実行可能な証跡バンドルを確定する
- Documentation: applicable + change: evidence-bundle.md を新規作成する
...
- Produced artifacts: docs/features/feat-semantic-emphasis-icons/evidence-bundle.md (再実行可能なコマンド全文と出力抜粋を含む検証証跡集約)
...
- Write scope/touches: docs/features/feat-semantic-emphasis-icons/evidence-bundle.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p11) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p11 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p11) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-EMPHASIS-ICONS-P10 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-semantic-emphasis-icons/evidence-bundle.md) が他 task の active lease と重複しないことを確認する
...
- 不足証跡の再実行そのもの (不足がある場合は P06/P09 へ差し戻す)
...
- evidence-bundle.md に P06 のテスト実行ログ、P09 の CI 品質ゲート実行結果、絵文字 lint の実行コマンドと出力が再実行可能な形式 (コマンド全文+出力抜粋) で集約されている
- 各証跡が生成された日時・実行環境 (ローカル/CI) が記録され、第三者が同じコマンドで再現できる状態になっている
...
- Rollout: evidence-bundle.md を write scope 内に限定して作成/実装し、次フェーズへ引き継ぐ
- Rollback trigger and steps: evidence-bundle.md を作成前状態へ戻し、不足している証跡を P06/P09 へ差し戻して再収集する
...
- Dependencies: SYS-EMPHASIS-ICONS-P10
