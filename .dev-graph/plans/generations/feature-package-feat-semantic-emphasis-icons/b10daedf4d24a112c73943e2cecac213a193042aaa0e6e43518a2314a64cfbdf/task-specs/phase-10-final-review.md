# System task overlay: 最終独立レビュー — quality_constraints 5 件の充足判定
...
- phase_ref: P10
- classification: confidence=0.9, reason="goal-spec.json を入力に P10 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p10.md}]
...
quality_constraints 5 件それぞれについて最終的な独立判定を行い、P01〜P09 の全成果物を参照可能な状態にまとめる。
...
- Frontend: N/A: 本 task はレビューのみでコードを変更しない
- Backend: N/A: 本 feature は backend 実装物を変更しない
- API: N/A: 本 feature は公開 API を変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: N/A: 本 task はレビューのみで CI 構成を変更しない
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: quality_constraints 5 件の最終合否判定を確定する
- Documentation: applicable + change: final-review.md を新規作成する
...
- Produced artifacts: docs/features/feat-semantic-emphasis-icons/final-review.md (quality_constraints 5 件の最終合否判定と P01〜P09 成果物への参照)
...
- Write scope/touches: docs/features/feat-semantic-emphasis-icons/final-review.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p10) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p10 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p10) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-EMPHASIS-ICONS-P09 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-semantic-emphasis-icons/final-review.md) が他 task の active lease と重複しないことを確認する
...
- final-review.md に quality_constraints 5 件 (emoji-ban-semantic-token-qa232-5 / icon-lint-ci-fail-closed-qa233-6 / icon-ownership-boundary-qa233-3 / design-system-token-and-contrast-gate / state-value-visible-label-qa232-2) それぞれの充足判定 (合格/不合格) と根拠が記録されている
- P01〜P09 の全成果物 (requirements-baseline.md 〜 quality-gate-report.md) への参照リンクが final-review.md に列挙されている
- 5 件全てが合格と判定されている (不合格がある場合は当該フェーズへの差し戻しが final-review.md に明記されている)
...
- Rollout: final-review.md を write scope 内に限定して作成/実装し、次フェーズへ引き継ぐ
- Rollback trigger and steps: final-review.md を作成前状態へ戻し、不合格項目を該当フェーズへ差し戻す
...
- Dependencies: SYS-EMPHASIS-ICONS-P09
