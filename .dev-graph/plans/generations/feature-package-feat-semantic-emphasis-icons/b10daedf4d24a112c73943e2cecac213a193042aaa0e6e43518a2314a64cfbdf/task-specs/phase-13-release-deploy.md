# System task overlay: リリース判定と確定仕様・アーキテクチャへの書き戻し
...
- phase_ref: P13
- classification: confidence=0.9, reason="goal-spec.json を入力に P13 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p13.md}]
...
本 feature の成果をいつでも反映できる状態に確定し、P01〜P12 で得た判断と改善点を確定仕様と architecture へ書き戻して次の外側ループへ引き継ぐ。deploy そのものの実行は本 feature の scope_out であり、本 task は行わない。
...
- Frontend: applicable + no change: 反映後に確認すべき事項 (callout/token/lint の稼働状態) を記述するのみで、この task ではコードを変更しない
- Backend: N/A: 本 feature は backend 実装物を変更しない
- API: N/A: 本 feature は公開 API を変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: N/A: 本 feature は Cloudflare deploy unit を変更しない
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: 反映後の確認事項を検証可能な形で確定する
- Documentation: applicable + change: 確定仕様と architecture へ書き戻す
- Operations: applicable + change: 反映可否の判定と確認手順を運用へ引き渡す
...
- Produced artifacts: リリース判定の記録 (反映可否と反映後の確認手順の引き渡し)、確定仕様と architecture への書き戻し差分
...
- Write scope/touches: features/feat-semantic-emphasis-icons.md, features/feat-semantic-emphasis-icons.context.json, architecture/harness-hub-design-system.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p13) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p13 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p13) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-EMPHASIS-ICONS-P12 であり、直前 phase の完了後にのみ着手する。resource_scope (features/feat-semantic-emphasis-icons.md, features/feat-semantic-emphasis-icons.context.json, architecture/harness-hub-design-system.md) が他 task の active lease と重複しないことを確認する
...
- 本番 deploy の実行 (feature context の scope_out。運用操作であり本 feature の成果物ではない)
- 確定仕様の既存章の改変 (追補のみ許可)
...
- P01〜P12 の全成果物が揃い、P10 final-review.md で quality_constraints 5 件が全て合格判定されていることを前提にリリース可否が判定されている
- features/feat-semantic-emphasis-icons.md と .context.json に本 feature の実装完了状態 (絵文字 lint 稼働・token 統一・可視ラベル併置) が書き戻されている
- architecture/harness-hub-design-system.md へ、ライトモード強調ブロック背景が semantic token 由来であることの確定事項が書き戻されている
...
- P13 spec/architecture writeback: required — 本 task の実行結果・判断・改善点を features/feat-semantic-emphasis-icons.md・features/feat-semantic-emphasis-icons.context.json・architecture/harness-hub-design-system.md へ追補として書き戻し、次の外側ループへ引き継ぐ。

## Rollout and rollback

- Rollout: release-record.md 相当の判定を final-review.md の合否に基づき確定し、そのうえで確定仕様と architecture への書き戻しを行う
- Rollback trigger and steps: features/feat-semantic-emphasis-icons.md・.context.json・architecture/harness-hub-design-system.md への書き戻しを revert し、P10 の最終レビュー結果を再確認する
...
- Dependencies: SYS-EMPHASIS-ICONS-P12
