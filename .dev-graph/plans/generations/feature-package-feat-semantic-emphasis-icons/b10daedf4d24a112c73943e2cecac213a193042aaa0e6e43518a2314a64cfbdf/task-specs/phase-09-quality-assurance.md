# System task overlay: 品質保証 — CI 品質ゲート (絵文字 lint fail-closed/axe/コントラスト/VRT) の確認
...
- owners: ["daishiman"]
- tags: ["feat-semantic-emphasis-icons", "macro-feature", "quality", "phase-p09"]
...
- phase_ref: P09
- classification: confidence=0.9, reason="goal-spec.json を入力に P09 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p09.md}]
...
絵文字 lint の CI 上での fail-closed 動作、callout の a11y、ライトモード背景のコントラスト・VRT 相当の品質ゲートが機能していることを確認する。
...
- Frontend: applicable + no change: callout の a11y とコントラストを確認するのみで、この task ではコードを変更しない
- Backend: N/A: 本 feature は backend 実装物を変更しない
- API: N/A: 本 feature は公開 API を変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: applicable + no change: CI の fail-closed 動作を確認するのみで、この task では CI 構成を変更しない
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: quality_constraints 5 件の CI 品質ゲートでの充足確認結果を確定する
- Documentation: applicable + change: quality-gate-report.md を新規作成する
...
- Produced artifacts: docs/features/feat-semantic-emphasis-icons/quality-gate-report.md (CI 品質ゲートの fail-closed 動作確認結果、a11y/コントラスト確認結果)
...
- Write scope/touches: docs/features/feat-semantic-emphasis-icons/quality-gate-report.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p09) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p09 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p09) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-EMPHASIS-ICONS-P08 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-semantic-emphasis-icons/quality-gate-report.md) が他 task の active lease と重複しないことを確認する
...
- 実装コードの修正 (未充足ゲートがある場合は P05/P06 へ差し戻す)
...
- 絵文字 lint が CI 上で fail-closed に機能していること (lint step 自体が skip/失敗した場合も CI 全体が失敗すること) が quality-gate-report.md に記録されている
- callout の a11y (axe 等) とライトモード背景のコントラスト・VRT (Visual Regression Test) 相当の確認結果が記録されている
- quality_constraints 5 件それぞれについて CI 品質ゲートでの充足確認結果が記録されている
...
- Rollout: quality-gate-report.md を write scope 内に限定して作成/実装し、次フェーズへ引き継ぐ
- Rollback trigger and steps: quality-gate-report.md を作成前状態へ戻し、未充足のゲートを P05/P06 へ差し戻す
...
- Dependencies: SYS-EMPHASIS-ICONS-P08
