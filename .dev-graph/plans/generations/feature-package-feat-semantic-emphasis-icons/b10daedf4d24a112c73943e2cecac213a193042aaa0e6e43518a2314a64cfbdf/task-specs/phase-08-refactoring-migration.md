# System task overlay: リファクタリング/マイグレーション — N/A (DB schema 変更なし)
...
- owners: ["daishiman"]
- tags: ["feat-semantic-emphasis-icons", "macro-feature", "documentation", "phase-p08"]
...
- phase_ref: P08
- classification: confidence=0.9, reason="goal-spec.json を入力に P08 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p08.md}]
...
本 feature が公開 API・DB schema・認可判定・Cloudflare deploy unit を変更しないことを確認し、P05 の token 置換が既存の呼び出し側インターフェースを破壊しない非破壊的変更であることを記録する。
...
- Frontend: applicable + no change: P05 の Badge.tsx 変更が props インターフェースを変えていないことを確認するのみで、この task ではコードを変更しない
- Backend: N/A: 本 feature は backend 実装物を変更しない
- API: N/A: 本 feature は公開 API を変更しない
- Data: N/A: 本 feature は DB schema を変更しない
- Infrastructure: N/A: 本 feature は Cloudflare deploy unit を変更しない
- Security: N/A: 本 feature は認可判定を変更しない
- Quality: applicable + no change: 非破壊的変更であることの確認結果を記録する
- Documentation: applicable + change: refactor-migration-note.md を新規作成する
...
- Produced artifacts: docs/features/feat-semantic-emphasis-icons/refactor-migration-note.md (リファクタリング/マイグレーション不要の確認記録)
...
- Write scope/touches: docs/features/feat-semantic-emphasis-icons/refactor-migration-note.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p08) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p08 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p08) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-EMPHASIS-ICONS-P07 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-semantic-emphasis-icons/refactor-migration-note.md) が他 task の active lease と重複しないことを確認する
...
- schema マイグレーションの実施 (本 feature は scope_out のため実施しない)
...
- refactor-migration-note.md に、本 feature が公開 API・DB schema・認可判定・Cloudflare deploy unit を変更しない scope_out 対象であり、リファクタリング/マイグレーション作業が不要であることが明記されている
- P05 で行った Badge.tsx 等の token 置換が、既存の呼び出し側インターフェース (props) を変更しない非破壊的変更であることが確認されている
...
- Rollout: refactor-migration-note.md を write scope 内に限定して作成/実装し、次フェーズへ引き継ぐ
- Rollback trigger and steps: refactor-migration-note.md を作成前状態へ戻す (実装済みコードへの影響はない)
...
- Dependencies: SYS-EMPHASIS-ICONS-P07
