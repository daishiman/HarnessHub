# System task overlay: 独立設計レビュー — lint 検査対象範囲・所有境界・token 適用範囲の妥当性確認
...
- phase_ref: P03
- classification: confidence=0.9, reason="goal-spec.json を入力に P03 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p03.md}]
...
P02 のアーキテクチャ決定を、lint 検査対象範囲・所有境界・token 適用範囲の 3 点について独立にレビューし、P04 以降が実装可能な状態まで確定する。
...
- Quality: applicable + change: P02 決定の妥当性を独立レビューし合否を確定する
- Documentation: applicable + change: design-review.md を新規作成する
...
- Produced artifacts: docs/features/feat-semantic-emphasis-icons/design-review.md (P02 architecture-decision.md への独立レビュー結果)
...
- Write scope/touches: docs/features/feat-semantic-emphasis-icons/design-review.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p03) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p03 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p03) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-EMPHASIS-ICONS-P02 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-semantic-emphasis-icons/design-review.md) が他 task の active lease と重複しないことを確認する
...
- design-review.md に P02 の architecture-decision.md の内容に対する独立レビュー結果 (合格/要修正の判定と根拠) が記録されている
- 絵文字 lint の検査対象範囲 (UI 文言・callout ラベル・空状態文言) が scope_in の記述と過不足なく一致することが確認されている
- packages/ui を唯一のアイコン供給元とする所有境界が、apps/hub 側で再実装や別アイコンライブラリ導入をしない制約として明記され、レビューで承認されている
...
- Rollout: design-review.md を write scope 内に限定して作成/実装し、次フェーズへ引き継ぐ
- Rollback trigger and steps: design-review.md を作成前状態へ戻し、P02 のアーキテクチャ決定を再設計に差し戻す
...
- Dependencies: SYS-EMPHASIS-ICONS-P02
