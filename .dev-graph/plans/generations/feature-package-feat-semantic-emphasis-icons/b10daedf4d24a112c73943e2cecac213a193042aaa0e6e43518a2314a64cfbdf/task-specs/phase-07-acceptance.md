# System task overlay: 受入 — goal-spec acceptance 5 項目の確認
...
- phase_ref: P07
- classification: confidence=0.9, reason="goal-spec.json を入力に P07 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p07.md}]
...
goal-spec.json の acceptance 5 項目それぞれについて、P01〜P06 の成果物とテスト結果を根拠に充足を確認する。
...
- Frontend: N/A: 本 task は確認のみでコードを変更しない
- Backend: N/A: 本 feature は backend 実装物を変更しない
- API: N/A: 本 feature は公開 API を変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: N/A: 本 task は確認のみで CI 構成を変更しない
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: acceptance 5 項目の充足判定を確定する
- Documentation: applicable + change: acceptance-review.md を新規作成する
...
- Produced artifacts: docs/features/feat-semantic-emphasis-icons/acceptance-review.md (acceptance 5 項目それぞれの充足判定と根拠)
...
- Write scope/touches: docs/features/feat-semantic-emphasis-icons/acceptance-review.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p07) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p07 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p07) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-EMPHASIS-ICONS-P06 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-semantic-emphasis-icons/acceptance-review.md) が他 task の active lease と重複しないことを確認する
...
- 実装コードの修正 (不合格の場合は P05/P06 へ差し戻す)
...
- goal-spec.json の acceptance 5 項目それぞれについて、充足の根拠 (対応する実装ファイル・テスト結果) が acceptance-review.md に個別に記録されている
- 5 項目全てが充足と判定され、未充足の項目がある場合はその項目が明示され P05/P06 への差し戻しが記録されている
...
- Rollout: acceptance-review.md を write scope 内に限定して作成/実装し、次フェーズへ引き継ぐ
- Rollback trigger and steps: acceptance-review.md を作成前状態へ戻し、未充足項目を P05/P06 へ差し戻す
...
- Dependencies: SYS-EMPHASIS-ICONS-P06
