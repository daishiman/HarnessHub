# System task overlay: 実装 — 絵文字 lint script・CI fail-closed 組込み・一覧/カード状態表現の token 統一・可視ラベル併置の実装
...
- owners: ["daishiman"]
- tags: ["feat-semantic-emphasis-icons", "macro-feature", "frontend", "phase-p05"]
...
- phase_ref: P05
- classification: confidence=0.9, reason="goal-spec.json を入力に P05 の単一責務 (frontend) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p05.md}]
...
P02 の決定と P04 のテストスタブに従い、絵文字 lint script・CI fail-closed 組込み・一覧/カード状態表現の token 統一・可視ラベル併置の 4 点を実装する。既存の callout/icon/token 実装は変更しない。
...
- Frontend: applicable + change: packages/ui/src/components/Badge.tsx の色指定を packages/ui/src/tokens/tokens.ts の semantic token のみに統一する
- Backend: N/A: 本 feature は backend 実装物を変更しない
- API: N/A: 本 feature は公開 API を変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: applicable + change: .github/workflows/ci.yml の static-gates 相当ジョブへ絵文字 lint を fail-closed で組み込む
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: scripts/lint-ui-text-emoji.py を新規実装する
...
- Produced artifacts: scripts/lint-ui-text-emoji.py (絵文字混入検出 lint)、.github/workflows/ci.yml への fail-closed 組込み差分、packages/ui/src/components/Badge.tsx の token 統一差分
...
- Write scope/touches: scripts/lint-ui-text-emoji.py, .github/workflows/ci.yml, packages/ui/src/components/Badge.tsx
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p05) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p05 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p05) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-EMPHASIS-ICONS-P04 であり、直前 phase の完了後にのみ着手する。resource_scope (scripts/lint-ui-text-emoji.py, .github/workflows/ci.yml, packages/ui/src/components/Badge.tsx) が他 task の active lease と重複しないことを確認する
...
- テストの実行 (P06 が所有)
- 既存の callout/icon/token 実装 (packages/ui/src/icons/index.tsx, packages/ui/src/components/Markdown.tsx, packages/ui/src/tokens/tokens.ts) の変更
...
- scripts/lint-ui-text-emoji.py が packages/ui/src 配下の文言・callout ラベル・空状態文言中の絵文字を検出し、検出時に非ゼロ終了する
- .github/workflows/ci.yml の static-gates 相当ジョブに絵文字 lint が fail-closed で組み込まれている (lint 未実行では CI が成功しない)
- 一覧・カードの状態表現 (Badge.tsx 等) が packages/ui/src/tokens/tokens.ts の semantic token のみで色付けされ、token 外の色指定が残っていない
- P04 のテストスタブが実装に対して green で通過する
...
- Rollout: lint-ui-text-emoji.py を write scope 内に限定して作成/実装し、次フェーズへ引き継ぐ
- Rollback trigger and steps: scripts/lint-ui-text-emoji.py と .github/workflows/ci.yml への変更、Badge.tsx への変更を revert し、実装前の状態に戻す
...
- Dependencies: SYS-EMPHASIS-ICONS-P04
