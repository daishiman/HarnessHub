# System task overlay: テスト実行 — 単体/lint/a11y/コントラストテストの実行と結果記録
...
- phase_ref: P06
- classification: confidence=0.9, reason="goal-spec.json を入力に P06 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p06.md}]
...
P04 のテストスタブと P05 の実装に対して、単体・lint・a11y・コントラストの各テストを実行し、結果を再実行可能な形で記録する。
...
- Frontend: applicable + no change: packages/ui のテストを実行するのみで、この task ではコードを変更しない
- Backend: N/A: 本 feature は backend 実装物を変更しない
- API: N/A: 本 feature は公開 API を変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: applicable + no change: CI 上での lint 実行結果を確認するのみで、この task では CI 構成を変更しない
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: 単体・lint・a11y・コントラストの実行結果を確定する
- Documentation: applicable + change: test-run-report.md を新規作成する
...
- Produced artifacts: docs/features/feat-semantic-emphasis-icons/test-run-report.md (単体/lint/a11y/コントラストテストの実行結果)
...
- Write scope/touches: docs/features/feat-semantic-emphasis-icons/test-run-report.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p06) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p06 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p06) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-EMPHASIS-ICONS-P05 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-semantic-emphasis-icons/test-run-report.md) が他 task の active lease と重複しないことを確認する
...
- P04 で作成したテストスタブと P05 の実装に対する単体テストの実行結果 (pass/fail 件数) が test-run-report.md に記録されている
- scripts/lint-ui-text-emoji.py を意図的な絵文字混入サンプルに対して実行した結果 (fail する) と、混入なしサンプルに対して実行した結果 (pass する) が記録されている
- callout の a11y 属性 (role/aria-label) とライトモード背景のコントラスト測定結果が記録されている
...
- Rollout: test-run-report.md を write scope 内に限定して作成/実装し、次フェーズへ引き継ぐ
- Rollback trigger and steps: test-run-report.md を作成前状態へ戻し、P05 の実装差分を修正対象として P05 へ差し戻す
...
- Dependencies: SYS-EMPHASIS-ICONS-P05
