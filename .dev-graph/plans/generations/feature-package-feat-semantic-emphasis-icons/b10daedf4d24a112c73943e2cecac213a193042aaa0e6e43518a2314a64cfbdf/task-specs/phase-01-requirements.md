# System task overlay: 要件ベースライン確定 — 絵文字禁止 semantic token/icon 表現の要件確定
...
- phase_ref: P01
- classification: confidence=0.9, reason="goal-spec.json を入力に P01 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p01.md}]
...
本 feature の受入可能な要件ベースラインを確定し、P02 以降の全 task が同一の合意事項 (絵文字禁止の対象範囲・既存実装の再利用範囲・quality_constraints 5 件の充足条件) を参照できる状態にする。
...
- Frontend: applicable + no change: 既存の callout/icon/token 実装を要件確定の対象として棚卸しするのみで、この task ではコードを変更しない
- Backend: N/A: 本 feature は backend 実装物を変更しない
- API: N/A: 本 feature は公開 API を変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: N/A: CI ワークフローの変更は P02/P05 が扱う
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: acceptance 5 件と quality_constraints 5 件を machine-verifiable な受入基準として baseline に固定する
- Documentation: applicable + change: requirements-baseline.md を新規作成する
...
- Produced artifacts: docs/features/feat-semantic-emphasis-icons/requirements-baseline.md (要件ベースライン文書。purpose/goal/scope_in 5 件/scope_out 4 件/acceptance 5 件の確定転記と、quality_constraints 5 件の個別充足条件、既存実装の棚卸し結果を含む)
...
- Write scope/touches: docs/features/feat-semantic-emphasis-icons/requirements-baseline.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p01) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p01 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p01) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は なし (P01 は本 package の起点 task) であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-semantic-emphasis-icons/requirements-baseline.md) が他 task の active lease と重複しないことを確認する
...
- docs/features/feat-semantic-emphasis-icons/requirements-baseline.md に goal-spec.json の purpose/goal/scope_in 5 件/scope_out 4 件/acceptance 5 件が逐語一致で転記されている
- quality_constraints 5 件 (emoji-ban-semantic-token-qa232-5 / icon-lint-ci-fail-closed-qa233-6 / icon-ownership-boundary-qa233-3 / design-system-token-and-contrast-gate / state-value-visible-label-qa232-2) が個別の充足条件として書き下されている
- 既存実装 (packages/ui/src/icons/index.tsx の callout 4 アイコン、packages/ui/src/components/Markdown.tsx の remarkCallouts、packages/ui/src/tokens/tokens.ts の infoBlue/infoBlueSoft) が『すでに存在する部分』として明記され、本 feature の残作業が絵文字 lint 未整備・一覧/カード状態表現の token 監査・可視ラベル併置監査に限定されることが要件として確定している
...
- Rollback trigger and steps: requirements-baseline.md を作成前状態 (ファイル未作成) へ戻し、goal-spec 側の再確定を dev-graph へ差し戻す
