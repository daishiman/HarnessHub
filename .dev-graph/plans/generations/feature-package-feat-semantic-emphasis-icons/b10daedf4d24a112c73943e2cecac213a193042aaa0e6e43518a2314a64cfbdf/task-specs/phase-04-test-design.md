# System task overlay: テストファースト設計 — 絵文字 lint・callout 描き分け・可視ラベル併置・ライトモード背景のテストスタブ作成
...
- phase_ref: P04
- classification: confidence=0.9, reason="goal-spec.json を入力に P04 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.9, candidate_path: tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p04.md}]
...
絵文字 lint・callout 描き分け・可視ラベル併置・ライトモード背景の 4 点について、実装前に失敗する検証可能なテストスタブを用意し、P05 の実装対象を確定する。
...
- Frontend: applicable + change: packages/ui/src/components/Markdown.test.tsx と packages/ui/src/tokens/tokens.test.ts にテストスタブを追加する
- Backend: N/A: 本 feature は backend 実装物を変更しない
- API: N/A: 本 feature は公開 API を変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: N/A: CI 組込み自体は P05 が行う
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: 絵文字 lint のテストスタブ (失敗系・成功系) を scripts/lint-ui-text-emoji.test.ts に追加する
- Documentation: N/A: 本 task は文書を新規作成しない
...
- Produced artifacts: 絵文字 lint・callout 描き分け・ライトモード背景・可視ラベル併置の各テストスタブ (実装前は red で失敗する)
...
- Write scope/touches: scripts/lint-ui-text-emoji.test.ts, packages/ui/src/components/Markdown.test.tsx, packages/ui/src/tokens/tokens.test.ts
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-emphasis-icons-p04) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-emphasis-icons-p04 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-emphasis-icons-p04) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-EMPHASIS-ICONS-P03 であり、直前 phase の完了後にのみ着手する。resource_scope (scripts/lint-ui-text-emoji.test.ts, packages/ui/src/components/Markdown.test.tsx, packages/ui/src/tokens/tokens.test.ts) が他 task の active lease と重複しないことを確認する
...
- 絵文字 lint の失敗系 (絵文字混入時に非ゼロ終了) と成功系 (混入なしで正常終了) のテストスタブが scripts/lint-ui-text-emoji.test.ts に作成されている
- callout 4 種それぞれで異なる Icon name と calloutStyle トークンが適用されることを確認するテストスタブが packages/ui/src/components/Markdown.test.tsx に作成されている
- ライトモードで callout point/note の背景色が infoBlueSoft (グレー系トークンではない) になることを確認するテストスタブが作成されている
- 状態・日時・金額・PII・略語の表現でアイコン単体ではなく可視ラベルが併置されることを確認するテストスタブが作成されている
...
- Rollout: lint-ui-text-emoji.test.ts を write scope 内に限定して作成/実装し、次フェーズへ引き継ぐ
- Rollback trigger and steps: 追加したテストスタブファイルを削除し、テストファースト設計を P03 のレビュー結果へ差し戻す
...
- Dependencies: SYS-EMPHASIS-ICONS-P03
