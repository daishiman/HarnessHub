# System task overlay: 独立設計レビュー — HearingSheet/FormData と共通ai_jobs(kind=sheet_generation) consumer 設計・AI キュー認可・Markdown sanitize の妥当性確認
...
- phase_ref: P03
- classification: confidence=0.88, reason="P02 で確定した HearingSheet/FormData と共通ai_jobs(kind=sheet_generation) consumer スキーマと AI キュー API 契約・Markdown sanitize 適用点を、設計担当から独立した視点でレビューする P03 タスク", candidates=[{artifact_kind: task, confidence: 0.88, candidate_path: tasks/feat-hearing-intake/sys-hearing-intake-p03.md}]
...
P02 で確定した HearingSheet/FormData と共通ai_jobs(kind=sheet_generation) consumer スキーマ、受付番号採番方式、AI キュー API 契約、Markdown sanitize 適用点を、設計担当から独立した視点でレビューし、SEC2/SEC5/SEC7/SEC8 と qa-021/qa-022/qa-023/qa-024 への適合を確認する。
...
- Data: applicable + change: HearingSheet/FormData と共通ai_jobs(kind=sheet_generation) consumer のカラム設計が D4 (tenant_id/workspace_id スコープ列必須) と qa-024 に適合しているかをレビューする
