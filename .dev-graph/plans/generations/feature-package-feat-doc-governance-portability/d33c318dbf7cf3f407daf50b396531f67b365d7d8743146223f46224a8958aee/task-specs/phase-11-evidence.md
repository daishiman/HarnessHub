# System task overlay: 証跡固定 — 実測ログと成果物 digest の evidence manifest 化
...
- phase_ref: P11
- classification: confidence=0.87, reason="qa-070 のドキュメント統治・移植性境界 3 検査のうち P11 責務 (証跡固定 — 実測ログと成果物 digest の evidence manifest 化) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-doc-governance-portability/sys-doc-governance-portability-p11.md}]
...
P06 (テスト実行)・P08 (移行)・P09 (品質保証) の実測ログと P05 の実装成果物について、path・sha256・生成 phase を evidence manifest として固定し、再検証可能な証跡を残す。

## 背景

「再実行しても同じ結果になる (冪等)」という quality_constraints を将来にわたって主張し続けるには、どの成果物がどの実測ログに紐づき、どのコマンドの再実行でその実測を再現できるかを機械可読に固定しておく必要がある。P11 は新たな実測は行わず、既存の P06/P08/P09 の実測ログと P05 の成果物を対象に digest を採取し、再実行コマンドとともに manifest 化する。
...
- Security: N/A: 本 phase は新たな検証を行わず既存実測の証跡固定のみ行う
- Quality: applicable + change: P06/P08/P09 の実測ログと P05 成果物の digest を evidence manifest として固定する
- Documentation: N/A: 本 phase は運用文書ではなく機械可読な evidence manifest を生成する
- Operations: N/A: 本 phase は運用手順を確定しない (証跡固定のみ)
...
- Produced artifacts:
- eval-log/dev-graph/doc-governance-portability/evidence-manifest.json
- Consumed artifacts: eval-log/dev-graph/doc-governance-portability/test-run-p06.json, eval-log/dev-graph/doc-governance-portability/migration-receipt.json, eval-log/dev-graph/doc-governance-portability/qa-fail-closed-report.json, scripts/lint-doc-line-limit.py, scripts/lint-mechanism-knowledge-boundary.py, scripts/lint-portability-knowledge-optin.py, scripts/doc-line-limit-allowlist.json
- Write scope/touches: eval-log/dev-graph/doc-governance-portability/evidence-manifest.json
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-doc-governance-portability-p11) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-doc-governance-portability-p11 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-doc-governance-portability-p11) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 phase 完了後にのみ着手する (直列 DAG)。resource_scope (eval-log/dev-graph/doc-governance-portability/evidence-manifest.json) が他 task の active lease と重複しないことを確認する
...
- Required evidence: evidence-manifest.json に P05/P06/P08/P09 各成果物の path・sha256・生成 phase・再実行コマンドが記録されていること

## Rollout and rollback

- Rollout: 成果物を write scope の範囲で作成・更新し、acceptance を満たしたことを確認してから次 phase へ引き継ぐ
- Rollback trigger and steps: digest 採取対象に未実測・未確定の成果物が含まれると判明した場合、該当成果物を生成した phase へ差し戻す
...
- evidence-manifest.json に P05 実装成果物 (3 lint script・allowlist・回帰テスト・CI 配線) と P06/P08/P09 実測ログの path・sha256・生成 phase が漏れなく記録されている
- 各成果物について再実行コマンドが記録され、再実行結果が既存 manifest の digest と一致することが確認可能な形式になっている
- 本 phase 自身が新たな実測・判定を行っていないこと (証跡固定のみ) が明示されている
