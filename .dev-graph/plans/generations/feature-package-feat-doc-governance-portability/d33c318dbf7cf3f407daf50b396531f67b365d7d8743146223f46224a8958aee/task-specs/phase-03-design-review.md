# System task overlay: 設計レビュー — fail-closed・冪等・digest 不変・境界非複製との整合検証
...
- phase_ref: P03
- classification: confidence=0.87, reason="qa-070 のドキュメント統治・移植性境界 3 検査のうち P03 責務 (設計レビュー — fail-closed・冪等・digest 不変・境界非複製との整合検証) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-doc-governance-portability/sys-doc-governance-portability-p03.md}]
...
P02 設計が quality_constraints 4 件 (fail-closed-lint・idempotent-verification・single-writer-boundary・digest-immutability) および仕組み-ナレッジ境界の非複製原則と整合していることをレビューし、実装着手前に不整合を検出する。

## 背景

3 検査は CI ゲートとして常時実行されるため、設計段階での fail-closed/冪等性の欠落は本番運用後の是正コストが大きい。既存の upsert-node.py (graph 側の単一 writer)・bd-bridge.py (beads 側の単一 writer) を検査が迂回する設計になっていないか、また文書分割時の index/一覧表参照構造 (single-writer-boundary quality_constraint が要求する非複製) を壊す設計になっていないかを本 phase で確認する。
...
- Security: applicable + review: single-writer-boundary quality_constraint と境界非複製原則の迂回耐性をレビューする
- Quality: applicable + review: fail-closed・冪等・digest 不変の整合を検証する
- Documentation: applicable + change: docs/features/feat-doc-governance-portability/design-review.md を新規作成する
- Operations: N/A: 本 phase は運用手順を確定しない (レビューのみ)
...
- Produced artifacts:
- docs/features/feat-doc-governance-portability/design-review.md
- Consumed artifacts: goal-spec.json, features/feat-doc-governance-portability.context.json, docs/features/feat-doc-governance-portability/design.md, system-spec/dev-workflow.md (qa-070)
- Write scope/touches: docs/features/feat-doc-governance-portability/design-review.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-doc-governance-portability-p03) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-doc-governance-portability-p03 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-doc-governance-portability-p03) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 phase 完了後にのみ着手する (直列 DAG)。resource_scope (docs/features/feat-doc-governance-portability/design-review.md) が他 task の active lease と重複しないことを確認する
...
- Required evidence: design-review.md に 5 観点それぞれの verdict と根拠が記載されていること
...
- design-review.md に 5 観点 (fail-closed・冪等・単一 writer 境界・digest 不変・仕組み-ナレッジ境界非複製) それぞれの整合検証結果が verdict 付きで記録されている
- 3 検査いずれも警告のみの soft lint ではなく exit 非 0 の fail-closed であることが確認されている
