# System task overlay: テスト実行 — 回帰テスト全件と 3 検査の実測記録
...
- phase_ref: P06
- classification: confidence=0.87, reason="qa-070 のドキュメント統治・移植性境界 3 検査のうち P06 責務 (テスト実行 — 回帰テスト全件と 3 検査の実測記録) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-doc-governance-portability/sys-doc-governance-portability-p06.md}]
...
P04 で設計した全テスト ID を実行し、pass/fail と実行コマンド・exit code を生のまま記録する。

## 背景

実装が契約通り動作することを実測で確認しない限り、次 phase (受入判定) の根拠にならない。P05 の実装成果物 (3 lint script・回帰テスト・CI 配線) を対象に、fixture 実行結果をそのまま保存し、恣意的な要約や成功への書き換えを行わない。
...
- Quality: applicable + change: 回帰テスト全件を実行し実測を記録する
- Documentation: N/A: 本 phase は文書ではなく機械可読な実行ログを生成する
- Operations: N/A: 本 phase は CI 配線自体を変更しない
...
- Produced artifacts:
- eval-log/dev-graph/doc-governance-portability/test-run-p06.json
- Consumed artifacts: docs/features/feat-doc-governance-portability/test-plan.md, scripts/lint-doc-line-limit.py, scripts/lint-mechanism-knowledge-boundary.py, scripts/lint-portability-knowledge-optin.py, tests/scripts-root/test_root__lint_doc_line_limit.py, tests/scripts-root/test_root__lint_mechanism_knowledge_boundary.py, tests/scripts-root/test_root__lint_portability_knowledge_optin.py
- Write scope/touches: eval-log/dev-graph/doc-governance-portability/test-run-p06.json
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-doc-governance-portability-p06) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-doc-governance-portability-p06 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-doc-governance-portability-p06) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 phase 完了後にのみ着手する (直列 DAG)。resource_scope (eval-log/dev-graph/doc-governance-portability/test-run-p06.json) が他 task の active lease と重複しないことを確認する
...
- Required evidence: test-run-p06.json に全テスト ID の実行コマンド・exit code・pass/fail 件数が記録されていること
