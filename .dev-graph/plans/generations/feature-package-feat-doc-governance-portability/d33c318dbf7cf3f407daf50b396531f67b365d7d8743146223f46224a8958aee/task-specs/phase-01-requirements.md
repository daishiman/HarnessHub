# System task overlay: 要件ベースライン確定 — qa-070 ドキュメント規約 2 件・3 検査スコープの baseline 文書化
...
- phase_ref: P01
- classification: confidence=0.87, reason="qa-070 のドキュメント統治・移植性境界 3 検査のうち P01 責務 (要件ベースライン確定 — qa-070 ドキュメント規約 2 件・3 検査スコープの baseline 文書化) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-doc-governance-portability/sys-doc-governance-portability-p01.md}]
...
qa-070 (appr-008 承認) で確定したドキュメント規約 2 件 (仕組み-ナレッジ オン/オフ分離、markdown 正本文書 1 文書 300 行上限) と goal-spec の purpose/goal/scope_in 3 件/scope_out 4 件/acceptance 4 件/quality_constraints 4 件 (id 単位) を machine-verifiable な要件ベースラインとして固定し、P02 以降の全 task が同一の合意事項を参照できる状態にする。

## 背景

system-spec/dev-workflow.md の qa-070 は、仕組み (plugins/・.claude/skills/ 等) とナレッジ (system-spec/・specs/・architecture/・features/・tasks/・issues/・docs/・eval-log/・.dev-graph/state 等) の境界を明確化し、移植チャネル (extract-blueprint / install-bundle 等) は仕組みのみを既定 (オン) にしてナレッジ同梱を明示 opt-in (オフ既定) とすることと、markdown 正本文書の 300 行上限を fail-closed CI lint で強制することを確定した。既存 6 文書 (docs/security-spec.md 等) は既に 300 行を超過しているため、縮小のみ許す remediation allowlist ratchet で当面の例外として扱う。本 task はこれらを再解釈や欠落なく baseline 化し、P02 で確定すべき事項 (3 検査の入出力契約・allowlist schema・hard-coded 参照検出の false-positive 除外基準・CI 配線先) を据置事項として明記する。
...
- Quality: applicable + change: goal-spec acceptance 4 件と quality_constraints 4 件を machine-verifiable な受入基準として固定する
- Documentation: applicable + change: docs/features/feat-doc-governance-portability/requirements-baseline.md を新規作成する
- Operations: applicable + review: allowlist ratchet の移行対象 (既存超過 6 文書) を初期記載し、P08 の移行スコープを明確化する
...
- Produced artifacts:
- docs/features/feat-doc-governance-portability/requirements-baseline.md
- Consumed artifacts: goal-spec.json, features/feat-doc-governance-portability.context.json, architecture/harness-hub-dev-workflow.md, system-spec/dev-workflow.md (qa-070), issues/sys-doc-granularity-remediation-20260722.md
- Write scope/touches: docs/features/feat-doc-governance-portability/requirements-baseline.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-doc-governance-portability-p01) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-doc-governance-portability-p01 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-doc-governance-portability-p01) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on が空のため他 task との着手順序制約はない。resource_scope (docs/features/feat-doc-governance-portability/requirements-baseline.md) が他 task の active lease と重複しないことを確認する
...
- Required evidence: requirements-baseline.md に goal-spec の全項目が過不足なく転記され、qa-070 のドキュメント規約 2 件との対応表が含まれること
...
- requirements-baseline.md に goal-spec.json の purpose/goal/scope_in 3 件/scope_out 4 件/acceptance 4 件/quality_constraints 4 件 (id 単位) が逐語一致で転記されている
- 3 検査スコープ ((1) 300 行 fail-closed lint と 6 文書 allowlist ratchet、(2) 仕組みファイルの hard-coded ナレッジ参照検査、(3) 移植チャネルの mechanism-only 既定/knowledge opt-in 検査) がそれぞれ scope_in の対応項目に紐付けて記載されている
- 出典として system-spec/dev-workflow.md の qa-070 (appr-008 承認) が参照のみで引用され、条文本文が requirements-baseline.md へ複製されていない
- P02 で確定すべき据置事項 (3 検査の入出力契約・allowlist schema・hard-coded 参照検出の false-positive 除外基準・CI 配線先) が明記されている
