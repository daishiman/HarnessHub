# System task overlay: リリース — main 反映と 3 検査の CI fail-closed 実効性の実証
...
- phase_ref: P13
- classification: confidence=0.87, reason="qa-070 のドキュメント統治・移植性境界 3 検査のうち P13 責務 (リリース — main 反映と 3 検査の CI fail-closed 実効性の実証) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-doc-governance-portability/sys-doc-governance-portability-p13.md}]
...
P01〜P12 の成果物を default branch (main) へ反映し、3 検査 (300 行 lint・仕組み-ナレッジ境界検査・移植導線 opt-in 検査) が governance-check.yml 経由で CI 上 fail-closed に稼働していることを実環境で実証する。

## 背景

これまでの phase の実測はローカル/開発 worktree での実行結果であり、main への反映後に CI パイプライン上で同一の fail-closed 挙動が再現されることを確認して初めてリリース完了となる。この実証をもって goal-spec の goal (3 検査が CI で fail-closed に動作し、既存超過 6 文書の allowlist は縮小のみ許す ratchet で管理され、再実行しても同じ結果になる状態) が達成されたと判定する。
...
- Infrastructure: applicable + change: governance-check.yml の CI 実行結果 (main 反映後) を実証対象とする
- Security: N/A: 迂回経路の検証自体は P09 で完了済み。本 phase は CI 上での再現確認のみ行う
- Quality: applicable + review: CI 上の実行結果が P06/P09 のローカル実測結果と一致することを確認する
- Documentation: N/A: 運用文書は P12 で確定済み。本 phase は追加文書を作成しない
- Operations: applicable + change: main 反映と CI 実証を実施する
...
- Produced artifacts:
- eval-log/dev-graph/doc-governance-portability/release-receipt.json
- Consumed artifacts: docs/features/feat-doc-governance-portability/final-review.md, docs/features/feat-doc-governance-portability/operations.md, eval-log/dev-graph/doc-governance-portability/evidence-manifest.json, .github/workflows/governance-check.yml
- Write scope/touches: eval-log/dev-graph/doc-governance-portability/release-receipt.json
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-doc-governance-portability-p13) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-doc-governance-portability-p13 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-doc-governance-portability-p13) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 phase 完了後にのみ着手する (直列 DAG)。resource_scope (eval-log/dev-graph/doc-governance-portability/release-receipt.json) が他 task の active lease と重複しないことを確認する
...
- Required evidence: release-receipt.json に main 反映後の CI run 結果 (governance-check.yml の change-category-guard job の 3 検査 exit code) と、P06/P09 ローカル実測結果との一致確認が記録されていること

## Rollout and rollback

- Rollout: 成果物を write scope の範囲で作成・更新し、acceptance を満たしたことを確認してから feature package を完了とする
- Rollback trigger and steps: main 上の CI 実行結果がローカル実測結果と乖離した場合、release-receipt.json に乖離内容を記録したまま該当 phase (P05 実装または P09 品質保証) へ差し戻す
...
- release-receipt.json に main 反映後の governance-check.yml CI run 結果が記録され、3 検査すべてが fail-closed に動作していることが確認されている
- CI 上の実行結果が P06 (テスト実行)・P09 (品質保証) のローカル実測結果と一致している
- goal-spec の goal 全体 (3 検査の CI fail-closed 稼働・allowlist ratchet の縮小のみ許す運用・冪等) が達成済みと最終確認されている
