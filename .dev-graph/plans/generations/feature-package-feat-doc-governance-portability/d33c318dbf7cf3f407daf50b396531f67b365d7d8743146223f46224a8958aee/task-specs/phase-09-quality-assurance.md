# System task overlay: 品質保証 — fail-closed 実効性と境界突破 (迂回経路) の悪性ケース実測
...
- phase_ref: P09
- classification: confidence=0.87, reason="qa-070 のドキュメント統治・移植性境界 3 検査のうち P09 責務 (品質保証 — fail-closed 実効性と境界突破の悪性ケース実測) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-doc-governance-portability/sys-doc-governance-portability-p09.md}]
...
3 検査 (300 行 lint・仕組み-ナレッジ境界検査・移植導線 opt-in 検査) が warning-only に緩んでいないこと、および迂回 (evasion) パターンでも fail-closed に遮断されることを、悪性ケースの実測で証明する。

## 背景

fail-closed の quality_constraints は「一度でも warning-only の抜け道が実在すれば規約全体が形骸化する」性質を持つ。P04 で設計した MUST_DETECT fixture の実行 (P06) だけでは、実装者が想定していない迂回経路 (例: allowlist への不正追加、コメント化による検査対象除外、qa 番号の分割記述による検出回避) までは検証できない。本 phase は P05 の実装成果物に対して、設計時に想定していなかった迂回パターンを追加で試行し、実効性を突破 (bypass) できないことを確認する。
...
- Security: applicable + review: 迂回経路 (allowlist 不正追加・検査対象除外・分割記述による検出回避) の悪性ケースを probing する
- Quality: applicable + change: fail-closed 実効性の悪性ケース実測を行い、qa-fail-closed-report.json を作成する
- Documentation: N/A: 本 phase は文書ではなく機械可読な実測レポートを生成する
- Operations: N/A: 本 phase は運用手順を確定しない (品質保証のみ)
...
- Produced artifacts:
- eval-log/dev-graph/doc-governance-portability/qa-fail-closed-report.json
- Consumed artifacts: scripts/lint-doc-line-limit.py, scripts/lint-mechanism-knowledge-boundary.py, scripts/lint-portability-knowledge-optin.py, scripts/doc-line-limit-allowlist.json, eval-log/dev-graph/doc-governance-portability/test-run-p06.json, eval-log/dev-graph/doc-governance-portability/migration-receipt.json
- Write scope/touches: eval-log/dev-graph/doc-governance-portability/qa-fail-closed-report.json
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-doc-governance-portability-p09) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-doc-governance-portability-p09 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-doc-governance-portability-p09) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 phase 完了後にのみ着手する (直列 DAG)。resource_scope (eval-log/dev-graph/doc-governance-portability/qa-fail-closed-report.json) が他 task の active lease と重複しないことを確認する
...
- Required evidence: qa-fail-closed-report.json に迂回パターン (allowlist 不正追加・検査対象コメント化除外・qa 番号分割記述回避を含む最低 3 系統) ごとの試行結果と、すべて exit 非 0 で遮断されたことの記録があること

## Rollout and rollback

- Rollout: 成果物を write scope の範囲で作成・更新し、acceptance を満たしたことを確認してから次 phase へ引き継ぐ
- Rollback trigger and steps: 迂回が成功 (fail-closed が破られた) 場合、qa-fail-closed-report.json に失敗を記録したまま P05 実装へ差し戻す
...
- qa-fail-closed-report.json に迂回パターン最低 3 系統 (allowlist 不正追加・検査対象コメント化除外・qa 番号分割記述回避) の試行結果が記録され、いずれも exit 非 0 で遮断されている
- 3 検査のいずれにも warning-only な (exit 0 のまま検出のみ報告する) 経路が存在しないことが確認されている
- コメント/docstring 内の qa 番号引用のような良性ケースが誤って遮断されないこと (false positive 非発生) が再確認されている
