...
- phase_ref: P10
- classification: confidence=0.87, reason="qa-070 のドキュメント統治・移植性境界 3 検査のうち P10 責務 (最終レビュー — 全 phase 成果の横断整合確認) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-doc-governance-portability/sys-doc-governance-portability-p10.md}]
...
P01〜P09 の成果物・acceptance 判定・quality_constraints 遵守状況を横断的に見直し、release (P13) へ進む前の最終的な整合確認を行う。

## 背景

各 phase は個別に acceptance を満たしていても、phase 間の記述 (write_scope・成果物の相互参照・qa-070 の引用箇所) に矛盾が残ることがある。特に P07 の受入判定が P08 完了前提の暫定判定だった項目 (allowlist ratchet の縮小のみ許す性質) を、P08/P09 完了後の実測に基づき最終確定させる必要がある。
...
- Security: N/A: 迂回経路の検証自体は P09 で完了済みであり、本 phase は判定の横断確認のみ行う
- Quality: applicable + change: P01〜P09 の成果物・acceptance・quality_constraints 遵守を横断確認する
- Documentation: applicable + change: docs/features/feat-doc-governance-portability/final-review.md を新規作成する
- Operations: N/A: 本 phase は運用手順を確定しない (横断レビューのみ)
...
- Produced artifacts:
- docs/features/feat-doc-governance-portability/final-review.md
- Consumed artifacts: docs/features/feat-doc-governance-portability/requirements-baseline.md, docs/features/feat-doc-governance-portability/design.md, docs/features/feat-doc-governance-portability/design-review.md, docs/features/feat-doc-governance-portability/test-plan.md, docs/features/feat-doc-governance-portability/acceptance-report.md, eval-log/dev-graph/doc-governance-portability/test-run-p06.json, eval-log/dev-graph/doc-governance-portability/migration-receipt.json, eval-log/dev-graph/doc-governance-portability/qa-fail-closed-report.json
- Write scope/touches: docs/features/feat-doc-governance-portability/final-review.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-doc-governance-portability-p10) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-doc-governance-portability-p10 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-doc-governance-portability-p10) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 phase 完了後にのみ着手する (直列 DAG)。resource_scope (docs/features/feat-doc-governance-portability/final-review.md) が他 task の active lease と重複しないことを確認する
...
- Required evidence: final-review.md に P01〜P09 各 phase の成果物パスと acceptance 判定結果の横断一覧、および P07 暫定判定の最終確定結果が記載されていること

## Rollout and rollback

- Rollout: 成果物を write scope の範囲で作成・更新し、acceptance を満たしたことを確認してから次 phase へ引き継ぐ
- Rollback trigger and steps: 横断確認で矛盾・未達が見つかった場合、final-review.md に指摘事項を記録したまま該当 phase へ差し戻す
...
- final-review.md に P01〜P09 各 phase の成果物・acceptance 判定が横断的に一覧化されている
- P07 で P08 完了待ちとされた acceptance 項目 (allowlist ratchet) が P08/P09 の実測結果に基づき最終確定している
- goal-spec の acceptance 4 件すべてが最終的に充足していると判定されている (未達があれば差し戻し先が明記されている)
