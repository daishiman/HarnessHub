...
- Compatibility/migration/backfill: P08のBuild-only next delta/backfillを本番適用する。combined 0008・snapshot・journal既存entryは不変で、Metrics delta/releaseは同梱しない

## 成果物

- Produced artifacts: docs/features/feat-build-pipeline-board/release-notes.md (本番反映結果・スモークテスト結果・正本書き戻し結果)、system-spec/frontend.md、system-spec/backend.md、system-spec/database.md、system-spec/security.md、architecture/harness-hub-frontend.md、architecture/harness-hub-backend.md (本番実測で確定した内容の反映)
- Consumed artifacts: docs/features/feat-build-pipeline-board/runbook.md, P08-resolved Build-only NNNN migration, matching snapshot/journal evidence, typed handoff receipts, P11 exact command/evidence index
- Write scope/touches: docs/features/feat-build-pipeline-board/release-notes.md, system-spec/frontend.md, system-spec/backend.md, system-spec/database.md, system-spec/security.md, architecture/harness-hub-frontend.md, architecture/harness-hub-backend.md
...
- Automated commands: `pnpm --filter @harness-hub/db migrate:deploy`, `pnpm --filter @harness-hub/hub deploy`, `pnpm --filter @harness-hub/hub exec vitest run src/__tests__/build-pipeline-board`, `python3 plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-build-pipeline-board`
- Required evidence: release-notes.md に本番反映日時、スモークテスト結果 (S13 到達確認・builds API 疎通確認・migration 適用確認)、system-spec/ と architecture/ の各関連正本への反映内容または意味変更なしの照合結果が記録されていること。Normative evidence: stage transition/API tests、axe report、CWV report (LCP/INP/CLS各good)、CI job URLまたは再実行可能ログを必須証跡とする。
...
- P13 spec/architecture writeback: required。実行結果・判断・改善知見を docs/features/feat-build-pipeline-board/ のP13成果物へ証跡化し、確定した内容を system-spec/ と architecture/ の関連正本へ反映する。意味変更がない場合も照合結果を記録する。

## Rollout and rollback

- Rollout: migration 適用後に本番デプロイを実行し、スモークテスト全件passを確認する。実行結果・判断・改善知見を関連するsystem-spec/とarchitecture/の正本へ書き戻し、照合結果をrelease-notes.mdへ記録して確定する
