# System task overlay: リファクタリングと移行 — scope 解決の二重実装排除と既定着地定数の集約
...
- phase_ref: P08
- classification: confidence=0.92, reason="goal-spec.json を入力に P08 の単一責務 (frontend) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-post-signin-scope-routing/sys-post-signin-scope-p08.md}]
...
scope 解決の分岐と既定着地の定数を集約し、経路ごとの二重実装と path 直書きを排除する。
...
統合修正では、既存経路を残したまま新経路を足すと分岐が二重化しやすい。とくに scope 解決は明示ヘッダー系統と session 系統で判定を書き分けると、片方だけ修正されて挙動が乖離する退行が起きる。合流点を単一関数に保つことが本 task の主目的である。既定着地 /sheets も同様に、画面ごとに直書きすると仕様変更時の追従漏れが生じるため単一定数へ集約する。本 feature は authorize() の判定規則そのものを所有しない (owner=feat-auth-tenancy)。業務画面本体も所有しない (owner=feat-dual-catalog-web)。Workspace 選択画面の UI は feat-workspace-switch-ux、Web 公開ウィザードの導線は feat-web-only-publish-journey が所有する。本 feature が所有するのは scope 解決の入力系統と着地先解決の結線に限られる。
...
- Entry gate: 直前 task SYS-POST-SIGNIN-SCOPE-P07 が done または closed であること
- Source pin: system-spec-harness v0.1.0 / run-system-spec-compile / assign-system-spec-completeness-evaluator
- Repository context: repo_identity=github:daishiman/HarnessHub、root_resolution_source=explicit-cli (validate-system-plan.py 実行時に --repo-root を明示指定する運用)、config=.dev-graph/config.json。全 path は repository 相対とし absolute path は使用しない

## Workstream applicability

- Frontend: applicable + change: 既定着地の定数集約と、着地先解決の呼び出し箇所の整理を行う
- Backend: applicable + change: scope 解決の分岐を単一関数へ集約する
- API: applicable: authorize() の外部契約 (reason と status) を変えずに内部構造だけを整理する
- Data: N/A: データモデル変更を伴わないため移行対象がない
- Infrastructure: N/A: インフラ変更を伴わないため移行対象がない
- Security: applicable: 整理の前後で deny-by-default と open redirect 防止の挙動が同一であることを確認する
- Quality: applicable + change: リファクタリング前後で P06 のテスト結果が同一であることを確認する
- Documentation: applicable + change: docs/features/feat-post-signin-scope-routing/refactoring-record.md を新規作成する
...
- Produced artifacts: docs/features/feat-post-signin-scope-routing/refactoring-record.md (集約前後の構造と、テスト結果同一性の確認記録)、および実装ファイルの整理
...
- Write scope/touches: apps/hub/src/lib/authz/resource.ts, apps/hub/src/lib/authz/types.ts, apps/hub/src/middleware/authz.ts, apps/hub/src/lib/auth/session.ts, apps/hub/src/lib/routing/post-signin-landing.ts, apps/hub/src/app/[tenant_slug]/signin/tenant-oidc-signin-form.tsx, apps/hub/src/app/page.tsx, docs/features/feat-post-signin-scope-routing/refactoring-record.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-post-signin-scope-p08) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-post-signin-scope-p08 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-post-signin-scope-p08) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on (SYS-POST-SIGNIN-SCOPE-P07) の完了後に着手する。resource_scope (apps/hub/src/lib/authz/resource.ts, apps/hub/src/lib/authz/types.ts, apps/hub/src/middleware/authz.ts, apps/hub/src/lib/auth/session.ts, apps/hub/src/lib/routing/post-signin-landing.ts, apps/hub/src/app/[tenant_slug]/signin/tenant-oidc-signin-form.tsx, apps/hub/src/app/page.tsx, docs/features/feat-post-signin-scope-routing/refactoring-record.md) が他 task の active lease と重複しないことを確認する
- Completion projection: feature branch 上の完了は pending event として記録され、default branch (main) へのクリーンな reconciliation で durable done へ確定する

## スコープ外

- 外部契約 (reason と status) の変更
- 新規機能の追加
- 仕様文書の更新 (owner=P12)
...
- Required evidence: docs/features/feat-post-signin-scope-routing/refactoring-record.md に、scope 解決の分岐が単一関数へ集約されたこと・既定着地の直書きが 0 件であること・リファクタリング前後で P06 のテスト結果が同一であることが記録されていること
...
- Rollout: 整理後に P06 のテストを再実行し結果同一を確認してから P09 へ引き継ぐ
- Rollback trigger and steps: テスト結果が変化した場合は整理を revert し、原因を特定してから本 task を再実行する
...
- Phase responsibility: 現行 context の purpose・goal・scope・acceptance のうち P08 の責務に対応する部分を扱う。
...
- Dependencies: SYS-POST-SIGNIN-SCOPE-P07
