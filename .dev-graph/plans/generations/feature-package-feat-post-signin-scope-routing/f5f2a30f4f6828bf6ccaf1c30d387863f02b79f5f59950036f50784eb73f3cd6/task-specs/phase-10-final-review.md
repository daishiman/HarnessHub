# System task overlay: 最終レビュー — 実行済み証跡のみによるリリース可否判定
...
- phase_ref: P10
- classification: confidence=0.92, reason="goal-spec.json を入力に P10 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-post-signin-scope-routing/sys-post-signin-scope-p10.md}]
...
P06・P07・P09 の実行済み証跡のみを根拠に、本 feature のリリース可否を判定する。
...
最終レビューでは、未実行・未実装の項目がリリース可の根拠に混入していないかを構造的に確認する。文書化された計画や、実装予定の記述は根拠にしない。本 feature は認可に触れるため、deny-by-default 非退行の証跡が欠けている場合はリリース不可とする。本 feature は authorize() の判定規則そのものを所有しない (owner=feat-auth-tenancy)。業務画面本体も所有しない (owner=feat-dual-catalog-web)。Workspace 選択画面の UI は feat-workspace-switch-ux、Web 公開ウィザードの導線は feat-web-only-publish-journey が所有する。本 feature が所有するのは scope 解決の入力系統と着地先解決の結線に限られる。
...
- Entry gate: 直前 task SYS-POST-SIGNIN-SCOPE-P09 が done または closed であること
- Source pin: system-spec-harness v0.1.0 / run-system-spec-compile / assign-system-spec-completeness-evaluator
- Repository context: repo_identity=github:daishiman/HarnessHub、root_resolution_source=explicit-cli (validate-system-plan.py 実行時に --repo-root を明示指定する運用)、config=.dev-graph/config.json。全 path は repository 相対とし absolute path は使用しない

## Workstream applicability

- Frontend: applicable: 着地先解決の証跡完全性を確認する
- Backend: applicable: scope 解決と所属検証の証跡完全性を確認する
- API: applicable: authorize() の応答証跡の完全性を確認する
- Data: N/A: データモデル変更を伴わないため確認対象がない
- Infrastructure: N/A: インフラ変更を伴わないため確認対象がない
- Security: applicable + change: deny-by-default 非退行と open redirect 防止の証跡が実行済みであることをリリース条件として判定する
- Quality: applicable + change: 未実行項目がリリース可の根拠に混入していないことを確認する
- Documentation: applicable + change: docs/features/feat-post-signin-scope-routing/final-review.md を新規作成する
...
- Produced artifacts: docs/features/feat-post-signin-scope-routing/final-review.md (リリース可否判定と根拠証跡の一覧)
...
- Write scope/touches: docs/features/feat-post-signin-scope-routing/final-review.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-post-signin-scope-p10) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-post-signin-scope-p10 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-post-signin-scope-p10) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on (SYS-POST-SIGNIN-SCOPE-P09) の完了後に着手する。resource_scope (docs/features/feat-post-signin-scope-routing/final-review.md) が他 task の active lease と重複しないことを確認する
...
- Required evidence: docs/features/feat-post-signin-scope-routing/final-review.md にリリース可否と、その根拠となる P06・P07・P09 の実行済み証跡が列挙されていること
...
- Rollout: リリース可の判定が出てから P11 へ引き継ぐ
- Rollback trigger and steps: リリース不可の場合は不足証跡の owner phase へ差し戻し、充足後に本 task を再実行する
...
- Phase responsibility: 現行 context の purpose・goal・scope・acceptance のうち P10 の責務に対応する部分を扱う。
...
- Dependencies: SYS-POST-SIGNIN-SCOPE-P09
