# System task overlay: 要件ベースライン確定 — scope 解決 2 系統・サインイン後着地先解決・open redirect 防止・deny-by-default 非退行の要件確定
...
- phase_ref: P01
- classification: confidence=0.92, reason="goal-spec.json を入力に P01 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-post-signin-scope-routing/sys-post-signin-scope-p01.md}]
...
feat-post-signin-scope-routing の受入可能な要件ベースラインを確定し、P02 以降の全 task が同一の合意事項 (scope 解決 2 系統、session への active workspace 束縛、サインイン後の着地先解決、open redirect 防止、deny-by-default 非退行) を参照できる状態にする。この task 完了時点で、goal-spec の purpose/goal/scope_in 10 件/scope_out 6 件/acceptance 8 件/quality_constraints 6 件が machine-verifiable な baseline 文書として固定される。
...
本 task は上記の確定要件を再解釈や欠落なく baseline 化するとともに、cross-feature 境界の未確定事項を上流未解決事項として baseline に明記し、P02 の必須解消事項として引き継ぐ。本 feature は authorize() の判定規則そのものを所有しない (owner=feat-auth-tenancy)。業務画面本体も所有しない (owner=feat-dual-catalog-web)。Workspace 選択画面の UI は feat-workspace-switch-ux、Web 公開ウィザードの導線は feat-web-only-publish-journey が所有する。本 feature が所有するのは scope 解決の入力系統と着地先解決の結線に限られる。
...
- Entry gate: goal-spec.json の feature_context_digest が sha256:d2f1b1eafc3773a672b279d784dbe1fec10902d32a31643edd8d0bf8379cfcfb に一致し、features/feat-post-signin-scope-routing.md の confirmation_status が confirmed であること
- Source pin: system-spec-harness v0.1.0 / run-system-spec-compile / assign-system-spec-completeness-evaluator
- Repository context: repo_identity=github:daishiman/HarnessHub、root_resolution_source=explicit-cli (validate-system-plan.py 実行時に --repo-root を明示指定する運用)、config=.dev-graph/config.json。全 path は repository 相対とし absolute path は使用しない

## Workstream applicability

- Frontend: N/A: 本 task は要件文書化のみで frontend 実装物を変更しない (着地先解決の実装は P05 が扱う)
- Backend: N/A: 本 task は要件文書化のみで backend 実装物を変更しない
- API: N/A: scope 解決の入出力契約詳細は P02 で定義する。本 task は要件記述のみ
- Data: N/A: 本 feature は既存 session と principal の情報のみを扱い、新規テーブルを設けない
- Infrastructure: N/A: Cloudflare Workers デプロイ単位は既に確立しており、本 feature は追加インフラを新設しない
- Security: applicable + change: quality_constraints 6 件 (authz-decision-order-and-deny-by-default-non-regression, scope-resolution-two-inputs-ambiguous-rejection, session-active-workspace-binding-membership-revalidation, post-signin-landing-resolution-single-default-constant, open-redirect-prevention-same-origin-relative-only, integration-fix-not-new-capability-cross-feature-boundary) を要件ベースラインへ確定記述する
- Quality: applicable + change: goal-spec acceptance 8 件と quality_constraints 6 件を machine-verifiable な受入基準として requirements-baseline.md に固定する
- Documentation: applicable + change: docs/features/feat-post-signin-scope-routing/requirements-baseline.md を新規作成する
- Operations: N/A: scope 未解決時の運用手順の具体化は P12 で行う
...
- Produced artifacts: docs/features/feat-post-signin-scope-routing/requirements-baseline.md (要件ベースライン文書。purpose/goal/scope_in 10 件/scope_out 6 件/acceptance 8 件/quality_constraints 6 件の確定転記と cross-feature 境界の P02 必須解消事項を含む)
...
- Write scope/touches: docs/features/feat-post-signin-scope-routing/requirements-baseline.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-post-signin-scope-p01) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-post-signin-scope-p01 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-post-signin-scope-p01) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on が空のため他 task との着手順序制約はない。resource_scope (docs/features/feat-post-signin-scope-routing/requirements-baseline.md) が他 task の active lease と重複しないことを確認する
...
- Web 公開ウィザードの導線 (owner=feat-web-only-publish-journey)
...
- Required evidence: docs/features/feat-post-signin-scope-routing/requirements-baseline.md に goal-spec acceptance 8 件と quality_constraints 6 件が過不足なく転記されていること
...
- Rollback trigger and steps: goal-spec と features/feat-post-signin-scope-routing.md の内容不一致が判明した場合、requirements-baseline.md を作成前状態 (ファイル未作成) に戻し、goal-spec 側の再確定を dev-graph へ差し戻す
...
- Phase responsibility: 現行 context の purpose・goal・scope・acceptance のうち P01 の責務に対応する部分を扱う。
