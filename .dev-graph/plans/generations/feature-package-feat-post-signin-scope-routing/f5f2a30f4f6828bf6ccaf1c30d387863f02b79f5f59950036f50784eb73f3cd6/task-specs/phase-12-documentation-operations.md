# System task overlay: ドキュメントと運用 — 画面遷移仕様の更新と scope 未解決時の運用手順整備
...
- phase_ref: P12
- classification: confidence=0.92, reason="goal-spec.json を入力に P12 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-post-signin-scope-routing/sys-post-signin-scope-p12.md}]
...
サインイン後の遷移と scope 解決の実装内容を確定仕様文書へ反映し、scope 未解決時の一次切り分け手順を運用 runbook として整備する。
...
docs/frontend-spec.md はダッシュボード完成前の / を /sheets へ移動させる規定を既に持つが、実装がそれに追従していなかったのが本 feature の出発点である。同じ乖離を再発させないため、本 task では実装後の挙動を仕様側へ書き戻し、両者の一致を確認する。docs/user-journeys.md のサインイン後導線も同様に更新する。運用面では、利用者が業務画面へ到達できない事象が報告されたときの一次切り分け (未認証か / scope 未解決か / 所属なしか) を runbook として整備する。本 feature は authorize() の判定規則そのものを所有しない (owner=feat-auth-tenancy)。業務画面本体も所有しない (owner=feat-dual-catalog-web)。Workspace 選択画面の UI は feat-workspace-switch-ux、Web 公開ウィザードの導線は feat-web-only-publish-journey が所有する。本 feature が所有するのは scope 解決の入力系統と着地先解決の結線に限られる。
...
- Entry gate: 直前 task SYS-POST-SIGNIN-SCOPE-P11 が done または closed であること
- Source pin: system-spec-harness v0.1.0 / run-system-spec-compile / assign-system-spec-completeness-evaluator
- Repository context: repo_identity=github:daishiman/HarnessHub、root_resolution_source=explicit-cli (validate-system-plan.py 実行時に --repo-root を明示指定する運用)、config=.dev-graph/config.json。全 path は repository 相対とし absolute path は使用しない

## Workstream applicability

- Frontend: applicable: docs/frontend-spec.md のサインイン後遷移の記述を実装と一致させる
- Backend: applicable: scope 解決の入力系統に関する記述を実装と一致させる
- API: applicable: authorize() の reason 一覧 (missing_tenant_scope / ambiguous_scope / tenant_mismatch / workspace_not_member) の記述を実装と一致させる
- Data: N/A: データモデル変更を伴わないため文書更新対象がない
- Infrastructure: N/A: インフラ変更を伴わないため文書更新対象がない
- Security: applicable: open redirect 防止規則を文書へ明記する
- Quality: applicable: 文書と実装の一致を確認する
- Documentation: applicable + change: docs/frontend-spec.md と docs/user-journeys.md を更新し、docs/features/feat-post-signin-scope-routing/operations-runbook.md を新規作成する
- Operations: applicable + change: scope 未解決時の一次切り分け手順を runbook として整備する
...
- Produced artifacts: docs/frontend-spec.md と docs/user-journeys.md の更新、および docs/features/feat-post-signin-scope-routing/operations-runbook.md (scope 未解決時の一次切り分け手順)
...
- Write scope/touches: docs/frontend-spec.md, docs/user-journeys.md, docs/features/feat-post-signin-scope-routing/operations-runbook.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-post-signin-scope-p12) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-post-signin-scope-p12 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-post-signin-scope-p12) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on (SYS-POST-SIGNIN-SCOPE-P11) の完了後に着手する。resource_scope (docs/frontend-spec.md, docs/user-journeys.md, docs/features/feat-post-signin-scope-routing/operations-runbook.md) が他 task の active lease と重複しないことを確認する
- Completion projection: feature branch 上の完了は pending event として記録され、default branch (main) へのクリーンな reconciliation で durable done へ確定する

## スコープ外

- 確定 spec と architecture の正本への書き戻し (owner=P13)
- 本体実装の変更 (owner=P05)
- 本番反映 (owner=P13)
...
- Required evidence: docs/frontend-spec.md と docs/user-journeys.md のサインイン後遷移の記述が実装と一致し、operations-runbook.md に未認証・scope 未解決・所属なしの 3 分岐の切り分け手順が記録されていること
...
- Rollout: 文書更新後に実装との一致を確認してから P13 へ引き継ぐ
- Rollback trigger and steps: 文書と実装の不一致が残る場合は該当箇所を修正し、本 task を再実行する
...
- Dependencies: SYS-POST-SIGNIN-SCOPE-P11
