# System task overlay: アーキテクチャ決定 — scope 解決の単一合流点・session への active workspace 束縛・着地先解決関数の配置と契約確定
...
- phase_ref: P02
- classification: confidence=0.92, reason="goal-spec.json を入力に P02 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-post-signin-scope-routing/sys-post-signin-scope-p02.md}]
...
scope 解決 2 系統の合流点・session への active workspace 束縛・サインイン後着地先解決関数の配置と入出力契約を architecture decision として確定し、P05 の実装が判断を持ち越さずに着手できる状態にする。
...
既存の authorize() は resolveRequestedScope で path とヘッダーから scope を解決し、解決不能なら ambiguous_scope、tenantId が null なら missing_tenant_scope を返す構造を持つ。本 task はこの構造を保ったまま session 由来の scope を第 2 の入力として合流させる位置を決める。合流点を authorize() の外側 (scope 解決関数の内部) に置くことで、判定順の変更を伴わずに入力だけを増やせる。本 feature は authorize() の判定規則そのものを所有しない (owner=feat-auth-tenancy)。業務画面本体も所有しない (owner=feat-dual-catalog-web)。Workspace 選択画面の UI は feat-workspace-switch-ux、Web 公開ウィザードの導線は feat-web-only-publish-journey が所有する。本 feature が所有するのは scope 解決の入力系統と着地先解決の結線に限られる。
...
- Entry gate: 直前 task SYS-POST-SIGNIN-SCOPE-P01 が done または closed であること
- Source pin: system-spec-harness v0.1.0 / run-system-spec-compile / assign-system-spec-completeness-evaluator
- Repository context: repo_identity=github:daishiman/HarnessHub、root_resolution_source=explicit-cli (validate-system-plan.py 実行時に --repo-root を明示指定する運用)、config=.dev-graph/config.json。全 path は repository 相対とし absolute path は使用しない

## Workstream applicability

- Frontend: applicable: 着地先解決関数の配置 (apps/hub/src/lib/routing 配下) と、サインインフォーム・トップページからの参照方法を決定する。実装は P05 が行う
- Backend: applicable: session から active tenant-workspace を読み出す経路と、所属再検証の呼び出し位置を決定する。実装は P05 が行う
- API: applicable: scope 解決関数の入出力契約 (入力 = path・ヘッダー・session principal、出力 = 解決済み scope または ambiguous 判定) を決定する
- Data: N/A: 既存 session と principal の保持情報のみを用い、新規テーブル・列を設けない
- Infrastructure: N/A: デプロイ単位は既存の cloudflare-workers/hub を変更しない
- Security: applicable + change: 判定順と deny-by-default を変えない設計であること、redirect が認可の迂回路にならないこと、session 保持値を所属検証の代替に使わないことを決定として明記する
- Quality: applicable: 決定ごとに P04 で検証可能な形 (入力と期待結果が一意に定まる形) で記述する
- Documentation: applicable + change: docs/features/feat-post-signin-scope-routing/architecture-decision.md を新規作成する
...
- Produced artifacts: docs/features/feat-post-signin-scope-routing/architecture-decision.md (scope 解決の単一合流点・session への active workspace 束縛・着地先解決関数の配置と入出力契約・既定着地の単一定数化を決定として記録した文書)
...
- Write scope/touches: docs/features/feat-post-signin-scope-routing/architecture-decision.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-post-signin-scope-p02) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-post-signin-scope-p02 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-post-signin-scope-p02) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on (SYS-POST-SIGNIN-SCOPE-P01) の完了後に着手する。resource_scope (docs/features/feat-post-signin-scope-routing/architecture-decision.md) が他 task の active lease と重複しないことを確認する
...
- 実装コードの作成 (owner=P05)
- Workspace 選択画面の UI 設計 (owner=feat-workspace-switch-ux)
...
- Required evidence: docs/features/feat-post-signin-scope-routing/architecture-decision.md に、scope 解決の合流点・着地先解決関数の入出力契約・既定着地の単一定数化・同一 origin 相対 path 制限・所属再検証のタイミングの 5 決定が、それぞれ根拠と却下案つきで記録されていること
...
- Rollout: architecture-decision.md を作成し、P01 の requirements-baseline.md と矛盾しないことを確認してから P03 へ引き継ぐ
- Rollback trigger and steps: 決定が P01 の要件と矛盾する場合、architecture-decision.md を作成前状態へ戻し、P01 の baseline との突合をやり直す
...
- Phase responsibility: 現行 context の purpose・goal・scope・acceptance のうち P02 の責務に対応する部分を扱う。
...
- Dependencies: SYS-POST-SIGNIN-SCOPE-P01
