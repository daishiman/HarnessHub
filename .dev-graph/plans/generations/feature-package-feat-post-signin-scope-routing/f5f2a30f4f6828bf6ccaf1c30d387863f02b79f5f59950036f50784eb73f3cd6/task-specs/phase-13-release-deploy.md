# System task overlay: リリースとデプロイ — 本番反映と確定仕様・アーキテクチャへの書き戻し

> QA trace: `qa-134` (frontend 着地先・scope 伝搬), `qa-135` (Web 完結 UX), `qa-136` (認可境界)。
...
- phase_ref: P13
- classification: confidence=0.92, reason="goal-spec.json を入力に P13 の単一責務 (infrastructure) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-post-signin-scope-routing/sys-post-signin-scope-p13.md}]
...
- Entry gate: 直前 task SYS-POST-SIGNIN-SCOPE-P12 が done または closed であること
- Source pin: system-spec-harness v0.1.0 / run-system-spec-compile / assign-system-spec-completeness-evaluator
- Repository context: repo_identity=github:daishiman/HarnessHub、root_resolution_source=explicit-cli (validate-system-plan.py 実行時に --repo-root を明示指定する運用)、config=.dev-graph/config.json。全 path は repository 相対とし absolute path は使用しない

## Workstream applicability

- Frontend: applicable: 本番反映後に業務画面 6 種への到達を実測する
- Backend: applicable: 本番反映後に scope 解決と所属検証の挙動を実測する
- API: applicable: 本番反映後に authorize() の reason 分布を確認する
- Data: N/A: データモデル変更を伴わないため移行・backfill の対象がない
- Infrastructure: applicable + change: cloudflare-workers/hub へのデプロイと smoke 確認を行う
- Security: applicable: 本番で open redirect が成立しないことと deny-by-default が維持されていることを実測する
- Quality: applicable: 実測結果を release-record.md へ記録する
- Documentation: applicable + change: architecture/harness-hub-frontend.md・architecture/harness-hub-security.md・specs/harness-hub-post-signin-workspace-scope-addendum.md へ書き戻す
- Operations: applicable: 反映後の監視項目と rollback 手順を確認する

## Architecture and deploy unit

- Architecture decisions: arch-harness-hub-frontend, arch-harness-hub-security (features/feat-post-signin-scope-routing.md architecture_refs の正本参照。specs/harness-hub-post-signin-workspace-scope-addendum.md A 節と B 節、qa-134 と qa-136 を含む)
- Deploy unit/environment: cloudflare-workers/hub (apps/hub は Hub Worker にバンドルされる。本 task で本番反映を行う)
- Compatibility/migration/backfill: N/A: schema 変更と backfill を伴わない。旧挙動 (missing_tenant_scope) は既定拒否側であるため、rollback による権限漏れは生じない

## 成果物

- Produced artifacts: docs/features/feat-post-signin-scope-routing/release-record.md (本番実測結果と書き戻し照合記録)、および architecture/harness-hub-frontend.md・architecture/harness-hub-security.md・specs/harness-hub-post-signin-workspace-scope-addendum.md の更新
...
- Write scope/touches: docs/features/feat-post-signin-scope-routing/release-record.md, architecture/harness-hub-frontend.md, architecture/harness-hub-security.md, specs/harness-hub-post-signin-workspace-scope-addendum.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-post-signin-scope-p13) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-post-signin-scope-p13 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-post-signin-scope-p13) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on (SYS-POST-SIGNIN-SCOPE-P12) の完了後に着手する。resource_scope (docs/features/feat-post-signin-scope-routing/release-record.md, architecture/harness-hub-frontend.md, architecture/harness-hub-security.md, specs/harness-hub-post-signin-workspace-scope-addendum.md) が他 task の active lease と重複しないことを確認する
- Completion projection: feature branch 上の完了は pending event として記録され、default branch (main) へのクリーンな reconciliation で durable done へ確定する

## スコープ外

- 新規機能の追加
- authorize() の判定順・role 判定の変更 (owner=feat-auth-tenancy)
...
- Required evidence: docs/features/feat-post-signin-scope-routing/release-record.md に業務画面 6 種の本番到達実測結果が記録され、書き戻し 3 ファイルそれぞれについて反映内容または意味変更なしの照合結果が記録されていること
...
- P13 spec/architecture writeback: required。実行結果・判断・改善知見を docs/features/feat-post-signin-scope-routing/release-record.md へ証跡化し、本番実測で確定した scope 解決 2 系統の合流点・着地先解決の既定値・open redirect 防止規則を architecture/harness-hub-frontend.md・architecture/harness-hub-security.md・specs/harness-hub-post-signin-workspace-scope-addendum.md の正本へ反映する。意味変更がない項目も照合結果として release-record.md に記録する。

## Rollout and rollback

- Rollout: cloudflare-workers/hub へデプロイし、業務画面 6 種の到達と deny-by-default 維持を実測してから完了とする
- Rollback trigger and steps: 本番で deny-by-default 退行または open redirect 成立が確認された場合、直前の deploy へ即時 rollback し、P05 へ差し戻す
...
- Phase responsibility: 現行 context の purpose・goal・scope・acceptance のうち P13 の責務に対応する部分を扱う。
...
- System specification: specs/harness-hub-post-signin-workspace-scope-addendum.md (A 節から G 節), system-spec/spec-state.json qa_log (qa-134, qa-135, qa-136)
...
- Dependencies: SYS-POST-SIGNIN-SCOPE-P12
