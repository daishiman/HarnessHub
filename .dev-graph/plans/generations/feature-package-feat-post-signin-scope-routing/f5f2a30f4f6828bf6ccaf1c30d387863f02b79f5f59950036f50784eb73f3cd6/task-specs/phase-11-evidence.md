# System task overlay: 証跡固定 — source digest と再実行コマンドの保存
...
- phase_ref: P11
- classification: confidence=0.92, reason="goal-spec.json を入力に P11 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-post-signin-scope-routing/sys-post-signin-scope-p11.md}]
...
本 feature package の source digest と、P06・P09 の再実行コマンドを再現可能な形で保存する。
...
- Entry gate: 直前 task SYS-POST-SIGNIN-SCOPE-P10 が done または closed であること
- Source pin: system-spec-harness v0.1.0 / run-system-spec-compile / assign-system-spec-completeness-evaluator
- Repository context: repo_identity=github:daishiman/HarnessHub、root_resolution_source=explicit-cli (validate-system-plan.py 実行時に --repo-root を明示指定する運用)、config=.dev-graph/config.json。全 path は repository 相対とし absolute path は使用しない

## Workstream applicability

- Frontend: N/A: 本 task は証跡固定のみで frontend 実装物を変更しない
- Backend: N/A: 本 task は証跡固定のみで backend 実装物を変更しない
- API: N/A: 本 task は証跡固定のみで API 契約を変更しない
- Data: N/A: データモデル変更を伴わない
- Infrastructure: N/A: インフラ変更を伴わない
- Security: applicable: 証跡に秘密情報 (token・cookie 値・session 識別子) を含めないことを確認する
- Quality: applicable + change: 再実行コマンドが実際に再実行可能であることを確認する
- Documentation: applicable + change: docs/features/feat-post-signin-scope-routing/evidence-record.md を新規作成する
...
- Produced artifacts: docs/features/feat-post-signin-scope-routing/evidence-record.md (source digest・再実行コマンド・証跡参照先の一覧)
...
- Write scope/touches: docs/features/feat-post-signin-scope-routing/evidence-record.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-post-signin-scope-p11) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-post-signin-scope-p11 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-post-signin-scope-p11) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on (SYS-POST-SIGNIN-SCOPE-P10) の完了後に着手する。resource_scope (docs/features/feat-post-signin-scope-routing/evidence-record.md) が他 task の active lease と重複しないことを確認する
...
- Required evidence: docs/features/feat-post-signin-scope-routing/evidence-record.md に source digest と再実行コマンドが記録され、参照先がリポジトリ相対 path であり絶対 path を含まないこと
...
- Rollout: 証跡を固定してから P12 へ引き継ぐ
- Rollback trigger and steps: 再実行コマンドが再現しない場合は記録を修正し、本 task を再実行する
...
- Phase responsibility: 現行 context の purpose・goal・scope・acceptance のうち P11 の責務に対応する部分を扱う。
...
- Dependencies: SYS-POST-SIGNIN-SCOPE-P10
