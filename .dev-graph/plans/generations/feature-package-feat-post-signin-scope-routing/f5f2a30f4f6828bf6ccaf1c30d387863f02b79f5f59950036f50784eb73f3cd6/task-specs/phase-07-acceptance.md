# System task overlay: 受入判定 — goal-spec acceptance 8 件の実測証跡による判定
...
- owners: ["daishiman"]
- tags: ["feat-post-signin-scope-routing", "macro-feature", "quality", "phase-p07"]
...
- phase_ref: P07
- classification: confidence=0.92, reason="goal-spec.json を入力に P07 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-post-signin-scope-routing/sys-post-signin-scope-p07.md}]
...
goal-spec acceptance 8 件を、P06 が実行した証跡だけを根拠に判定する。
...
受入判定は実行済み証跡のみを根拠にする。計画中の作業・未実行のテスト・文書上の宣言を根拠に PASS としない。acceptance の中でも「業務画面 6 種が通常のブラウザ操作で 403 missing_tenant_scope にならない」は、単体テストだけでは成立を示せないため、結合レベルの実測証跡を要求する。本 feature は authorize() の判定規則そのものを所有しない (owner=feat-auth-tenancy)。業務画面本体も所有しない (owner=feat-dual-catalog-web)。Workspace 選択画面の UI は feat-workspace-switch-ux、Web 公開ウィザードの導線は feat-web-only-publish-journey が所有する。本 feature が所有するのは scope 解決の入力系統と着地先解決の結線に限られる。
...
- Entry gate: 直前 task SYS-POST-SIGNIN-SCOPE-P06 が done または closed であること
- Source pin: system-spec-harness v0.1.0 / run-system-spec-compile / assign-system-spec-completeness-evaluator
- Repository context: repo_identity=github:daishiman/HarnessHub、root_resolution_source=explicit-cli (validate-system-plan.py 実行時に --repo-root を明示指定する運用)、config=.dev-graph/config.json。全 path は repository 相対とし absolute path は使用しない

## Workstream applicability

- Frontend: applicable: 着地先解決に関する acceptance を実測証跡で判定する
- Backend: applicable: scope 解決と所属検証に関する acceptance を実測証跡で判定する
- API: applicable: authorize() の応答に関する acceptance を実測証跡で判定する
- Data: N/A: データモデル変更を伴わないため判定対象がない
- Infrastructure: N/A: インフラ変更を伴わないため判定対象がない
- Security: applicable: open redirect 防止と deny-by-default 非退行の acceptance を実測証跡で判定する
- Quality: applicable + change: acceptance 8 件と証跡の対応表を作り、根拠のない PASS を 0 にする
- Documentation: applicable + change: docs/features/feat-post-signin-scope-routing/acceptance-record.md を新規作成する
...
- Produced artifacts: docs/features/feat-post-signin-scope-routing/acceptance-record.md (acceptance 8 件の判定結果と根拠証跡の対応表)
...
- Write scope/touches: docs/features/feat-post-signin-scope-routing/acceptance-record.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-post-signin-scope-p07) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-post-signin-scope-p07 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-post-signin-scope-p07) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on (SYS-POST-SIGNIN-SCOPE-P06) の完了後に着手する。resource_scope (docs/features/feat-post-signin-scope-routing/acceptance-record.md) が他 task の active lease と重複しないことを確認する
...
- Required evidence: docs/features/feat-post-signin-scope-routing/acceptance-record.md に acceptance 8 件それぞれの判定と、根拠となる実行済みテスト ID・実測結果が記録されていること
...
- Rollout: acceptance 全件 PASS を確認してから P08 へ引き継ぐ
- Rollback trigger and steps: FAIL がある場合は該当 acceptance の owner phase へ差し戻し、修正後に本 task を再実行する
...
- Phase responsibility: 現行 context の purpose・goal・scope・acceptance のうち P07 の責務に対応する部分を扱う。
...
- Dependencies: SYS-POST-SIGNIN-SCOPE-P06
