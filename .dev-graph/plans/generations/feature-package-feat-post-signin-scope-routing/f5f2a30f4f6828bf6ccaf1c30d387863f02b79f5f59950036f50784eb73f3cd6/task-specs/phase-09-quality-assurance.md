# System task overlay: 品質保証 — deny-by-default 非退行・open redirect 防止・所属検証の fail-closed 検査
...
- phase_ref: P09
- classification: confidence=0.92, reason="goal-spec.json を入力に P09 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-post-signin-scope-routing/sys-post-signin-scope-p09.md}]
...
deny-by-default 非退行・open redirect 防止・所属検証の 3 検査を fail-closed の品質ゲートとして成立させ、検査を無効化した変異版でゲートが反転することまで確認する。
...
品質ゲートは「通ること」ではなく「壊れたときに落ちること」で価値が決まる。たとえば missing_tenant_scope の分岐が到達不能になっても、正常系テストだけでは緑のままになりうる。そこで本 task は、検査対象の実装を意図的に壊した変異版を用意し、ゲートが実際に FAIL へ反転することを確認する。反転しない検査は、検査として成立していないものとして作り直す。本 feature は authorize() の判定規則そのものを所有しない (owner=feat-auth-tenancy)。業務画面本体も所有しない (owner=feat-dual-catalog-web)。Workspace 選択画面の UI は feat-workspace-switch-ux、Web 公開ウィザードの導線は feat-web-only-publish-journey が所有する。本 feature が所有するのは scope 解決の入力系統と着地先解決の結線に限られる。
...
- Entry gate: 直前 task SYS-POST-SIGNIN-SCOPE-P08 が done または closed であること
- Source pin: system-spec-harness v0.1.0 / run-system-spec-compile / assign-system-spec-completeness-evaluator
- Repository context: repo_identity=github:daishiman/HarnessHub、root_resolution_source=explicit-cli (validate-system-plan.py 実行時に --repo-root を明示指定する運用)、config=.dev-graph/config.json。全 path は repository 相対とし absolute path は使用しない

## Workstream applicability

- Frontend: applicable: 着地先解決の検査が変異版で反転することを確認する
- Backend: applicable: scope 解決と所属検証の検査が変異版で反転することを確認する
- API: applicable: authorize() の reason と status の検査が変異版で反転することを確認する
- Data: N/A: データモデル変更を伴わないため検査対象がない
- Infrastructure: N/A: インフラ変更を伴わないため IaC と smoke の対象がない
- Security: applicable + change: deny-by-default 非退行・open redirect 防止・所属検証の 3 検査を fail-closed 化する
- Quality: applicable + change: 各検査の変異版反転結果を記録し、反転しない検査を 0 にする
- Documentation: applicable + change: docs/features/feat-post-signin-scope-routing/quality-assurance-record.md を新規作成する
...
- Produced artifacts: docs/features/feat-post-signin-scope-routing/quality-assurance-record.md (3 検査の fail-closed 化と変異版反転の確認記録)
...
- Write scope/touches: docs/features/feat-post-signin-scope-routing/quality-assurance-record.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-post-signin-scope-p09) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-post-signin-scope-p09 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-post-signin-scope-p09) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on (SYS-POST-SIGNIN-SCOPE-P08) の完了後に着手する。resource_scope (docs/features/feat-post-signin-scope-routing/quality-assurance-record.md) が他 task の active lease と重複しないことを確認する
...
- Required evidence: docs/features/feat-post-signin-scope-routing/quality-assurance-record.md に 3 検査それぞれの変異内容と、変異版で検査が FAIL へ反転した実測結果が記録されていること
...
- Rollout: 3 検査すべてが変異版で反転することを確認してから P10 へ引き継ぐ
- Rollback trigger and steps: 反転しない検査がある場合は検査を作り直し、本 task を再実行する
...
- Phase responsibility: 現行 context の purpose・goal・scope・acceptance のうち P09 の責務に対応する部分を扱う。
...
- Dependencies: SYS-POST-SIGNIN-SCOPE-P08
