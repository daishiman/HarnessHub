...
- Compatibility/migration/backfill: P08のMetrics専用delta/backfillをBuild releaseから独立してexpand-first適用し、0008不変SHA、backfill件数・旧新reader互換・forward-fix rollbackを証跡化する

## 成果物

- Produced artifacts: docs/features/feat-metrics-tracking/release-record.md (本番反映内容・スモークテスト結果・cron初回実行・正本書き戻し結果)、system-spec/frontend.md、system-spec/backend.md、system-spec/database.md、architecture/harness-hub-frontend.md、architecture/harness-hub-backend.md、architecture/harness-hub-data.md (本番実測で確定した内容の反映)
- Consumed artifacts: docs/features/feat-metrics-tracking/runbook.md, docs/features/feat-metrics-tracking/final-review-record.md, packages/db/migrations/ 配下で0008より後のMetrics専用delta SQL, 対応する packages/db/migrations/meta/ のsnapshotと packages/db/migrations/meta/_journal.json
- Write scope/touches: docs/features/feat-metrics-tracking/release-record.md, system-spec/frontend.md, system-spec/backend.md, system-spec/database.md, architecture/harness-hub-frontend.md, architecture/harness-hub-backend.md, architecture/harness-hub-data.md
...
- Automated commands: `pnpm --filter @harness-hub/db migrate:deploy`, `pnpm --filter @harness-hub/hub deploy`, `python3 plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-metrics-tracking`
- Required evidence: release-record.md にresolver越境/未解決fail-closed、ingest replay/422、Turso全次元transaction failure injection、D1 zero-write、日次+週次cron、`insufficient_history`・`zero_median`・`anomaly=false`の別状態、notification冪等、完了率/利用率snapshotと分母0、`/dashboard`・`/tracking`、SVG/table同値、0008不変、Metrics単独delta/backfill/forward-fix rollbackを実測PASSとして記録する。計画・文書・Build同時releaseを代替証跡にしない。
...
- P13 spec/architecture writeback: required。実行結果・判断・改善知見を docs/features/feat-metrics-tracking/ のP13成果物へ証跡化し、確定した内容を system-spec/ と architecture/ の関連正本へ反映する。意味変更がない場合も照合結果を記録する。

## Rollout and rollback

- Rollout: マイグレーション適用後Hub Workerへ本番デプロイし、スモークテスト全項目passを確認する。実行結果・判断・改善知見を関連するsystem-spec/とarchitecture/の正本へ書き戻し、照合結果をrelease-record.mdへ記録する
- Rollback trigger and steps: スモークテストがfailした場合は機能flagとWorkerを直前版へ戻す。適用済み0008・Metrics delta・journal履歴は変更または取り消さず、必要な互換性修復を追加のMetrics専用forward-fix deltaとしてP05/P08へ差し戻し、release-record.mdへ原因・影響・復旧証跡を記録する
...
This section is normative for P13 and supersedes every older statement in this task that conflicts with `appr-043` or `qa-226`〜`qa-229`. Metrics単独releaseでmigration適用・smoke・rollback evidence・正本writebackを完了する。
