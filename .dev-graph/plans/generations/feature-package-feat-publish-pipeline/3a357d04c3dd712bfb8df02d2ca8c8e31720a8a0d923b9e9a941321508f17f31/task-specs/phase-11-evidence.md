...
- Quality: applicable + change: quality_constraints 9 件それぞれの再現手順を集約する
...
- Produced artifacts: docs/features/feat-publish-pipeline/evidence-summary.md (P06/P07/P09/P10 の証跡集約、quality_constraints 9 件それぞれの再現コマンド列を含む)
...
- Required evidence: docs/features/feat-publish-pipeline/evidence-summary.md に quality_constraints 9 件それぞれの再現コマンド列と対応する結果が記載されていること. Normative evidence: session/Bearer parity、session CSRF欠落403、Bearerでcookie/CSRF非依存、cross-tenant拒否、role matrix、状態遷移/property test、P12 runbookとP13 smokeを必須とする。
