...
- phase_ref: P06
- classification: confidence=0.86, reason="P05 の実装に対して P04 のテストを実行し quality_constraints 9 件の充足状況を機械的に確認する P06 テスト実行タスク", candidates=[{artifact_kind: task, confidence: 0.86, candidate_path: tasks/feat-publish-pipeline/sys-publish-pipeline-p06.md}]
...
P05 で実装した状態機械・検査 pipeline・REST endpoint・promote/rollback・監査 event 記録に対して、P04 で設計した 8 系統のテストケース (状態機械 property test、検査 pipeline 挙動同値テスト、TargetChannel 直列化並行性テスト、R2 冪等性テスト、監査 hash chain 一貫性テスト、REST 契約テスト、rollback 2 版目以降限定検査テスト、dual principal/CSRF 境界テスト) を実行し、結果を test-run-results.md として記録する。

## 背景

本 task は quality_constraints 9 件それぞれについて pass/fail を機械的に確認する。特に状態機械 property test は publish-request-state-machine-section7-2-property-test-qa009 の直接的な検証手段であり、docs/backend-spec.md §5.1 の遷移図に許可されていない全ての遷移候補が拒否されることを確認する。検査 pipeline 挙動同値テストは inspection-pipeline-shared-pure-function-package-qa010-qa020 の検証手段であり、Python 資産と TypeScript 移植版の出力差分が 0 件であることを確認する。TargetChannel 直列化並行性テストは targetchannel-serialization-single-inflight-publishrequest の検証手段であり、同一 TargetChannel に別の非終端 request がある状態で後続 Draft を submit すると 409 `channel_busy` を返すことを、先読みと partial UNIQUE 競合の両経路で確認する。監査 hash chain 一貫性テストは append-only-audit-event-all-publish-operations の検証手段であり、全操作後に `seq`/`prev_hash`/`event_hash` の連鎖が破綻していないことを確認する。いずれかのテストが fail した場合は P05 (実装) へ差し戻す。

9 件目の publish-api-dual-principal-csrf-boundary-qa059 は、session/Bearer parity、session state-changing の Origin/CSRF 欠落 403、Bearer の cookie/CSRF 非依存、invalid Bearer の cookie fallback 禁止、approve/cancel/deployment の endpoint 別 principal/role、cross-tenant/owner 拒否を route/authz テストで直接確認する。
...
- Backend: applicable + change: apps/hub/tests/publish-pipeline/ のテストを実行する
...
- Quality: applicable + change: quality_constraints 9 件全ての pass/fail を確認する
...
- Produced artifacts: docs/features/feat-publish-pipeline/test-run-results.md (quality_constraints 9 件それぞれの pass/fail 結果と証跡を含む)
- Consumed artifacts: apps/hub/src/lib/publish/, apps/hub/tests/publish-pipeline/, packages/inspection/src/, packages/db/__tests__/, docs/features/feat-publish-pipeline/test-design.md
- Write scope/touches: docs/features/feat-publish-pipeline/test-run-results.md, apps/hub/tests/publish-pipeline/ (テスト結果の記録に限る)
...
- Automated commands:
  1. `pnpm --filter hub exec vitest run tests/publish-pipeline`
  2. `pnpm --filter @harness-hub/inspection test`
  3. `pnpm --filter @harness-hub/db test`
  4. `node apps/hub/scripts/check-publish-inspection-gate.mjs`
  5. `node apps/hub/scripts/check-db-schema-boundary.mjs`
...
- Expected exit: 全コマンドが exit 0。Hub の publish suite は状態機械・dual principal/CSRF・直列化・レート制限・Idempotency-Key を、inspection/DB suite は secret scan・R2 参照・監査 hash chain を含めて検証する。
- Required evidence: docs/features/feat-publish-pipeline/test-run-results.md に quality_constraints 9 件全ての pass 結果が記録されている (fail が残る場合は差し戻し理由が明記されている). Normative evidence: session/Bearer parity、session CSRF欠落403、Bearerでcookie/CSRF非依存、invalid Bearer の cookie fallback 禁止、cross-tenant拒否、role matrix、状態遷移/property test、P12 runbookとP13 smokeを必須とする。
