# System task overlay: ドキュメントと運用 — 稼働ビルドの確認手順と鮮度警告時の対応手順
...
- phase_ref: P12
- classification: confidence=0.92, reason="goal-spec.json を入力に P12 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-build-identity-deploy-freshness/sys-build-identity-p12.md}]
...
『本番が古いのでは』と疑ったときに誰でも 1 回の操作で確認できる手順と、鮮度警告が出たときの対応手順を運用文書として残す。
...
- Infrastructure: applicable + no change: 運用手順を記述する (構成は変更しない)
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: N/A: 本 task では検査項目を変更しない
- Documentation: applicable + change: operations.md を新規作成する
- Operations: applicable + change: 稼働ビルド確認と鮮度警告対応の手順を整備する
...
- Produced artifacts: docs/features/feat-build-identity-deploy-freshness/operations.md (稼働ビルド確認手順・鮮度警告時の対応手順・誤検出時の扱い)
...
- Write scope/touches: docs/features/feat-build-identity-deploy-freshness/operations.md, docs
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-build-identity-p12) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-build-identity-p12 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-build-identity-p12) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-BUILD-IDENTITY-P11 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-build-identity-deploy-freshness/operations.md, docs) が他 task の active lease と重複しないことを確認する
...
- 稼働ビルドの commit を確認する手順が、認証情報を必要としない形で書かれている
- 鮮度警告が出たときの対応手順 (再デプロイの判断・誤検出時の扱い) が書かれている
- 手順が実際のコマンド/URL を含み、読み手が読み替えを要求されない
...
- Rollout: operations.md を作成し P13 へ引き継ぐ
- Rollback trigger and steps: 手順が実機で再現しない場合、operations.md を差し戻して実測で書き直す
...
- Dependencies: SYS-BUILD-IDENTITY-P11
