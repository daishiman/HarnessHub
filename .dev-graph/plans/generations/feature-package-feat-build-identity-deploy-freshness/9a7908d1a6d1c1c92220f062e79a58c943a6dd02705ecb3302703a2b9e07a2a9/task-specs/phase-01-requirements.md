# System task overlay: 要件ベースライン確定 — 稼働ビルドの素性確認 (V6) と deploy 反映鮮度検出 (V7) の要件確定
...
- owners: ["daishiman"]
- tags: ["feat-build-identity-deploy-freshness", "macro-feature", "documentation", "phase-p01"]
...
- phase_ref: P01
- classification: confidence=0.92, reason="goal-spec.json を入力に P01 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-build-identity-deploy-freshness/sys-build-identity-p01.md}]
...
本 feature の受入可能な要件ベースラインを確定し、P02 以降の全 task が同一の合意事項 (commit 識別子の公開範囲・認証なしで読めること・鮮度検出が『古い状態の継続』を見ること) を参照できる状態にする。
...
- Backend: N/A: 本 task は backend 実装物を変更しない
- API: N/A: 公開する読出経路の契約は P02 が定義し、本 task では変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: applicable + no change: 露出経路の要件を記述するのみで、この task では配信構成を変更しない
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: acceptance 5 件を machine-verifiable な受入基準として baseline に固定する
...
- Produced artifacts: docs/features/feat-build-identity-deploy-freshness/requirements-baseline.md (要件ベースライン文書。purpose/goal/scope_in 4 件/scope_out 3 件/acceptance 5 件の確定転記と、露出してよい情報の範囲・鮮度判定の継続時間しきい値という 2 つの未決事項の P02 必須解消指定を含む)
...
- Write scope/touches: docs/features/feat-build-identity-deploy-freshness/requirements-baseline.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-build-identity-p01) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-build-identity-p01 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-build-identity-p01) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は なし (P01 は本 package の起点 task) であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-build-identity-deploy-freshness/requirements-baseline.md) が他 task の active lease と重複しないことを確認する
...
- docs/features/feat-build-identity-deploy-freshness/requirements-baseline.md に goal-spec.json の purpose/goal/scope_in 4 件/scope_out 3 件/acceptance 5 件が逐語一致で転記されている
- 露出してよい情報の範囲 (commit 識別子まで) と、露出してはならないもの (内部 path・secret・個人データ) が明示されている
- 鮮度検出が『古いこと』ではなく『古い状態が継続していること』を見る要件であることが、実測 (2026-08-03 の修正が 2026-08-07 時点で未反映 = 4 日継続) を根拠に記述されている
...
- Rollback trigger and steps: goal-spec と feature context の内容不一致が判明した場合、requirements-baseline.md を作成前状態 (ファイル未作成) へ戻し、goal-spec 側の再確定を dev-graph へ差し戻す
