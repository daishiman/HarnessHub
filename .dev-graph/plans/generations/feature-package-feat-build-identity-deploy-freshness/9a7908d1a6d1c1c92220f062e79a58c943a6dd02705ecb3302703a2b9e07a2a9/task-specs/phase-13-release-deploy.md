...
- phase_ref: P13
- classification: confidence=0.92, reason="goal-spec.json を入力に P13 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-build-identity-deploy-freshness/sys-build-identity-p13.md}]
...
本 feature の成果をいつでも反映できる状態に確定し、反映後に何を確認するかを運用側へ引き渡したうえで、過程で得た判断と改善点を確定仕様と architecture へ書き戻して次の外側ループへ引き継ぐ。deploy そのものの実行は feature context の scope_out であり、本 task は行わない。利用者の当初の困りごとの是正 (着地先を直した commit 以降の本番反映) も他 feature と運用の担当であり、本 task はその反映を検出できる状態を残すところまでを担う。
...
- Backend: applicable + no change: 反映後に確認する読出経路の contract を記述する (実装は変更しない)
- API: N/A: 公開する読出経路の契約は P02 が定義し、本 task では変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: applicable + no change: 反映手順と確認事項を記述する (deploy の実行は scope 外)
...
- Write scope/touches: docs/features/feat-build-identity-deploy-freshness/release-record.md, specs/harness-hub-post-signin-landing-observability-addendum.md, architecture/harness-hub-infrastructure.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-build-identity-p13) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-build-identity-p13 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-build-identity-p13) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-BUILD-IDENTITY-P12 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-build-identity-deploy-freshness/release-record.md, specs/harness-hub-post-signin-landing-observability-addendum.md, architecture/harness-hub-infrastructure.md) が他 task の active lease と重複しないことを確認する
...
- 反映可否の判定と、反映後に確認すべき事項 (稼働ビルドの commit が既定 branch の HEAD と一致すること) が release-record.md に確定している
- 鮮度検出の継続時間しきい値が、反映直後の一時的な乖離では発火しない値であることが記録されている
- 本 feature で得た判断と改善点が確定仕様と architecture へ書き戻されている
- 書き戻しが既存の確定章を上書きせず、追補として追加されている
...
- P13 spec/architecture writeback: required — 本 task の実行結果・判断・改善点を specs/harness-hub-post-signin-landing-observability-addendum.md と architecture/harness-hub-infrastructure.md へ追補として書き戻し、次の外側ループへ引き継ぐ。

## Rollout and rollback

- Rollout: release-record.md に反映可否と反映後の確認手順を確定し、そのうえで確定仕様と architecture への書き戻しを行う
- Rollback trigger and steps: 書き戻し内容が確定仕様と矛盾する場合は追補を書き戻し前へ戻し、release-record には判断の経緯を残す
...
- Dependencies: SYS-BUILD-IDENTITY-P12
