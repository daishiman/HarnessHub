# System task overlay: テスト設計 — 埋込・公開読出・鮮度検出の実行可能テスト定義
...
- phase_ref: P04
- classification: confidence=0.92, reason="goal-spec.json を入力に P04 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-build-identity-deploy-freshness/sys-build-identity-p04.md}]
...
acceptance 5 件を実行可能なテスト ID へ落とし、特に『しきい値を超えた状態を再現する fixture で検査が実際に落ちる』ことを検証できる形にする。
...
- Security: applicable + no change: 露出範囲の検査項目を定義する
- Quality: applicable + change: テスト ID と acceptance の対応表を定義する
- Documentation: applicable + change: test-design.md を新規作成する
...
- Produced artifacts: docs/features/feat-build-identity-deploy-freshness/test-design.md (テスト ID・入力・期待結果・acceptance との対応表)
...
- Write scope/touches: docs/features/feat-build-identity-deploy-freshness/test-design.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-build-identity-p04) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-build-identity-p04 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-build-identity-p04) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-BUILD-IDENTITY-P03 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-build-identity-deploy-freshness/test-design.md) が他 task の active lease と重複しないことを確認する
...
- acceptance 5 件すべてに対応するテスト ID が定義され、対応が表で示されている
- 鮮度検出については、しきい値超過を再現する fixture で検査が落ちること (発火の確認) がテスト ID として定義されている
- 検査が発火しない状態 (対象が無い・入力が空) と、発火して合格した状態が別のテスト ID として区別されている
...
- Rollout: test-design.md を作成し、P05 の実装がテスト ID を満たす形で進むよう引き継ぐ
- Rollback trigger and steps: acceptance との対応に欠落があれば test-design.md を再作成し、欠落の原因が要件側なら P01 へ差し戻す
...
- Dependencies: SYS-BUILD-IDENTITY-P03
