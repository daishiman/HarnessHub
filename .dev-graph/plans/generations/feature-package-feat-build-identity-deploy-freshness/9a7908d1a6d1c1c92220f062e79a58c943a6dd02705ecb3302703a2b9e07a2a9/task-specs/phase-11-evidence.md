...
- owners: ["daishiman"]
- tags: ["feat-build-identity-deploy-freshness", "macro-feature", "documentation", "phase-p11"]
...
- phase_ref: P11
- classification: confidence=0.92, reason="goal-spec.json を入力に P11 の単一責務 (documentation) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-build-identity-deploy-freshness/sys-build-identity-p11.md}]
...
後から誰でも同じ結果を再現できるよう、digest と再実行コマンドを固定する。
...
- Quality: applicable + no change: 証跡の再現性を確認する
- Documentation: applicable + change: evidence.md を新規作成する
...
- Produced artifacts: docs/features/feat-build-identity-deploy-freshness/evidence.md (各証跡の digest・取得時刻・世代非依存の再実行コマンド)
...
- Write scope/touches: docs/features/feat-build-identity-deploy-freshness/evidence.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-build-identity-p11) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-build-identity-p11 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-build-identity-p11) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-BUILD-IDENTITY-P10 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-build-identity-deploy-freshness/evidence.md) が他 task の active lease と重複しないことを確認する
...
- 各証跡に digest と取得時刻が付いている
- 再実行コマンドが世代非依存であり、staging path のような消滅する参照を含まない
...
- Rollout: evidence.md を作成し P12 へ引き継ぐ
- Rollback trigger and steps: digest が再現しない証跡は evidence から外し、再取得を該当 phase へ差し戻す
...
- Dependencies: SYS-BUILD-IDENTITY-P10
