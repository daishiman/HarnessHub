...
- owners: ["daishiman"]
- tags: ["feat-build-identity-deploy-freshness", "macro-feature", "quality", "phase-p10"]
...
- phase_ref: P10
- classification: confidence=0.92, reason="goal-spec.json を入力に P10 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-build-identity-deploy-freshness/sys-build-identity-p10.md}]
...
P01..P09 の証跡だけを根拠に、本番反映してよいかを判定する。
...
- Quality: applicable + change: 証跡のみでリリース可否を判定する
...
- Produced artifacts: docs/features/feat-build-identity-deploy-freshness/final-review.md (リリース可否と、可否を左右した証跡の列挙)
...
- Write scope/touches: docs/features/feat-build-identity-deploy-freshness/final-review.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-build-identity-p10) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-build-identity-p10 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-build-identity-p10) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-BUILD-IDENTITY-P09 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-build-identity-deploy-freshness/final-review.md) が他 task の active lease と重複しないことを確認する
...
- 判定の根拠がすべて実行済み証跡への参照であり、未実行の予定が根拠に含まれていない
- 未達項目がある場合、それがリリース可否にどう影響するかが明示されている
...
- Rollout: final-review.md を作成し P11 へ引き継ぐ
- Rollback trigger and steps: 判定不能な場合は該当 phase へ差し戻す
...
- Dependencies: SYS-BUILD-IDENTITY-P09
