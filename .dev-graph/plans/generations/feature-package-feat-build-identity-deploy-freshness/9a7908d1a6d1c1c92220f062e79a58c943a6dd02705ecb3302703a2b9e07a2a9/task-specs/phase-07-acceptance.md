# System task overlay: 受入判定 — goal-spec acceptance 5 件の実測証跡による判定
...
- phase_ref: P07
- classification: confidence=0.92, reason="goal-spec.json を入力に P07 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-build-identity-deploy-freshness/sys-build-identity-p07.md}]
...
acceptance 5 件を、実行済みの証跡だけを根拠に判定する。
...
- Infrastructure: N/A: 本 task では配信構成と CI 構成を変更しない
- Security: applicable + no change: 露出範囲の acceptance を判定する
- Quality: applicable + change: acceptance 5 件を証跡で判定する
- Documentation: applicable + change: acceptance-report.md を新規作成する
...
- Produced artifacts: docs/features/feat-build-identity-deploy-freshness/acceptance-report.md (acceptance 5 件ごとの判定と根拠証跡の参照)
...
- Write scope/touches: docs/features/feat-build-identity-deploy-freshness/acceptance-report.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-build-identity-p07) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-build-identity-p07 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-build-identity-p07) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-BUILD-IDENTITY-P06 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-build-identity-deploy-freshness/acceptance-report.md) が他 task の active lease と重複しないことを確認する
...
- acceptance 5 件すべてに判定と根拠証跡が対応している
- 未実測の項目が PASS と書かれていない (未実測は未実測として残る)
- 稼働物の観測を要する acceptance は、既に稼働している成果物への認証なし読出しかローカル稼働で判定し、判定のために本番 deploy を実行しない (deploy は feature context の scope_out)
...
- Rollout: acceptance-report.md を作成し P08 へ引き継ぐ
- Rollback trigger and steps: 未達 acceptance があれば該当 phase へ差し戻し、report には未達として残す
...
- Dependencies: SYS-BUILD-IDENTITY-P06
