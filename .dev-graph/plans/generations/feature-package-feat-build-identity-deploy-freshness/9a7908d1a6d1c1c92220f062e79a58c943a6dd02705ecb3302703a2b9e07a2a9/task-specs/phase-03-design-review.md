# System task overlay: 設計レビュー — 情報露出・deploy 直後の誤検出・CI 依存の 3 リスク検証
...
- phase_ref: P03
- classification: confidence=0.92, reason="goal-spec.json を入力に P03 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-build-identity-deploy-freshness/sys-build-identity-p03.md}]
...
P02 の決定が、認証なし公開に伴う情報露出、deploy 直後の一時的乖離による誤検出、CI 経路への依存という 3 つのリスクに耐えるかを、実装前に検証する。
...
- Infrastructure: applicable + no change: CI 依存の失敗様式を検証する (構成変更はしない)
- Security: applicable + change: 認証なし公開の露出範囲を検証する
- Quality: applicable + change: 3 リスクの検証結果を文書化する
...
- Produced artifacts: docs/features/feat-build-identity-deploy-freshness/design-review.md (3 リスクごとの検証結果と、未解消なら P05 への制約事項)
...
- Write scope/touches: docs/features/feat-build-identity-deploy-freshness/design-review.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-build-identity-p03) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-build-identity-p03 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-build-identity-p03) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-BUILD-IDENTITY-P02 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-build-identity-deploy-freshness/design-review.md) が他 task の active lease と重複しないことを確認する
...
- 認証なし読出経路が露出する情報に、内部 path・secret・個人データが含まれないことが設計上示されている
- deploy 直後の一時的な乖離で検出が発火しないことが、継続時間しきい値の設計から示されている
- CI が動かない状態 (workflow 失敗・skip) でも鮮度検出の欠測が静かに『正常』へ倒れないことが示されている
...
- Rollout: design-review.md を作成し、未解消リスクを P05 の制約として引き継ぐ
- Rollback trigger and steps: リスクが設計で解消できない場合、P02 の決定へ差し戻し architecture-decision.md を再確定する
...
- Dependencies: SYS-BUILD-IDENTITY-P02
