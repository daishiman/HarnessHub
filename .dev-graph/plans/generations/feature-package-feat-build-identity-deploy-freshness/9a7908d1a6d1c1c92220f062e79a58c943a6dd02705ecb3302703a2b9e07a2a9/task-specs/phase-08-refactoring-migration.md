# System task overlay: リファクタリングと移行 — しきい値の単一定数化と既存 check 系列への整列
...
- phase_ref: P08
- classification: confidence=0.92, reason="goal-spec.json を入力に P08 の単一責務 (backend) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-build-identity-deploy-freshness/sys-build-identity-p08.md}]
...
実装が既存の apps/hub/scripts/check-*.mjs 系列と同じ呼出し規約・出力形式に揃い、しきい値が一箇所から解決されることを確かめる。
...
- Backend: applicable + change: 読出経路の実装を整列する
- API: N/A: 公開する読出経路の契約は P02 が定義し、本 task では変更しない
- Data: N/A: 本 feature は新規テーブルと永続データを設けない
- Infrastructure: applicable + change: 検査 script を既存系列へ揃える
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: 回帰 0 件を確認する
...
- Produced artifacts: 既存 check 系列と整合した鮮度検査 script、単一箇所から解決されるしきい値
...
- Write scope/touches: apps/hub/scripts/check-deploy-freshness.mjs, apps/hub/src/app/api/health
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-build-identity-p08) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-build-identity-p08 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-build-identity-p08) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-BUILD-IDENTITY-P07 であり、直前 phase の完了後にのみ着手する。resource_scope (apps/hub/scripts/check-deploy-freshness.mjs, apps/hub/src/app/api/health) が他 task の active lease と重複しないことを確認する
...
- しきい値が単一箇所から解決され、判定ロジック側に literal で散らばっていない
- 鮮度検査 script の呼出し規約と出力形式が既存 check-*.mjs 系列と揃っている
- P06 のテスト ID が引き続き全件通る (回帰 0 件)
...
- Rollout: 整列を行い P06 のテストを再実行して回帰が無いことを確認する
- Rollback trigger and steps: 回帰が出た場合は整列前へ戻し、P06 のテスト結果を正本として再検討する
...
- Dependencies: SYS-BUILD-IDENTITY-P07
