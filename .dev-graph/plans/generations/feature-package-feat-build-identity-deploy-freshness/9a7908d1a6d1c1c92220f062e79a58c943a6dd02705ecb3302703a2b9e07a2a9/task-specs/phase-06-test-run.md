# System task overlay: テスト実行 — P04 のテスト ID 実行と結果証跡の収集
...
- phase_ref: P06
- classification: confidence=0.92, reason="goal-spec.json を入力に P06 の単一責務 (quality) を実行する task", candidates=[{artifact_kind: task, confidence: 0.92, candidate_path: tasks/feat-build-identity-deploy-freshness/sys-build-identity-p06.md}]
...
P04 が定義したテスト ID を実行し、合否と再実行可能な証跡を残す。
...
- Infrastructure: applicable + no change: CI 上での実行結果を収集する
- Security: N/A: 本 task では露出範囲に影響する変更を行わない
- Quality: applicable + change: テスト ID を実行し結果証跡を残す
- Documentation: applicable + change: test-results.md を新規作成する
...
- Produced artifacts: docs/features/feat-build-identity-deploy-freshness/test-results.md (テスト ID ごとの実行コマンド・結果・再実行手順)
...
- Write scope/touches: docs/features/feat-build-identity-deploy-freshness/test-results.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-build-identity-p06) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-build-identity-p06 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-build-identity-p06) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: depends_on は SYS-BUILD-IDENTITY-P05 であり、直前 phase の完了後にのみ着手する。resource_scope (docs/features/feat-build-identity-deploy-freshness/test-results.md) が他 task の active lease と重複しないことを確認する
...
- P04 の全テスト ID が実行され、結果が記録されている
- しきい値超過 fixture で鮮度検査が実際に落ちたことが実行出力で示されている
- 各結果に再実行可能なコマンドが添えられている
...
- Rollout: テストを実行し結果を記録して P07 へ引き継ぐ
- Rollback trigger and steps: テストが落ちた場合は P05 へ差し戻し、test-results.md には落ちた事実を残す (削除しない)
...
- Dependencies: SYS-BUILD-IDENTITY-P05
