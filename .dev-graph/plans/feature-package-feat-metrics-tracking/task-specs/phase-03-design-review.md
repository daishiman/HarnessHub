...
P02 の architecture-decision-record.md は、試算エンジン純関数の実装 owner を feat-metrics-tracking (packages/estimation) に確定した。この判断は docs/shared-layers.md §5「共通層に第 3 の利用者が現れたときに初めて共通化する」原則と、実際の呼び出しフットプリント分析 (試算エンジンを実行時に呼ぶのは本 feature の Workers cron 週次 rollup 処理のみ)、および feat-user-org-admin 側 P02 の既確定事項 (試算エンジン scope 外、TenantCoefficient 保存管理のみ担当) を根拠としている。本 task はこの判断根拠に飛躍や見落としがないか、特に S17 (feat-user-org-admin 管轄) が将来的に試算エンジンを直接呼び出す設計変更が生じないか、Publisher 側が将来クライアント側金額換算を必要とする要求が生じないか (SEC5 により現状は禁止されているため該当なしと確認する) を独立して検証する。加えて metrics_events/metrics_rollups のカラム設計が tenant_id スコープ列必須 (D4) を満たすか、ingest API の短命 token + Idempotency-Key TTL 24h が qa-023 (B2) の記述と一致するか、dim=user の金額換算 admin 限定が SEC4 逆算対策として十分かをレビューする。
