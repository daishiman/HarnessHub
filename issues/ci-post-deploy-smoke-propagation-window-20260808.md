...
連続一致は伝播ムラに対する**確率的な**緩和であり、確定的な保証ではない。どの colo に当たるかは呼び出し側で制御できないため、「3 回とも同じ拠点に当たり、その拠点だけ先に切替わっていた」場合は依然として通過しうる。`cf-ray` の記録は、この限界が実際に起きたかを事後に判別するために入れてある。観測されたら次の手 (拠点を分散させる観測、または Cloudflare API による deployment 状態の直接確認) を検討する。
