...
updated_at: "2026-07-25T15:44:20Z"
status: "draft"
depends_on: []
related_nodes: ["feat-hub-foundation","SYS-HUB-FOUNDATION-P13","arch-harness-hub-infrastructure"]
resource_scope: ["docs/features/feat-hub-foundation/runbook.md","docs/features/feat-hub-foundation/acceptance-report.md","docs/features/feat-hub-foundation/evidence/","docs/infrastructure-spec.md"]
...
一方 **A3「SLO 99.5% の計測と `/health` が稼働する」は未達**のまま残った。`/health` の本番稼働は実測済みだが、SLO 99.5% を算定するための**外部死活監視が未設定**で、可用性の時系列が存在しないためである。
...
| 外部死活監視 | **未設定** |
| SLO ダッシュボード | **未設定** |
| `CRON_HEARTBEAT_URL`（Worker secret） | **未投入**（cron 実行監視が効かない） |
