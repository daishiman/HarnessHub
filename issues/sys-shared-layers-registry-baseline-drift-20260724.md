---
graph_node_id: "issue-shared-layers-registry-baseline-drift-20260724"
artifact_kind: "issue"
artifact_subtypes: []
...
resource_scope: []
purpose: "未解決の Beads issue HarnessHub-42g を canonical graph から到達可能に戻し、課題内容を失わず ready/parity の信号を回復する"
goal: "requirements-baseline.md L130 github-actions-ci が旧 5 ゲートを inline 列挙 (登録簿 G1〜G11 へ未追随)"
mvp_alignment: null
scope_in: ["Beads issue HarnessHub-42g の題名・説明・notes・受入条件を保持した issue node の復元"]
scope_out: ["orphan 復元と同時に元 issue の実装や close を行うこと"]
acceptance: ["Beads issue HarnessHub-42g の未解決内容と判断根拠が保持され、実装時に検証結果を記録できる","C02 writer の検証を通り、external_ref が canonical graph の実在 node を指す"]
architecture_refs: []
parent_feature: null
feature_package_id: null
phase_ref: null
file_path: "issues/sys-shared-layers-registry-baseline-drift-20260724.md"
template_id: "issue"
template_version: "1.0.0"
confirmation_status: "draft"
evaluation_status: "pending"
confirmation_evidence: {"evaluated_digest":null,"evaluator":null,"evidence_ref":null}
source_lineage: {"imported_at":"2026-07-28T00:24:44.696Z","origin_kind":"generated","source_digest":"c959ce4eb7e773b09cb9a1cdfff4e1a2a9f857f2c5b6a80b9c8c08e26f757c65","source_path":"issues/sys-orphan-external-ref-backlog-disposition-20260726.md#HarnessHub-42g","source_plugin":"dev-graph","source_version":null}
classification_confidence: 1
classification_reason: "2026-07-28 orphan 再棚卸しで HarnessHub-42g の本文と notes を確認し、未解決で実作業を持つため参照剥がし・close ではなく node 復元と判断"
classification_candidates: [{"artifact_kind":"issue","candidate_path":"issues/sys-shared-layers-registry-baseline-drift-20260724.md","confidence":1}]
issue_linkage: null
...
requirements-baseline.md L130 github-actions-ci が旧 5 ゲートを inline 列挙 (登録簿 G1〜G11 へ未追随)

## 背景と問題

Beads の未解決 issue `HarnessHub-42g` は `dev-graph:issue-shared-layers-registry-baseline-drift-20260724` を参照しているが、
canonical graph に node が無く、課題本文が ready/parity の対象から外れていた。
課題自体は未解決で内容も有効なため、参照を剥がしたり close したりせず node として復元する。

### Beads に記録された内容

Beads には題名以外の詳細説明が未記録である。実装前に現行仕様との差分を再確認する。
...
1. `bd --readonly show HarnessHub-42g --json` で `external_ref` と元の本文を読む。
2. `python3 plugins/dev-graph/scripts/bd-bridge.py --op orphan-audit --repo-root . --scan-refs` を実行する。
3. 出力で `HarnessHub-42g` が非クローズ orphan に含まれることを確認する。
...
- 解決タスク: `issue-shared-layers-registry-baseline-drift-20260724`

## 受入条件

- [ ] Beads issue HarnessHub-42g の未解決内容と判断根拠が保持され、実装時に検証結果を記録できる
