...
検知の原理:
  複数ファイルの mtime が分単位まで完全一致することは、一括書き込みの調査開始点で
  あって、原因を単独では確定しない。rsync/tar/cp -p や Finder のコピーなど、元 mtime
  を保持したまま配置する機構だけでなく、git reset --hard や pull のような Git 操作でも
  起こりうる。検知後は runbook の reflog・差分・実体照合で裏取りする。
...
def changed_paths(cwd=None):
    ...
    proc = _git("status", "--porcelain=v1", "-z", "--untracked-files=all", cwd=cwd)
    ...
    while index < len(records):
        ...
        paths.append(path)
        # -z の rename/copy は「新しい path」「古い path」の 2 record で表す。
        index += 2 if "R" in status or "C" in status else 1
    return paths
