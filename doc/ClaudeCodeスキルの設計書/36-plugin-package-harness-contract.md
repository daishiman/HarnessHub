...
| 全 plugin の `references/package-contract.json` | 22 plugin 中 7 plugin のみ配置（2026-07-21 実測） | 各 plugin に schema 準拠の contract を配置。未配置 plugin は live-trial 計画にも package 検査にも載らないため、未配置そのものが被覆欠落である | `HarnessHub-dbb` |
