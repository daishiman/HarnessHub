---
graph_node_id: "SYS-MVP-FIRST-SCHEDULING-P01"
artifact_kind: "task"
artifact_subtypes: []
...
target_date: null
iteration: null
title: "要件ベースライン確定 — MVP ファースト化 3 軸判断基準 (目的・背景・MVP) の baseline 文書化"
owners: ["daishiman"]
created_at: "2026-07-23T07:08:08Z"
updated_at: "2026-07-23T13:06:33Z"
status: "closed"
depends_on: []
related_nodes: ["feat-mvp-first-scheduling","arch-harness-hub-dev-workflow"]
resource_scope: ["docs/features/feat-mvp-first-scheduling/requirements-baseline.md"]
purpose: "goal-spec.json の purpose/background/goal/scope_in 5 件/scope_out 5 件/acceptance 5 件 (id 単位) を machine-verifiable な要件ベースラインとして固定し、P02 以降の全 task が同一の合意事項を参照できる状態にする。"
goal: "P01 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["docs/features/feat-mvp-first-scheduling/requirements-baseline.md"]
...
phase_ref: "P01"
file_path: "tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p01.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-23T07:08:08Z","origin_kind":"system-dev-planner","source_digest":"55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387","source_path":".dev-graph/plans/generations/feature-package-feat-mvp-first-scheduling/55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387/task-specs/phase-01-requirements.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.87
classification_reason: "qa-069 の MVP ファースト化要求のうち P01 責務 (要件ベースライン確定 — MVP ファースト化 3 軸判断基準 (目的・背景・MVP) の baseline 文書化) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p01.md","confidence":0.87}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-6gl.1","linked_at":"2026-07-23T08:22:25Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: [{"base_branch":"main","branch":"devgraph/SYS-MVP-FIRST-SCHEDULING-P01","head_sha":"6ab7d593967b2f93ed1c2a5f069719bfe032c71f","last_seen_at":"2026-07-23T13:04:13.437445Z","lease_acquired_at":"2026-07-23T13:04:12.265282Z","released_at":null,"state":"claimed","worktree_id":"wt_f5b59c7488943714"}]
completion_evidence: {"completed_at":null,"evidence_refs":[],"policy":"linked_pr_merged_all","reconciled_at":null,"source":null,"status":"in_progress"}
implementation_readiness: {"checked_at":"2026-07-23T06:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 要件ベースライン確定 — MVP ファースト化 3 軸判断基準 (目的・背景・MVP) の baseline 文書化
...
- parent_feature: feat-mvp-first-scheduling
- phase_ref: P01
- classification: confidence=0.87, reason="qa-069 の MVP ファースト化要求のうち P01 責務 (要件ベースライン確定 — MVP ファースト化 3 軸判断基準 (目的・背景・MVP) の baseline 文書化) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p01.md}]
...
goal-spec.json の purpose/background/goal/scope_in 5 件/scope_out 5 件/acceptance 5 件 (id 単位) を machine-verifiable な要件ベースラインとして固定し、P02 以降の全 task が同一の合意事項を参照できる状態にする。

## 背景

system-spec/dev-workflow.md の qa-069 は、dev-graph/beads (bd) のタスク優先度選定が品質・本質・再現性を先回りする基準に寄り、MVP から離れて同じ基盤タスクを繰り返す停滞を確定した。schedule-graph.py の着手候補算出 (`sorted(ready_ids - lease_conflicts)`) は node_id の辞書順ソートのみで MVP 適合度を一切評価しておらず、この gap を本 task で baseline として固定する。

## 前提条件

- Macro entry gate: `parent_feature.depends_on all done|closed`。canonical parent feature (feat-mvp-first-scheduling) の現行 depends_on (空配列) を都度評価し、task edge へ複製しない。
...
- Frontend: N/A: 本 feature は dev-graph/beads のタスク優先度選定基準変更であり frontend 実装物を変更しない
...
- Quality: applicable + change: goal-spec acceptance 5 件を machine-verifiable な受入基準として固定する
- Documentation: applicable + change: docs/features/feat-mvp-first-scheduling/requirements-baseline.md を新規作成する
- Operations: N/A: 本 phase は運用手順を変更しない

## Architecture and deploy unit

- Architecture decisions: arch-harness-hub-dev-workflow (features/feat-mvp-first-scheduling.md architecture_refs の正本参照)
- Deploy unit/environment: dev-tooling/repository (plugins/dev-graph の script・schema・規約文書。Cloudflare Workers へのデプロイは伴わない)
- Compatibility/migration/backfill: 既存 promoted package・証跡の digest を失効させない。実データ migration は P08 が所有する

## 成果物

- Produced artifacts:
- docs/features/feat-mvp-first-scheduling/requirements-baseline.md
- Consumed artifacts:
- goal-spec.json
- features/feat-mvp-first-scheduling.md
- architecture/harness-hub-dev-workflow.md
- system-spec/dev-workflow.md (qa-069)
- Write scope/touches: docs/features/feat-mvp-first-scheduling/requirements-baseline.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-mvp-first-scheduling-p01) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-mvp-first-scheduling-p01 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-mvp-first-scheduling-p01) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
...
- Required evidence: requirements-baseline.md に goal-spec の purpose/background/goal/scope_in 5 件/scope_out 5 件/acceptance 5 件が過不足なく転記され、qa-069 との対応表が含まれること
