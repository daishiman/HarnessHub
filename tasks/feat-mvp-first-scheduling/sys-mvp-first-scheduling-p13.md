---
graph_node_id: "SYS-MVP-FIRST-SCHEDULING-P13"
artifact_kind: "task"
artifact_subtypes: []
...
purpose: "実装・テスト・受入・証跡が揃った成果を default branch (main) へ反映し、MVP-first 選定が実運用で機能することを実証したうえで、system spec/architecture への writeback を行う。"
goal: "P13 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["eval-log/dev-graph/mvp-first-scheduling/release-receipt.json"]
...
phase_ref: "P13"
file_path: "tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p13.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-23T07:08:08Z","origin_kind":"system-dev-planner","source_digest":"55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387","source_path":".dev-graph/plans/generations/feature-package-feat-mvp-first-scheduling/55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387/task-specs/phase-13-release-deploy.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.87
classification_reason: "qa-069 の MVP ファースト化要求のうち P13 責務 (リリース — main 反映と MVP-first 選定の実証・spec/architecture writeback) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p13.md","confidence":0.87}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-6gl.13","linked_at":"2026-07-23T08:22:25Z","sync_state":"linked"}
...
# System task overlay: リリース — main 反映と MVP-first 選定の実証・spec/architecture writeback
...
- parent_feature: feat-mvp-first-scheduling
- phase_ref: P13
- classification: confidence=0.87, reason="qa-069 の MVP ファースト化要求のうち P13 責務 (リリース — main 反映と MVP-first 選定の実証・spec/architecture writeback) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p13.md}]
...
実装・テスト・受入・証跡が揃った成果を default branch (main) へ反映し、MVP-first 選定が実運用で機能することを実証したうえで、system spec/architecture への writeback を行う。

## 背景

goal-spec の到達状態 (next/schedule と bd ready が MVP 適合を第一ソートキーとして動作する) を本番相当の環境で実証し、qa-069 の解決を system-spec 側へ反映する最終 phase。

## 前提条件

- P01 upstream entry gate: N/A: intra-feature depends_on gate
- Required predecessor: SYS-MVP-FIRST-SCHEDULING-P12 が done であること
...
- Infrastructure: N/A: デプロイ基盤 (Cloudflare Workers 等) を変更しない。dev-tooling script の main 反映のみ
- Security: N/A: 本 phase は認可・秘密情報の取り扱いを変更しない
- Quality: applicable + change: main 反映後に qa-066 ゲートが非退行であることを確認する
- Documentation: applicable + change: system spec/architecture への writeback を行う
- Operations: applicable + change: eval-log/dev-graph/mvp-first-scheduling/release-receipt.json を新規作成する

## Architecture and deploy unit

- Architecture decisions: arch-harness-hub-dev-workflow
- Deploy unit/environment: dev-tooling/repository (plugins/dev-graph。Cloudflare Workers へのデプロイは伴わない)
- Compatibility/migration/backfill: 既存 node の MVP metadata 未設定状態を fallback 動作のまま main へ反映し、追加の migration/backfill は発生しない

## 成果物

- Produced artifacts:
- eval-log/dev-graph/mvp-first-scheduling/release-receipt.json
- Consumed artifacts:
- docs/features/feat-mvp-first-scheduling/final-review.md
- eval-log/dev-graph/mvp-first-scheduling/evidence-manifest.json
- Write scope/touches: eval-log/dev-graph/mvp-first-scheduling/release-receipt.json
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-mvp-first-scheduling-p13) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-mvp-first-scheduling-p13 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-mvp-first-scheduling-p13) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (SYS-MVP-FIRST-SCHEDULING-P12) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Required evidence: release-receipt.json に main 反映結果、qa-066 ゲートの非退行確認、system spec/architecture writeback 先の記録が含まれていること
...
- P13 spec/architecture writeback: required: 本 feature の実装結果・設計判断・改善知見を system-spec/dev-workflow.md (qa-069) と architecture/harness-hub-dev-workflow.md へ反映する

## Rollout and rollback

- Rollout: default branch (main) へ反映し、qa-066 ゲート非退行を確認したうえで system spec/architecture writeback を実施する
- Rollback trigger and steps: main 反映後に qa-066 ゲートの退行または acceptance 未達が判明した場合、当該変更を revert し P05 へ差し戻す
