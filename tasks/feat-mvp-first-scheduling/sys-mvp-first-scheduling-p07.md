---
graph_node_id: "SYS-MVP-FIRST-SCHEDULING-P07"
artifact_kind: "task"
artifact_subtypes: []
...
target_date: null
iteration: null
title: "受入判定 — goal-spec acceptance 5 件の突合と未達の差し戻し"
owners: ["daishiman"]
created_at: "2026-07-23T07:08:08Z"
updated_at: "2026-07-23T14:03:20Z"
status: "closed"
depends_on: ["SYS-MVP-FIRST-SCHEDULING-P06"]
related_nodes: ["feat-mvp-first-scheduling","arch-harness-hub-dev-workflow"]
resource_scope: ["docs/features/feat-mvp-first-scheduling/acceptance-report.md"]
purpose: "goal-spec acceptance 5 件を P06 の実測証跡に照らして突合し、未達がある場合は P05 へ差し戻す。"
goal: "P07 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["docs/features/feat-mvp-first-scheduling/acceptance-report.md"]
...
phase_ref: "P07"
file_path: "tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p07.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-23T07:08:08Z","origin_kind":"system-dev-planner","source_digest":"55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387","source_path":".dev-graph/plans/generations/feature-package-feat-mvp-first-scheduling/55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387/task-specs/phase-07-acceptance.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.87
classification_reason: "qa-069 の MVP ファースト化要求のうち P07 責務 (受入判定 — goal-spec acceptance 5 件の突合と未達の差し戻し) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p07.md","confidence":0.87}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-6gl.7","linked_at":"2026-07-23T08:22:25Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: [{"base_branch":"main","branch":"devgraph/SYS-MVP-FIRST-SCHEDULING-P07","head_sha":"6ab7d593967b2f93ed1c2a5f069719bfe032c71f","last_seen_at":"2026-07-23T14:01:24.562653Z","lease_acquired_at":"2026-07-23T14:01:22.983074Z","released_at":null,"state":"claimed","worktree_id":"wt_f5b59c7488943714"}]
completion_evidence: {"completed_at":null,"evidence_refs":[],"policy":"linked_pr_merged_all","reconciled_at":null,"source":null,"status":"in_progress"}
implementation_readiness: {"checked_at":"2026-07-23T06:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 受入判定 — goal-spec acceptance 5 件の突合と未達の差し戻し
...
- parent_feature: feat-mvp-first-scheduling
- phase_ref: P07
- classification: confidence=0.87, reason="qa-069 の MVP ファースト化要求のうち P07 責務 (受入判定 — goal-spec acceptance 5 件の突合と未達の差し戻し) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p07.md}]
...
goal-spec acceptance 5 件を P06 の実測証跡に照らして突合し、未達がある場合は P05 へ差し戻す。

## 背景

acceptance は goal-spec.json に固定されており、本 task はこれを実測エビデンスとの対応表として検証する。

## 前提条件

- P01 upstream entry gate: N/A: intra-feature depends_on gate
- Required predecessor: SYS-MVP-FIRST-SCHEDULING-P06 が done であること
...
- Backend: N/A: 本 phase は受入判定のみで実装コードを変更しない
...
- Quality: applicable + change: goal-spec acceptance 5 件を P06 証跡と突合する
- Documentation: applicable + change: docs/features/feat-mvp-first-scheduling/acceptance-report.md を新規作成する
...
- Compatibility/migration/backfill: N/A: 本 phase は受入判定のみで互換性変更を伴わない

## 成果物

- Produced artifacts:
- docs/features/feat-mvp-first-scheduling/acceptance-report.md
- Consumed artifacts:
- eval-log/dev-graph/mvp-first-scheduling/test-run-p06.json
- goal-spec.json
- Write scope/touches: docs/features/feat-mvp-first-scheduling/acceptance-report.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-mvp-first-scheduling-p07) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-mvp-first-scheduling-p07 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-mvp-first-scheduling-p07) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (SYS-MVP-FIRST-SCHEDULING-P06) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Required evidence: acceptance-report.md に goal-spec acceptance 5 件それぞれの判定 (pass/fail) と根拠となる証跡へのリンクが記載されていること。3 件目 (MVP 判断軸 metadata を持つ node が validate-graph-schema.py PASS で登録できる) は P04 の TC-MVP-SCHEMA-REG-01 定義・P05 の schema 実装・P06 の test-run-p06.json 内 validate-graph-schema.py 実行結果を根拠として明記する
...
- Rollout: P06 証跡と acceptance を突合し、全件 pass の場合のみ P08 へ引き継ぐ。未達がある場合は P05 へ差し戻す
- Rollback trigger and steps: acceptance 未達が判明した場合、未達項目を記録したうえで P05 へ差し戻す
