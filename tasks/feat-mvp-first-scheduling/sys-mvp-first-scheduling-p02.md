---
graph_node_id: "SYS-MVP-FIRST-SCHEDULING-P02"
artifact_kind: "task"
artifact_subtypes: []
...
purpose: "MVP 判断軸 (目的・背景・MVP 適合度) を表現する node metadata の追加案、schedule/next の MVP 適合第一ソートキー設計、bd-bridge ready 候補順序の MVP-first 整合方式、品質・再現性強化系タスクの繰り延べ規則、選定 receipt の出力形式を P07/P10 が判定可能な粒度で確定する。"
goal: "P02 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["docs/features/feat-mvp-first-scheduling/design.md"]
...
phase_ref: "P02"
file_path: "tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p02.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-23T07:08:08Z","origin_kind":"system-dev-planner","source_digest":"55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387","source_path":".dev-graph/plans/generations/feature-package-feat-mvp-first-scheduling/55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387/task-specs/phase-02-architecture.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.87
classification_reason: "qa-069 の MVP ファースト化要求のうち P02 責務 (設計 — MVP 適合軸メタデータ・schedule/next ソートキー・bd-bridge ready 整合・繰り延べ規則・選定 receipt の決定論設計) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p02.md","confidence":0.87}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-6gl.2","linked_at":"2026-07-23T08:22:25Z","sync_state":"linked"}
...
# System task overlay: 設計 — MVP 適合軸メタデータ・schedule/next ソートキー・bd-bridge ready 整合・繰り延べ規則・選定 receipt の決定論設計
...
- parent_feature: feat-mvp-first-scheduling
- phase_ref: P02
- classification: confidence=0.87, reason="qa-069 の MVP ファースト化要求のうち P02 責務 (設計 — MVP 適合軸メタデータ・schedule/next ソートキー・bd-bridge ready 整合・繰り延べ規則・選定 receipt の決定論設計) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p02.md}]
...
MVP 判断軸 (目的・背景・MVP 適合度) を表現する node metadata の追加案、schedule/next の MVP 適合第一ソートキー設計、bd-bridge ready 候補順序の MVP-first 整合方式、品質・再現性強化系タスクの繰り延べ規則、選定 receipt の出力形式を P07/P10 が判定可能な粒度で確定する。

## 背景

既存 graph-node.schema.json の `purpose`/`goal` は artifact_kind=feature のときだけ非 null 必須という制約 (MM-01) を持ち task-kind の node には適用できない。bd-bridge.py の `_ready_with_parity()` は `bd ready --json` が返す順序をそのまま `ready_set` へ転写するだけで独自の並べ替えを行わない。この 2 点の実装事実を踏まえ、bd CLI 本体を変更せず (scope_out) に MVP-first を成立させる設計を確定する。

## 前提条件

- P01 upstream entry gate: N/A: intra-feature depends_on gate
- Required predecessor: SYS-MVP-FIRST-SCHEDULING-P01 が done であること
...
- Backend: N/A: Hub 本体の backend 実装物を変更しない (scope_out)。設計対象は dev-tooling script (schedule-graph.py/bd-bridge.py) であり実装は P05 が担う
- API: N/A: Hub 本体の API を変更しない (scope_out)
- Data: applicable + 設計: graph-node.schema.json への MVP 判断軸 node metadata 追加案を設計する (data workstream 相当の分類は quality/documentation の secondary で表現)
- Infrastructure: N/A: デプロイ基盤を変更しない
- Security: N/A: 本 phase は認可・秘密情報の取り扱いを変更しない
- Quality: applicable + change: MVP-first ソート・繰り延べ規則・receipt 形式の受入基準を設計判断として固定する
- Documentation: applicable + change: docs/features/feat-mvp-first-scheduling/design.md を新規作成する
- Operations: applicable + change: 選定 receipt の運用面の設計方針を記述する

## Architecture and deploy unit

- Architecture decisions: arch-harness-hub-dev-workflow
- Deploy unit/environment: dev-tooling/repository
- Compatibility/migration/backfill: graph-node.schema.json への新規フィールド追加は既存 node に対して optional/nullable とし、後方互換を破壊しない設計とする
...
- Write scope/touches: docs/features/feat-mvp-first-scheduling/design.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-mvp-first-scheduling-p02) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-mvp-first-scheduling-p02 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-mvp-first-scheduling-p02) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (SYS-MVP-FIRST-SCHEDULING-P01) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Required evidence: design.md に MVP metadata schema 案・ソートキー疑似コード・bd-bridge 整合方式・繰り延べ規則・receipt 形式の 5 点が明記されていること
...
- Rollout: design.md を write scope の範囲で作成し、P03 の設計レビューに供する
- Rollback trigger and steps: P03 の設計レビューで差し戻された場合、design.md を修正し本 task を再実行する
