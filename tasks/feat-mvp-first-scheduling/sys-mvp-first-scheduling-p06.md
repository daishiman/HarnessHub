---
graph_node_id: "SYS-MVP-FIRST-SCHEDULING-P06"
artifact_kind: "task"
artifact_subtypes: []
...
target_date: null
iteration: null
title: "テスト実行 — 回帰テスト全件と ready/next 実測の記録"
owners: ["daishiman"]
created_at: "2026-07-23T07:08:08Z"
updated_at: "2026-07-23T13:59:54Z"
status: "closed"
depends_on: ["SYS-MVP-FIRST-SCHEDULING-P05"]
related_nodes: ["feat-mvp-first-scheduling","arch-harness-hub-dev-workflow"]
resource_scope: ["eval-log/dev-graph/mvp-first-scheduling/test-run-p06.json"]
purpose: "P04 で定義した全テスト ID を実行し、pass/fail の実測結果を記録する。"
goal: "P06 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["eval-log/dev-graph/mvp-first-scheduling/test-run-p06.json"]
...
phase_ref: "P06"
file_path: "tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p06.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-23T07:08:08Z","origin_kind":"system-dev-planner","source_digest":"55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387","source_path":".dev-graph/plans/generations/feature-package-feat-mvp-first-scheduling/55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387/task-specs/phase-06-test-run.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.87
classification_reason: "qa-069 の MVP ファースト化要求のうち P06 責務 (テスト実行 — 回帰テスト全件と ready/next 実測の記録) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p06.md","confidence":0.87}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-6gl.6","linked_at":"2026-07-23T08:22:25Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: [{"base_branch":"main","branch":"devgraph/SYS-MVP-FIRST-SCHEDULING-P06","head_sha":"6ab7d593967b2f93ed1c2a5f069719bfe032c71f","last_seen_at":"2026-07-23T13:47:48.943169Z","lease_acquired_at":"2026-07-23T13:47:47.950158Z","released_at":null,"state":"claimed","worktree_id":"wt_f5b59c7488943714"}]
completion_evidence: {"completed_at":null,"evidence_refs":[],"policy":"linked_pr_merged_all","reconciled_at":null,"source":null,"status":"in_progress"}
implementation_readiness: {"checked_at":"2026-07-23T06:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: テスト実行 — 回帰テスト全件と ready/next 実測の記録
...
- parent_feature: feat-mvp-first-scheduling
- phase_ref: P06
- classification: confidence=0.87, reason="qa-069 の MVP ファースト化要求のうち P06 責務 (テスト実行 — 回帰テスト全件と ready/next 実測の記録) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p06.md}]
...
P04 で定義した全テスト ID を実行し、pass/fail の実測結果を記録する。

## 背景

P07/P10 は実行された証跡だけを判定対象とする (P04 test-plan の trace rule)。P05 の実装完了だけでは受入判定の根拠にならない。

## 前提条件

- P01 upstream entry gate: N/A: intra-feature depends_on gate
- Required predecessor: SYS-MVP-FIRST-SCHEDULING-P05 が done であること
...
- Quality: applicable + change: P04 の全テスト ID を実行し pass/fail を記録する
...
- Produced artifacts:
- eval-log/dev-graph/mvp-first-scheduling/test-run-p06.json
- Consumed artifacts:
...
- Write scope/touches: eval-log/dev-graph/mvp-first-scheduling/test-run-p06.json
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-mvp-first-scheduling-p06) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-mvp-first-scheduling-p06 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-mvp-first-scheduling-p06) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (SYS-MVP-FIRST-SCHEDULING-P05) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Automated commands:
  - `python3 -m pytest plugins/dev-graph/tests/test_schedule_graph_mvp_first.py plugins/dev-graph/tests/test_bd_bridge_mvp_ready_order.py plugins/dev-graph/tests/test_graph_node_mvp_schema_registration.py --junitxml=eval-log/dev-graph/mvp-first-scheduling/test-run-p06.xml`
  - `python3 plugins/dev-graph/scripts/validate-graph-schema.py --graph .dev-graph/state/graph.json --repo-root .`
- Required evidence: test-run-p06.json に P04 の全テスト ID (schema 登録検証カテゴリの TC-MVP-SCHEMA-REG-01 を含む) の pass/fail 結果と実行日時、および validate-graph-schema.py の実行結果 (exit code・PASS/FAIL) が記録されていること
...
- Rollout: P04 の全テスト ID を実行し、結果を write scope のログへ記録してから P07 へ引き継ぐ
- Rollback trigger and steps: テスト実行が失敗した場合、失敗内容を記録したうえで P05 へ差し戻す
