---
graph_node_id: "SYS-MVP-FIRST-SCHEDULING-P04"
artifact_kind: "task"
artifact_subtypes: []
...
target_date: null
iteration: null
title: "テスト設計 — MVP-first ソート・冪等性・後方互換・receipt 出力の回帰テスト設計"
owners: ["daishiman"]
created_at: "2026-07-23T07:08:08Z"
updated_at: "2026-07-23T13:26:48Z"
status: "closed"
depends_on: ["SYS-MVP-FIRST-SCHEDULING-P03"]
related_nodes: ["feat-mvp-first-scheduling","arch-harness-hub-dev-workflow"]
resource_scope: ["docs/features/feat-mvp-first-scheduling/test-plan.md"]
purpose: "MVP-first ソート・選定結果の冪等性・MVP metadata 未設定 node の後方互換・選定 receipt 出力・MVP 判断軸 metadata を持つ node の schema 登録検証の 5 カテゴリについて、P06 が実行できる具体的なテスト ID と期待結果を定義する。"
goal: "P04 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["docs/features/feat-mvp-first-scheduling/test-plan.md"]
...
phase_ref: "P04"
file_path: "tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p04.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-23T07:08:08Z","origin_kind":"system-dev-planner","source_digest":"55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387","source_path":".dev-graph/plans/generations/feature-package-feat-mvp-first-scheduling/55a34fe2a62841c0175b568204b4a1fde8e1fd04d1c0496bb4e0444e3cf86387/task-specs/phase-04-test-design.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.87
classification_reason: "qa-069 の MVP ファースト化要求のうち P04 責務 (テスト設計 — MVP-first ソート・冪等性・後方互換・receipt 出力の回帰テスト設計) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p04.md","confidence":0.87}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-6gl.4","linked_at":"2026-07-23T08:22:25Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: [{"base_branch":"main","branch":"devgraph/SYS-MVP-FIRST-SCHEDULING-P04","head_sha":"6ab7d593967b2f93ed1c2a5f069719bfe032c71f","last_seen_at":"2026-07-23T13:24:47.954359Z","lease_acquired_at":"2026-07-23T13:24:41.556131Z","released_at":null,"state":"claimed","worktree_id":"wt_f5b59c7488943714"}]
completion_evidence: {"completed_at":null,"evidence_refs":[],"policy":"linked_pr_merged_all","reconciled_at":null,"source":null,"status":"in_progress"}
implementation_readiness: {"checked_at":"2026-07-23T06:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: テスト設計 — MVP-first ソート・冪等性・後方互換・receipt 出力の回帰テスト設計
...
- parent_feature: feat-mvp-first-scheduling
- phase_ref: P04
- classification: confidence=0.87, reason="qa-069 の MVP ファースト化要求のうち P04 責務 (テスト設計 — MVP-first ソート・冪等性・後方互換・receipt 出力の回帰テスト設計) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-mvp-first-scheduling/sys-mvp-first-scheduling-p04.md}]
...
MVP-first ソート・選定結果の冪等性・MVP metadata 未設定 node の後方互換・選定 receipt 出力・MVP 判断軸 metadata を持つ node の schema 登録検証の 5 カテゴリについて、P06 が実行できる具体的なテスト ID と期待結果を定義する。

## 背景

acceptance の『同一入力で next を再実行しても選定 batch と順序が一致する (冪等)』は決定論検査でしか確認できない要件であり、実装着手前にテスト対象を固定しておく必要がある。また acceptance の『MVP 判断軸 metadata を持つ node が validate-graph-schema.py PASS で登録できる』は P05 の schema 変更が既存の登録検証経路 (validate-graph-schema.py) を壊していないことを保証する専用カテゴリとして、他カテゴリと独立にテスト ID を持つ必要がある。

## 前提条件

- P01 upstream entry gate: N/A: intra-feature depends_on gate
- Required predecessor: SYS-MVP-FIRST-SCHEDULING-P03 が done であること
...
- Quality: applicable + change: MVP-first ソート・冪等性・後方互換・receipt 出力・schema 登録検証 (validate-graph-schema.py PASS) の回帰テスト ID を定義する
- Documentation: applicable + change: docs/features/feat-mvp-first-scheduling/test-plan.md を新規作成する
...
- Compatibility/migration/backfill: 後方互換テスト (MVP metadata 未設定 node の fallback) をテスト対象に含める
...
- Write scope/touches: docs/features/feat-mvp-first-scheduling/test-plan.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-mvp-first-scheduling-p04) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-mvp-first-scheduling-p04 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-mvp-first-scheduling-p04) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (SYS-MVP-FIRST-SCHEDULING-P03) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Required evidence: test-plan.md に MVP-first ソート・冪等性・後方互換・receipt 出力・schema 登録検証の 5 カテゴリごとにテスト ID と期待結果が列挙されていること。schema 登録検証カテゴリには少なくとも TC-MVP-SCHEMA-REG-01 (MVP 判断軸 metadata (目的・背景・MVP 適合度) を持つ task-kind node を検証用グラフへ登録し、`validate-graph-schema.py --graph --repo-root .` が PASS すること) を含み、goal-spec acceptance 3 件目 (MVP 判断軸 metadata を持つ node が validate-graph-schema.py PASS で登録できる) を直接カバーすること
...
- Rollout: test-plan.md を write scope の範囲で作成し、P05 の実装対象を固定する
- Rollback trigger and steps: P05/P06 実行時にテスト ID の不備が判明した場合、test-plan.md を修正し本 task を再実行する
