---
graph_node_id: "SYS-DEV-PIPELINE-IMPROVEMENT-P04"
artifact_kind: "task"
artifact_subtypes: []
...
updated_at: "2026-07-23T11:18:39.529241Z"
status: "closed"
...
purpose: "P05 で実装する lint script 3 本と P08 の冪等 migration に対する回帰テストを設計し、実行可能なテスト ID 一覧を確定する。悪性ケース (検出すべき違反) と良性ケース (通過すべき正常形) の両方を fixture で定義する。"
goal: "P04 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["docs/features/feat-dev-pipeline-improvement/test-plan.md"]
...
acceptance: ["test-plan.md に lint-open-residue / lint-eval-log-layout / lint-handoff-disposition の各 MUST_DETECT (悪性) / MUST_PASS (良性) ケースがテスト ID 付きで定義されている","spec-drift-guardian close-gate 配線の回帰テスト (verdict 未着 proposal-only close が遮断される MUST_BLOCK ケースと、verdict 付き close が通過する MUST_PASS ケース) がテスト ID 付きで定義されている","P08 冪等 migration の再実行差分 0 検証と disposition 遡及付与の全件性検証 (31 findings 差分監査行の存在検証を含む) がテスト ID 付きで定義されている","全テスト ID が P06 で実行可能な pytest 形式 (plugins/dev-graph/tests または plugins/spec-drift-guardian/tests 配下) として設計されている"]
architecture_refs: ["arch-harness-hub-dev-workflow"]
parent_feature: "feat-dev-pipeline-improvement"
feature_package_id: "feature-package/feat-dev-pipeline-improvement"
phase_ref: "P04"
file_path: "tasks/feat-dev-pipeline-improvement/sys-dev-pipeline-improvement-p04.md"
template_id: "task"
...
classification_reason: "qa-067 の開発管理パイプライン改善 8 要件のうち P04 責務 (テスト設計 — lint script 群と冪等 migration の回帰テスト設計) を実行する task"
...
beads_linkage: {"bd_issue_id":"HarnessHub-k2u.4","linked_at":"2026-07-21T16:50:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-07-22T06:30:15Z","evidence_refs":["docs/features/feat-dev-pipeline-improvement/test-plan.md"],"policy":"manual","reconciled_at":"2026-07-23T11:10:47Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-07-21T15:10:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: テスト設計 — lint script 群と冪等 migration の回帰テスト設計
...
- parent_feature: feat-dev-pipeline-improvement
- phase_ref: P04
- classification: confidence=0.87, reason="qa-067 の開発管理パイプライン改善 8 要件のうち P04 責務 (テスト設計 — lint script 群と冪等 migration の回帰テスト設計) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-dev-pipeline-improvement/sys-dev-pipeline-improvement-p04.md}]
...
P05 で実装する lint script 3 本と P08 の冪等 migration に対する回帰テストを設計し、実行可能なテスト ID 一覧を確定する。悪性ケース (検出すべき違反) と良性ケース (通過すべき正常形) の両方を fixture で定義する。

## 背景

guard hook の前例 (MUST_BLOCK/MUST_PASS 契約テスト) に倣い、検出力を fixture で固定する。テスト設計が無いまま実装すると、lint が緩すぎて素通りするか厳しすぎて誤遮断するかを実測できない。
...
- Quality: applicable + change: MUST_DETECT/MUST_PASS の fixture 設計とテスト ID 採番を行う
...
- Produced artifacts:
- docs/features/feat-dev-pipeline-improvement/test-plan.md
...
- Write scope/touches: docs/features/feat-dev-pipeline-improvement/test-plan.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-dev-pipeline-improvement-p04) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-dev-pipeline-improvement-p04 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-dev-pipeline-improvement-p04) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 phase 完了後にのみ着手する (直列 DAG)。resource_scope (docs/features/feat-dev-pipeline-improvement/test-plan.md) が他 task の active lease と重複しないことを確認する
...
- Required evidence: test-plan.md のテスト ID 一覧が P05 実装対象と 1 対 1 で対応していること
...
- Rollback trigger and steps: テスト設計が P02 契約と乖離した場合、test-plan.md を修正し P02/P03 との整合を回復する
...
- test-plan.md に lint-open-residue / lint-eval-log-layout / lint-handoff-disposition の各 MUST_DETECT (悪性) / MUST_PASS (良性) ケースがテスト ID 付きで定義されている
- spec-drift-guardian close-gate 配線の回帰テスト (verdict 未着 proposal-only close が遮断される MUST_BLOCK ケースと、verdict 付き close が通過する MUST_PASS ケース) がテスト ID 付きで定義されている
- P08 冪等 migration の再実行差分 0 検証と disposition 遡及付与の全件性検証 (31 findings 差分監査行の存在検証を含む) がテスト ID 付きで定義されている
- 全テスト ID が P06 で実行可能な pytest 形式 (plugins/dev-graph/tests または plugins/spec-drift-guardian/tests 配下) として設計されている
