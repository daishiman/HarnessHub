---
graph_node_id: "SYS-DEV-PIPELINE-IMPROVEMENT-P09"
artifact_kind: "task"
artifact_subtypes: []
...
updated_at: "2026-07-22T12:52:14.327167Z"
status: "active"
...
purpose: "新設 lint 3 本と close-loop 検査が、悪性ケース (規約違反ファイルの直下追加・disposition 欠落 handoff の新規作成・解決済み issue の open 残置) を実際に遮断することを一時 fixture で実測し、Goodhart 緑化 (検査を通すためだけの形式的修正) が起きていないことを検証する。"
goal: "P09 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["eval-log/dev-graph/pipeline-improvement/qa-fail-closed-report.json"]
...
acceptance: ["qa-fail-closed-report.json に悪性ケース 3 系統以上の実測 (投入した違反・期待 exit 非 0・実測 exit code) が記録され、全件検出されている","検査への迂回経路 (lint 対象外 path への書込・schema_version 偽装) の探索結果が記録され、発見された迂回は P05 へ差し戻されている"]
architecture_refs: ["arch-harness-hub-dev-workflow"]
parent_feature: "feat-dev-pipeline-improvement"
feature_package_id: "feature-package/feat-dev-pipeline-improvement"
phase_ref: "P09"
file_path: "tasks/feat-dev-pipeline-improvement/sys-dev-pipeline-improvement-p09.md"
template_id: "task"
...
# System task overlay: 品質保証 — fail-closed 実効性の悪性ケース実測
...
- parent_feature: feat-dev-pipeline-improvement
- phase_ref: P09
- classification: confidence=0.87, reason="qa-067 の開発管理パイプライン改善 8 要件のうち P09 責務 (品質保証 — fail-closed 実効性の悪性ケース実測) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-dev-pipeline-improvement/sys-dev-pipeline-improvement-p09.md}]
...
新設 lint 3 本と close-loop 検査が、悪性ケース (規約違反ファイルの直下追加・disposition 欠落 handoff の新規作成・解決済み issue の open 残置) を実際に遮断することを一時 fixture で実測し、Goodhart 緑化 (検査を通すためだけの形式的修正) が起きていないことを検証する。

## 背景

検査は「存在すること」ではなく「検出すること」が価値である。過去の教訓 (live-trial の偽 PASS・digest 単独書き換え) に倣い、検出力を悪性ケースの実測で証明する。
...
- Security: applicable + change: 検査迂回の攻撃面を評価する
- Quality: applicable + change: 悪性ケース実測と迂回経路探索を行う
...
- Produced artifacts:
- eval-log/dev-graph/pipeline-improvement/qa-fail-closed-report.json
...
- Write scope/touches: eval-log/dev-graph/pipeline-improvement/qa-fail-closed-report.json
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-dev-pipeline-improvement-p09) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-dev-pipeline-improvement-p09 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-dev-pipeline-improvement-p09) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 phase 完了後にのみ着手する (直列 DAG)。resource_scope (eval-log/dev-graph/pipeline-improvement/qa-fail-closed-report.json) が他 task の active lease と重複しないことを確認する
...
- Required evidence: qa-fail-closed-report.json の実測が再現手順付きで記録されていること
...
- Rollback trigger and steps: 検出漏れが見つかった場合、P05 へ差し戻し fixture を MUST_DETECT へ追加してから再実装する
...
- 検査への迂回経路 (lint 対象外 path への書込・schema_version 偽装) の探索結果が記録され、発見された迂回は P05 へ差し戻されている
