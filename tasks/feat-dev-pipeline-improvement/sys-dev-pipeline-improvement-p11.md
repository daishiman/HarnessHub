---
graph_node_id: "SYS-DEV-PIPELINE-IMPROVEMENT-P11"
artifact_kind: "task"
artifact_subtypes: []
...
updated_at: "2026-07-23T11:20:48.885708Z"
status: "closed"
depends_on: ["SYS-DEV-PIPELINE-IMPROVEMENT-P10"]
related_nodes: ["feat-dev-pipeline-improvement","arch-harness-hub-dev-workflow"]
resource_scope: ["eval-log/dev-graph/pipeline-improvement/evidence-manifest.json"]
purpose: "P06/P08/P09 の実測ログと主要成果物の sha256 を evidence manifest として固定し、後からの改変を検出可能にする。"
goal: "P11 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["eval-log/dev-graph/pipeline-improvement/evidence-manifest.json"]
...
phase_ref: "P11"
file_path: "tasks/feat-dev-pipeline-improvement/sys-dev-pipeline-improvement-p11.md"
template_id: "task"
...
classification_reason: "qa-067 の開発管理パイプライン改善 8 要件のうち P11 責務 (証跡固定 — 実測ログと digest の evidence manifest 化) を実行する task"
...
beads_linkage: {"bd_issue_id":"HarnessHub-k2u.11","linked_at":"2026-07-21T16:50:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-07-22T11:38:22Z","evidence_refs":["eval-log/dev-graph/pipeline-improvement/evidence-manifest.json"],"policy":"manual","reconciled_at":"2026-07-23T11:10:47Z","source":"reconciliation","status":"done"}
...
- parent_feature: feat-dev-pipeline-improvement
- phase_ref: P11
- classification: confidence=0.87, reason="qa-067 の開発管理パイプライン改善 8 要件のうち P11 責務 (証跡固定 — 実測ログと digest の evidence manifest 化) を実行する task", candidates=[{artifact_kind: task, confidence: 0.87, candidate_path: tasks/feat-dev-pipeline-improvement/sys-dev-pipeline-improvement-p11.md}]
...
P06/P08/P09 の実測ログと主要成果物の sha256 を evidence manifest として固定し、後からの改変を検出可能にする。

## 背景

quality_constraints の digest-immutability に基づき、証跡は manifest の digest で束縛する。manifest 自体も再実行で再現可能な形式とする。
...
- Documentation: applicable + change: evidence manifest を生成し digest を固定する
...
- Produced artifacts:
- eval-log/dev-graph/pipeline-improvement/evidence-manifest.json
...
- Write scope/touches: eval-log/dev-graph/pipeline-improvement/evidence-manifest.json
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-dev-pipeline-improvement-p11) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-dev-pipeline-improvement-p11 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-dev-pipeline-improvement-p11) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 phase 完了後にのみ着手する (直列 DAG)。resource_scope (eval-log/dev-graph/pipeline-improvement/evidence-manifest.json) が他 task の active lease と重複しないことを確認する
...
- Required evidence: manifest の sha256 照合が exit 0 で再現できること
...
- Rollback trigger and steps: 照合不一致が見つかった場合、当該証跡の生成 phase へ差し戻し原因を記録する
...
- evidence-manifest.json に P06/P08/P09 の実測ログと P05 成果物の path・sha256・生成 phase が記録されている
- manifest 記載の sha256 が実ファイルと全件一致する
