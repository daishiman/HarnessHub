...
goal: P06 のテスト結果を goal-spec の acceptance 3 項目 (tenant スコープ doc の分離・Markdown XSS sanitize・doc 編集操作の監査記録) に照らして受入判定を行い、feature acceptance の根拠を確定する。
scope_in: ["docs/features/feat-docs-cms/acceptance-report.md"]
scope_out: ["実装コードの修正 (未達の場合の修正は P05 へ差し戻す)", "goal-spec 自体の再確定 (dev-graph の scope)"]
...
> task projection (P07 / parent: feat-docs-cms)。本文の正本は published task spec であり、ここには複製しない。

## 正本仕様書

- `.dev-graph/plans/feature-package-feat-docs-cms/task-specs/phase-07-acceptance.md`
...
- 完了: 正本仕様書の Verification and evidence を満たし bd close (SYS-DOCS-CMS-P07)
