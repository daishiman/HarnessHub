...
classification_reason: "system-spec/testing-qa.md qa-070/qa-072/qa-073/qa-074 のタスク仕様書テスト戦略必須化要求のうち P12 責務 (運用文書化 — テスト戦略 section の記述手順と保守性制約 (pixel 位置・DOM 構造依存禁止) の運用ガイド) を実行する task"
...
- parent_feature: feat-task-spec-test-strategy
- phase_ref: P12
- classification: confidence=0.86, reason="system-spec/testing-qa.md qa-070/qa-072/qa-073/qa-074 のタスク仕様書テスト戦略必須化要求のうち P12 責務 (運用文書化 — テスト戦略 section の記述手順と保守性制約の運用ガイド) を実行する task", candidates=[{artifact_kind: task, confidence: 0.86, candidate_path: tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p12.md}]
...
system-dev-planner が生成する task spec のテスト戦略 section は、記述者や実行タイミングによって表現がぶれないことが qa-073/qa-075 の要求である。P05 の実装 (schema・validator・derivation) だけでは運用時の記述判断 (層別方針の当てはめ方、保守性制約の線引きの実務適用) が属人化するリスクが残るため、運用ガイドを P13 のリリース前に固定する。
