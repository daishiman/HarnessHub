---
graph_node_id: "SYS-TASK-SPEC-TEST-STRATEGY-P08"
artifact_kind: "task"
artifact_subtypes: []
...
purpose: "P05 で追加したテスト戦略 section 必須化の fail-closed validator が、既存の promoted 世代 (例: feature-package/feat-mvp-first-scheduling) の task spec digest を変えないことを実測確認し、compatibility-note.md へ記録する。scope_out の「既存タスク仕様書資産の一括再生成をしない」方針を実測で裏付ける。"
goal: "P08 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["docs/features/feat-task-spec-test-strategy/compatibility-note.md"]
...
phase_ref: "P08"
file_path: "tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p08.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-24T23:09:08Z","origin_kind":"system-dev-planner","source_digest":"7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae","source_path":".dev-graph/plans/generations/feature-package-feat-task-spec-test-strategy/7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae/task-specs/phase-08-refactoring-migration.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.86
classification_reason: "system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P08 責務 (移行 — 既存 promoted 世代を一括再生成しない互換性確認 (旧世代 digest 不変の実測記録)) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p08.md","confidence":0.86}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-a4ks.8","linked_at":"2026-07-25T00:00:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-07-25T14:18:56Z","evidence_refs":["issues/sys-lint-open-residue-ci-red-20260725.md"],"policy":"manual","reconciled_at":"2026-07-26T01:19:20.811908Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-07-24T21:59:18Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 移行 — 既存 promoted 世代を一括再生成しない互換性確認 (旧世代 digest 不変の実測記録)
...
- parent_feature: feat-task-spec-test-strategy
- phase_ref: P08
- classification: confidence=0.86, reason="system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P08 責務 (移行 — 既存 promoted 世代を一括再生成しない互換性確認 (旧世代 digest 不変の実測記録)) を実行する task", candidates=[{artifact_kind: task, confidence: 0.86, candidate_path: tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p08.md}]
...
P05 で追加したテスト戦略 section 必須化の fail-closed validator が、既存の promoted 世代 (例: feature-package/feat-mvp-first-scheduling) の task spec digest を変えないことを実測確認し、compatibility-note.md へ記録する。scope_out の「既存タスク仕様書資産の一括再生成をしない」方針を実測で裏付ける。

## 背景

新規検査を既存 promoted 世代へ遡及適用すると、意図しない digest 変化や既存契約の破壊が起きうる。本 phase は新検査が新規世代 (feature-package/feat-task-spec-test-strategy 自身) にのみ適用され、既存世代を不変に保つことを実測で確認する責務を持つ。

## 前提条件

- P01 upstream entry gate marker: parent_feature.depends_on all done|closed (intra-feature の depends_on chain 解決を指し、P01 自体の完了ではなく parent_feature 全体の depends_on 充足を確認する)
- Required predecessor: SYS-TASK-SPEC-TEST-STRATEGY-P07 が done であること
- Required spec/architecture/phase/task nodes: feat-task-spec-test-strategy, arch-harness-hub-testing-qa
- Entry gate: goal-spec.json の source_feature_digest が sha256:eafd046f7f71c3c44f48a69297d08e0ca160a3f503e243a99a8a11c7bd178df7 に一致し、features/feat-task-spec-test-strategy.md の frontmatter と goal-spec の purpose/goal/scope_in/scope_out/acceptance が逐語一致すること
...
- Backend: N/A: 本 phase は互換性確認文書の作成のみで実装コードを変更しない
...
- Quality: applicable + change: 既存 promoted 世代の task spec digest が新検査導入前後で不変であることを実測する
- Documentation: applicable + change: 確認結果を compatibility-note.md へ記録する
- Operations: applicable + change: 既存 promoted 世代の再生成運用へ新検査を遡及適用しない方針を確認する

## Architecture and deploy unit

- Architecture decisions: arch-harness-hub-testing-qa
- Deploy unit/environment: dev-tooling/repository
- Compatibility/migration/backfill: applicable + change: 本 phase が既存 promoted 世代との互換性確認そのものであり、旧世代 digest 不変の実測結果を compatibility-note.md へ記録する

## 成果物

- Produced artifacts:
  - docs/features/feat-task-spec-test-strategy/compatibility-note.md
- Consumed artifacts:
  - plugins/system-dev-planner/scripts/validate-system-plan.py
  - docs/features/feat-task-spec-test-strategy/acceptance-report.md
  - .dev-graph/plans/generations/feature-package-feat-mvp-first-scheduling
- Write scope/touches: docs/features/feat-task-spec-test-strategy/compatibility-note.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-task-spec-test-strategy-p08) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-task-spec-test-strategy-p08 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-task-spec-test-strategy-p08) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (SYS-TASK-SPEC-TEST-STRATEGY-P07) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Automated commands:
  - `python3 plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-mvp-first-scheduling`
  - `python3 plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --staging .`
- Required evidence: compatibility-note.md に既存 promoted 世代の task spec digest が新検査導入前後で不変であることの実測結果、および exact-13 契約・13-node DAG 検査が非退行であることの実測結果が記録されていること
...
- Rollout: 既存 promoted 世代の digest 不変を確認し、結果を compatibility-note.md へ記録してから P09 へ引き継ぐ
- Rollback trigger and steps: 旧世代 digest が変化した場合、変更を revert し新検査を新規世代のみへ適用する設計へ戻す
...
- Feature: `features/feat-task-spec-test-strategy.md` (source_feature_digest `sha256:eafd046f7f71c3c44f48a69297d08e0ca160a3f503e243a99a8a11c7bd178df7`)
...
- goal-spec: goal-spec.json (parent_feature=feat-task-spec-test-strategy, source_feature_digest=sha256:eafd046f7f71c3c44f48a69297d08e0ca160a3f503e243a99a8a11c7bd178df7)
- 仕様正本: system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 (タスク仕様書テスト戦略必須化要求)
- trace rule: P04 defines executable test IDs; P05 implements their subjects; P06 executes them; P07/P10 adjudicate only executed evidence; P09 makes applicable checks fail-closed; P11 preserves source digest and rerun commands; P12/P13 cannot substitute documentation or planned work for missing implementation/evidence.

## 依存

- `SYS-TASK-SPEC-TEST-STRATEGY-P07`
