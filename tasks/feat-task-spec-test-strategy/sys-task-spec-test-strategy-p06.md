---
graph_node_id: "SYS-TASK-SPEC-TEST-STRATEGY-P06"
artifact_kind: "task"
artifact_subtypes: []
...
purpose: "P04 で定義したテスト戦略 section 関連の全テスト ID (section 欠落拒否・4 項目完備 PASS・再生成冪等性・exact-13 非退行の回帰) を、P05 で実装したスキーマ・fail-closed validator・テンプレート必須 section 組込に対して実行し、pass/fail の実測結果を記録する。"
goal: "P06 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["eval-log/system-dev-planner/task-spec-test-strategy/test-run-p06.json"]
...
phase_ref: "P06"
file_path: "tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p06.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-24T23:09:08Z","origin_kind":"system-dev-planner","source_digest":"7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae","source_path":".dev-graph/plans/generations/feature-package-feat-task-spec-test-strategy/7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae/task-specs/phase-06-test-run.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.86
classification_reason: "system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P06 責務 (テスト実行 — 回帰テスト全件実行と欠落拒否・冪等性の実測記録) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p06.md","confidence":0.86}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-a4ks.6","linked_at":"2026-07-25T00:00:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-07-25T14:18:45Z","evidence_refs":["issues/sys-lint-open-residue-ci-red-20260725.md"],"policy":"manual","reconciled_at":"2026-07-26T01:19:20.811908Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-07-24T21:59:18Z","missing_sections":[],"status":"complete"}
---

# System task overlay: テスト実行 — 回帰テスト全件実行と欠落拒否・冪等性の実測記録
...
- parent_feature: feat-task-spec-test-strategy
- phase_ref: P06
- classification: confidence=0.86, reason="system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P06 責務 (テスト実行 — 回帰テスト全件実行と欠落拒否・冪等性の実測記録) を実行する task", candidates=[{artifact_kind: task, confidence: 0.86, candidate_path: tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p06.md}]
...
P04 で定義したテスト戦略 section 関連の全テスト ID (section 欠落拒否・4 項目完備 PASS・再生成冪等性・exact-13 非退行の回帰) を、P05 で実装したスキーマ・fail-closed validator・テンプレート必須 section 組込に対して実行し、pass/fail の実測結果を記録する。

## 背景

P07 (受入判定) と P10 (最終レビュー) は実行された証跡だけを判定対象とする。P05 の実装完了そのものは受入判定の根拠にならず、P04 が定義したテスト ID の実行結果という一次証跡が別途必要になる。これは system-spec/testing-qa.md qa-081 が要求する「何度実行しても同じ品質基準で再現する仕組み」を、実装完了の宣言ではなく実測で担保する責務でもある。
...
- Required predecessor: SYS-TASK-SPEC-TEST-STRATEGY-P05 が done であること
...
- Backend: N/A: 本 phase はテスト実行のみで実装コードを変更しない
...
- Quality: applicable + change: P04 が定義した全テスト ID を P05 実装に対して実行し pass/fail を記録する
...
- Compatibility/migration/backfill: N/A: 本 phase はテスト実行のみで互換性変更を伴わない

## 成果物

- Produced artifacts:
  - eval-log/system-dev-planner/task-spec-test-strategy/test-run-p06.json
- Consumed artifacts:
...
- Write scope/touches: eval-log/system-dev-planner/task-spec-test-strategy/test-run-p06.json
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-task-spec-test-strategy-p06) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-task-spec-test-strategy-p06 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-task-spec-test-strategy-p06) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (SYS-TASK-SPEC-TEST-STRATEGY-P05) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Automated commands:
  - `python3 -m pytest plugins/system-dev-planner/tests/test_task_spec_test_strategy_sections.py plugins/system-dev-planner/tests/test_task_spec_test_strategy_derivation.py --junitxml=eval-log/system-dev-planner/task-spec-test-strategy/test-run-p06.xml`
  - `python3 plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --staging .`
- Required evidence: test-run-p06.json に P04 が定義した全テスト ID (section 欠落拒否カテゴリ・4 項目完備 PASS カテゴリ・再生成冪等性カテゴリ・exact-13 非退行カテゴリを含む) の pass/fail 結果と実行日時、および validate-system-plan.py の実行結果 (exit code・PASS/FAIL) が記録されていること

## Inner goal-seek execution loop

- Methodology contract: `system-task-goal-seek/v1`
- Goal: P06 の Phase acceptance と Verification and evidence をすべて満たす
...
- Rollout: P04 が定義した全テスト ID を実行し、結果を write scope のログへ記録してから P07 へ引き継ぐ
- Rollback trigger and steps: test 失敗が実装欠陥に起因する場合は P05 へ差し戻し、test 定義側の誤りなら P04 へ差し戻す
...
- `SYS-TASK-SPEC-TEST-STRATEGY-P05`
