...
classification_reason: "system-spec/testing-qa.md qa-070/qa-072/qa-073/qa-074 のタスク仕様書テスト戦略必須化要求のうち P06 責務 (テスト実行 — 回帰テスト全件実行と欠落拒否・冪等性の実測記録) を実行する task"
...
- parent_feature: feat-task-spec-test-strategy
- phase_ref: P06
- classification: confidence=0.86, reason="system-spec/testing-qa.md qa-070/qa-072/qa-073/qa-074 のタスク仕様書テスト戦略必須化要求のうち P06 責務 (テスト実行 — 回帰テスト全件実行と欠落拒否・冪等性の実測記録) を実行する task", candidates=[{artifact_kind: task, confidence: 0.86, candidate_path: tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p06.md}]
...
P07 (受入判定) と P10 (最終レビュー) は実行された証跡だけを判定対象とする。P05 の実装完了そのものは受入判定の根拠にならず、P04 が定義したテスト ID の実行結果という一次証跡が別途必要になる。これは system-spec/testing-qa.md qa-075 が要求する「何度実行しても同じ品質基準で再現する仕組み」を、実装完了の宣言ではなく実測で担保する責務でもある。
