---
graph_node_id: "SYS-TASK-SPEC-TEST-STRATEGY-P10"
artifact_kind: "task"
artifact_subtypes: []
...
purpose: "P01..P09 の全成果物 (要件ベースライン・設計・設計レビュー・テスト設計・実装・テスト実行・受入判定・移行互換性・悪性ケース品質保証) を横断的に突き合わせ、テスト戦略 4 項目 (テストレベル選定・カバレッジ目標・層別方針・保守性制約)・fail-closed validator・再生成冪等性・exact-13 非退行の観点に齟齬がないかを確認する。"
goal: "P10 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["docs/features/feat-task-spec-test-strategy/final-review.md"]
...
phase_ref: "P10"
file_path: "tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p10.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-24T23:09:08Z","origin_kind":"system-dev-planner","source_digest":"7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae","source_path":".dev-graph/plans/generations/feature-package-feat-task-spec-test-strategy/7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae/task-specs/phase-10-final-review.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.86
classification_reason: "system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P10 責務 (最終レビュー — 全 phase 成果の横断整合確認) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p10.md","confidence":0.86}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-a4ks.10","linked_at":"2026-07-25T00:00:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-07-25T14:19:11Z","evidence_refs":["issues/sys-lint-open-residue-ci-red-20260725.md"],"policy":"manual","reconciled_at":"2026-07-26T01:19:20.811908Z","source":"reconciliation","status":"done"}
...
- parent_feature: feat-task-spec-test-strategy
- phase_ref: P10
- classification: confidence=0.86, reason="system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P10 責務 (最終レビュー — 全 phase 成果の横断整合確認) を実行する task", candidates=[{artifact_kind: task, confidence: 0.86, candidate_path: tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p10.md}]
...
P01..P09 の全成果物 (要件ベースライン・設計・設計レビュー・テスト設計・実装・テスト実行・受入判定・移行互換性・悪性ケース品質保証) を横断的に突き合わせ、テスト戦略 4 項目 (テストレベル選定・カバレッジ目標・層別方針・保守性制約)・fail-closed validator・再生成冪等性・exact-13 非退行の観点に齟齬がないかを確認する。

## 背景

各 phase は独立した責務を個別に遂行するため、phase 間で前提や用語の解釈にずれが生じるリスクがある。qa-076/qa-078/qa-079/qa-080 が要求する品質基準の一貫性を担保するため、証跡固定 (P11) とリリース (P13) の前に、全 phase 成果を横断で照合するレビューを設ける。

## 前提条件

- P01 entry gate marker: parent_feature.depends_on all done|closed
- Required predecessor: SYS-TASK-SPEC-TEST-STRATEGY-P09 が done であること
...
- Backend: N/A: 本 phase はレビューのみで実装コードを変更しない
...
- Quality: applicable + change: P01..P09 の全成果物の横断整合を確認し、齟齬があれば該当 phase へ差し戻す
- Documentation: applicable + change: docs/features/feat-task-spec-test-strategy/final-review.md を新規作成する
...
- Compatibility/migration/backfill: N/A: 本 phase はレビューのみで互換性変更を伴わない
...
- docs/features/feat-task-spec-test-strategy/design-review.md
...
- Write scope/touches: docs/features/feat-task-spec-test-strategy/final-review.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-task-spec-test-strategy-p10) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-task-spec-test-strategy-p10 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-task-spec-test-strategy-p10) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (SYS-TASK-SPEC-TEST-STRATEGY-P09) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Required evidence: final-review.md に P01..P09 の全成果物の横断整合確認結果 (齟齬の有無と対応) が記録されていること
...
- Rollout: P01..P09 の全成果物の横断整合を確認したうえで final-review.md を作成し、P11 へ引き継ぐ
- Rollback trigger and steps: 横断整合の齟齬が見つかった場合、該当 phase へ差し戻し本 task を再実行する
...
- `SYS-TASK-SPEC-TEST-STRATEGY-P09`
