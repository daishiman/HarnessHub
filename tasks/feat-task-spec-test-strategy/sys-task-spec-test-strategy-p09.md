---
graph_node_id: "SYS-TASK-SPEC-TEST-STRATEGY-P09"
artifact_kind: "task"
artifact_subtypes: []
...
purpose: "section 部分欠落・見出し順序入替・空本文などの悪性入力に対し、P05 の fail-closed validator が必ず非0終了で拒否することを実測し、qa-fail-closed-report.json へ記録する。"
goal: "P09 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["eval-log/system-dev-planner/task-spec-test-strategy/qa-fail-closed-report.json"]
...
phase_ref: "P09"
file_path: "tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p09.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-24T23:09:08Z","origin_kind":"system-dev-planner","source_digest":"7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae","source_path":".dev-graph/plans/generations/feature-package-feat-task-spec-test-strategy/7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae/task-specs/phase-09-quality-assurance.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.86
classification_reason: "system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P09 責務 (品質保証 — section 部分欠落・順序入替・空本文の悪性ケースに対する fail-closed 実効性の実測) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p09.md","confidence":0.86}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-a4ks.9","linked_at":"2026-07-25T00:00:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-07-25T14:19:09Z","evidence_refs":["issues/sys-lint-open-residue-ci-red-20260725.md"],"policy":"manual","reconciled_at":"2026-07-26T01:19:20.811908Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-07-24T21:59:18Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 品質保証 — section 部分欠落・順序入替・空本文の悪性ケースに対する fail-closed 実効性の実測
...
- parent_feature: feat-task-spec-test-strategy
- phase_ref: P09
- classification: confidence=0.86, reason="system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P09 責務 (品質保証 — section 部分欠落・順序入替・空本文の悪性ケースに対する fail-closed 実効性の実測) を実行する task", candidates=[{artifact_kind: task, confidence: 0.86, candidate_path: tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p09.md}]
...
section 部分欠落・見出し順序入替・空本文などの悪性入力に対し、P05 の fail-closed validator が必ず非0終了で拒否することを実測し、qa-fail-closed-report.json へ記録する。

## 背景

acceptance の「テスト戦略 section を欠いた task spec 入力に対し validator が非0終了で拒否する」を目視確認でなく、悪性ケースに対する fail-closed 実効性の実測として担保する。P02 の設計で定めた検査点が実装 (P05) で正しく機能しているかを網羅的な負テストで検証する責務を持つ。
...
- Required predecessor: SYS-TASK-SPEC-TEST-STRATEGY-P08 が done であること
...
- Security: applicable + change: section 部分欠落・順序入替・空本文の悪性入力に対する fail-closed 拒否を悪性ケースで実測する
- Quality: applicable + change: 悪性ケースの網羅性と exact-13 契約・13-node DAG 検査の非退行を実測する
...
- Produced artifacts:
  - eval-log/system-dev-planner/task-spec-test-strategy/qa-fail-closed-report.json
- Consumed artifacts:
...
- Write scope/touches: eval-log/system-dev-planner/task-spec-test-strategy/qa-fail-closed-report.json
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-task-spec-test-strategy-p09) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-task-spec-test-strategy-p09 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-task-spec-test-strategy-p09) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (SYS-TASK-SPEC-TEST-STRATEGY-P08) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Automated commands:
  - `python3 -m pytest plugins/system-dev-planner/tests/test_task_spec_test_strategy_sections.py -k malformed --junitxml=eval-log/system-dev-planner/task-spec-test-strategy/qa-fail-closed-run.xml`
  - `python3 plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --staging .`
- Required evidence: qa-fail-closed-report.json に section 部分欠落・見出し順序入替・空本文の各悪性ケースで validator が非0終了で拒否したことの実測結果、および exact-13 契約・13-node DAG 検査が非退行であることが記録されていること
...
- Rollout: 悪性ケースを実行し fail-closed 動作を確認してから P10 へ引き継ぐ
- Rollback trigger and steps: 悪性ケースが素通りした場合、検査条件を P02 の設計へ差し戻して強化する
...
- `SYS-TASK-SPEC-TEST-STRATEGY-P08`
