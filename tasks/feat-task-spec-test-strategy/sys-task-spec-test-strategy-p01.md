...
updated_at: "2026-07-25T00:47:22.619662Z"
status: "active"
depends_on: []
...
classification_reason: "system-spec/testing-qa.md qa-070/qa-072/qa-073/qa-074 のタスク仕様書テスト戦略必須化要求のうち P01 責務 (要件ベースライン確定 — テスト戦略 4 項目 (テストレベル選定・カバレッジ目標・層別方針・保守性制約) の要件ベースライン文書化) を実行する task"
...
system-spec/testing-qa.md の qa-070/qa-072/qa-073/qa-074 は、タスク仕様書がテスト網羅の明記を持たないため実装後に結合不備と既存機能の破壊を後追い発見していること (qa-070)、フロント/バックエンド/インフラの層別テスト種別と保守性方針が確定していないこと (qa-072)、およびテスト戦略セクションをテンプレート必須項目として組み込み仕様生成パイプライン側で機械検証する仕組みが未整備であること (qa-073/qa-074) を確定した。本 feature は Hub 本体の実装ではなく system-dev-planner が生成する仕様書契約そのものの改修であり、depends_on は空で feat-mvp-first-scheduling (qa-069) と並行進行できる。この gap を本 task で baseline として固定する。
...
- Required evidence: requirements-baseline.md に goal-spec の purpose/background/goal/scope_in 7 件/scope_out 7 件/acceptance 7 件が過不足なく転記され、qa-070/qa-072/qa-073/qa-074 との対応表が含まれること
