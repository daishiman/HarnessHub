---
graph_node_id: "SYS-TASK-SPEC-TEST-STRATEGY-P01"
artifact_kind: "task"
artifact_subtypes: []
...
purpose: "goal-spec.json の purpose/background/goal/scope_in 7 件/scope_out 7 件/acceptance 7 件を machine-verifiable な要件ベースラインとして固定し、P02 以降の全 task が同一の合意事項を参照できる状態にする。特にテスト戦略 4 項目 (テストレベル選定・カバレッジ目標・層別方針・保守性制約) が要求する具体的な粒度を、この段階で曖昧さを残さず文書化する。"
goal: "P01 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["docs/features/feat-task-spec-test-strategy/requirements-baseline.md"]
...
phase_ref: "P01"
file_path: "tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p01.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-24T23:09:08Z","origin_kind":"system-dev-planner","source_digest":"7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae","source_path":".dev-graph/plans/generations/feature-package-feat-task-spec-test-strategy/7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae/task-specs/phase-01-requirements.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.86
classification_reason: "system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P01 責務 (要件ベースライン確定 — テスト戦略 4 項目 (テストレベル選定・カバレッジ目標・層別方針・保守性制約) の要件ベースライン文書化) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p01.md","confidence":0.86}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-a4ks.1","linked_at":"2026-07-25T00:00:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-07-25T14:18:00Z","evidence_refs":["issues/sys-lint-open-residue-ci-red-20260725.md"],"policy":"manual","reconciled_at":"2026-07-26T01:19:20.811908Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-07-24T21:59:18Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 要件ベースライン確定 — テスト戦略 4 項目 (テストレベル選定・カバレッジ目標・層別方針・保守性制約) の要件ベースライン文書化
...
- parent_feature: feat-task-spec-test-strategy
- phase_ref: P01
- classification: confidence=0.86, reason="system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P01 責務 (要件ベースライン確定 — テスト戦略 4 項目 (テストレベル選定・カバレッジ目標・層別方針・保守性制約) の要件ベースライン文書化) を実行する task", candidates=[{artifact_kind: task, confidence: 0.86, candidate_path: tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p01.md}]
...
goal-spec.json の purpose/background/goal/scope_in 7 件/scope_out 7 件/acceptance 7 件を machine-verifiable な要件ベースラインとして固定し、P02 以降の全 task が同一の合意事項を参照できる状態にする。特にテスト戦略 4 項目 (テストレベル選定・カバレッジ目標・層別方針・保守性制約) が要求する具体的な粒度を、この段階で曖昧さを残さず文書化する。

## 背景

system-spec/testing-qa.md の qa-076/qa-078/qa-079/qa-080 は、タスク仕様書がテスト網羅の明記を持たないため実装後に結合不備と既存機能の破壊を後追い発見していること (qa-076)、フロント/バックエンド/インフラの層別テスト種別と保守性方針が確定していないこと (qa-078)、およびテスト戦略セクションをテンプレート必須項目として組み込み仕様生成パイプライン側で機械検証する仕組みが未整備であること (qa-079/qa-080) を確定した。本 feature は Hub 本体の実装ではなく system-dev-planner が生成する仕様書契約そのものの改修であり、depends_on は空で feat-mvp-first-scheduling (qa-069) と並行進行できる。この gap を本 task で baseline として固定する。

## 前提条件

- Macro entry gate: parent_feature.depends_on all done|closed。canonical parent feature (feat-task-spec-test-strategy) の現行 depends_on (空配列) を都度評価し、task edge へ複製しない。
...
- Frontend: N/A: 本 feature は system-dev-planner が生成する仕様書契約の改修であり frontend 実装物を変更しない
...
- Quality: applicable + change: goal-spec acceptance 7 件を machine-verifiable な受入基準として固定する
- Documentation: applicable + change: docs/features/feat-task-spec-test-strategy/requirements-baseline.md を新規作成する
- Operations: applicable + change: 要件ベースラインの固定により、後続 P12 の運用ガイド記述とテスト戦略 section の運用手順化の土台を提供する
...
- Produced artifacts: docs/features/feat-task-spec-test-strategy/requirements-baseline.md
- Consumed artifacts: goal-spec.json, features/feat-task-spec-test-strategy.md, architecture/harness-hub-testing-qa.md, system-spec/testing-qa.md
- Write scope/touches: docs/features/feat-task-spec-test-strategy/requirements-baseline.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-task-spec-test-strategy-p01) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-task-spec-test-strategy-p01 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-task-spec-test-strategy-p01) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (なし (P01 は本 package の起点)) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Required evidence: requirements-baseline.md に goal-spec の purpose/background/goal/scope_in 7 件/scope_out 7 件/acceptance 7 件が過不足なく転記され、qa-076/qa-078/qa-079/qa-080 との対応表が含まれること
...
- Rollout: 成果物を write scope の範囲で作成し、acceptance を満たしたことを確認してから P02 へ引き継ぐ
...
- (なし: P01 は parent feature の macro entry gate を実行時評価)
