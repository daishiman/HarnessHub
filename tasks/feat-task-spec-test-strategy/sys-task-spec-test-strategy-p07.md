---
graph_node_id: "SYS-TASK-SPEC-TEST-STRATEGY-P07"
artifact_kind: "task"
artifact_subtypes: []
...
target_date: null
iteration: null
title: "受入判定 — goal-spec acceptance 7 件の突合と未達の差し戻し"
owners: ["daishiman"]
created_at: "2026-07-24T23:09:08Z"
updated_at: "2026-07-26T01:19:20.811908Z"
status: "active"
depends_on: ["SYS-TASK-SPEC-TEST-STRATEGY-P06"]
related_nodes: ["feat-task-spec-test-strategy","arch-harness-hub-testing-qa"]
resource_scope: ["docs/features/feat-task-spec-test-strategy/acceptance-report.md"]
purpose: "goal-spec acceptance 7 件それぞれについて P06 の実測結果 (test-run-p06.json) と突合し、成否を acceptance-report.md へ記録する。未達が 1 件でもあれば受入を保留し、該当する担当 phase へ差し戻す。"
goal: "P07 の受入条件と品質ゲートを満たし、再実行可能な検証証跡を残す"
scope_in: ["docs/features/feat-task-spec-test-strategy/acceptance-report.md"]
...
phase_ref: "P07"
file_path: "tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p07.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-07-24T23:09:08Z","origin_kind":"system-dev-planner","source_digest":"7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae","source_path":".dev-graph/plans/generations/feature-package-feat-task-spec-test-strategy/7d185f453466523edcc7c6071fde7c24001612da367de329c10ba13c82a43bae/task-specs/phase-07-acceptance.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 0.86
classification_reason: "system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P07 責務 (受入判定 — goal-spec acceptance 7 件の突合と未達の差し戻し) を実行する task"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p07.md","confidence":0.86}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-a4ks.7","linked_at":"2026-07-25T00:00:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-07-25T14:18:50Z","evidence_refs":["issues/sys-lint-open-residue-ci-red-20260725.md"],"policy":"manual","reconciled_at":"2026-07-26T01:19:20.811908Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-07-24T21:59:18Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 受入判定 — goal-spec acceptance 7 件の突合と未達の差し戻し
...
- parent_feature: feat-task-spec-test-strategy
- phase_ref: P07
- classification: confidence=0.86, reason="system-spec/testing-qa.md qa-076/qa-078/qa-079/qa-080 のタスク仕様書テスト戦略必須化要求のうち P07 責務 (受入判定 — goal-spec acceptance 7 件の突合と未達の差し戻し) を実行する task", candidates=[{artifact_kind: task, confidence: 0.86, candidate_path: tasks/feat-task-spec-test-strategy/sys-task-spec-test-strategy-p07.md}]
...
goal-spec acceptance 7 件それぞれについて P06 の実測結果 (test-run-p06.json) と突合し、成否を acceptance-report.md へ記録する。未達が 1 件でもあれば受入を保留し、該当する担当 phase へ差し戻す。

## 背景

P07/P10 は実行された証跡だけを判定対象とする (P04 test-plan の trace rule)。acceptance 突合を目視確認ではなく、実行済み evidence に基づく機械可読な判定として固定することで、qa-076/qa-078/qa-079/qa-080 が要求する品質基準の冪等な再現性を担保する。
...
- Required predecessor: SYS-TASK-SPEC-TEST-STRATEGY-P06 が done であること
...
- Backend: N/A: 本 phase は受入判定文書の作成のみで実装コードを変更しない
...
- Quality: applicable + change: goal-spec acceptance 7 件を P06 実測結果 (test-run-p06.json) と突合し各項目の PASS/FAIL を判定する
- Documentation: applicable + change: 突合結果を acceptance-report.md として文書化する
...
- Compatibility/migration/backfill: N/A: 本 phase は受入判定文書の作成のみで互換性変更を伴わない
...
- Consumed artifacts:
  ...
  - goal-spec.json
- Write scope/touches: docs/features/feat-task-spec-test-strategy/acceptance-report.md
...
- PR body contract: Closes に紐づく beads issue 番号 + dev-graph graph_node_id (sys-task-spec-test-strategy-p07) を本文に明記し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent の宣言のみを行い、dev-graph が実際の binding 解決・mutation・reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/sys-task-spec-test-strategy-p07 として払い出す。system-dev-planner は事前に branch 名を確定しない
- Worktree lease: 実装着手前に graph_node_id (sys-task-spec-test-strategy-p07) の worktree lease を claim し、heartbeat 送出と完了時 release を行う
- Parallel safety: 直前 predecessor task (SYS-TASK-SPEC-TEST-STRATEGY-P06) が done になるまで着手しない。write scope が他 task の active lease と重複しないことを確認する
...
- Automated commands:
  - `python3 -c "import json;d=json.load(open('eval-log/system-dev-planner/task-spec-test-strategy/test-run-p06.json'));assert all(r.get('status') in ('pass','fail') for r in d.get('results', []))"`
  - `python3 plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --staging .`
- Required evidence: acceptance-report.md に acceptance 7 件それぞれの成否、test-run-p06.json 中の対応するテスト ID、判定日時が記録され、1 件でも未達の場合は差し戻し先 phase が明記されていること
...
- Rollout: acceptance 7 件の突合結果を acceptance-report.md へ記録し、全件 PASS を確認してから P08 へ引き継ぐ
- Rollback trigger and steps: 未達 acceptance が 1 件でもあれば受入を保留し、該当する担当 phase へ差し戻す
...
- `SYS-TASK-SPEC-TEST-STRATEGY-P06`
