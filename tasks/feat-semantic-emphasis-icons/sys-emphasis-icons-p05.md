---
graph_node_id: "SYS-EMPHASIS-ICONS-P05"
artifact_kind: "task"
artifact_subtypes: []
...
target_date: null
iteration: null
title: "実装 — 絵文字 lint script・CI fail-closed 組込み・一覧/カード状態表現の token 統一・可視ラベル併置の実装"
owners: ["daishiman"]
created_at: "2026-08-13T23:31:45Z"
updated_at: "2026-08-14T11:01:12Z"
status: "closed"
depends_on: ["SYS-EMPHASIS-ICONS-P04"]
related_nodes: ["feat-semantic-emphasis-icons","arch-harness-hub-design-system","arch-harness-hub-frontend"]
resource_scope: ["scripts/lint-ui-text-emoji.py",".github/workflows/ci.yml","packages/ui/src/components/Badge.tsx"]
purpose: "P02 の決定と P04 のテストスタブに従い、絵文字 lint script・CI fail-closed 組込み・一覧/カード状態表現の token 統一・可視ラベル併置の 4 点を実装する。既存の callout/icon/token 実装は変更しない。"
goal: "content-addressed published task spec の全責務・受入条件・検証・rollback を満たし、再実行可能な証跡を残す"
scope_in: ["scripts/lint-ui-text-emoji.py",".github/workflows/ci.yml","packages/ui/src/components/Badge.tsx"]
scope_out: ["配色仕様書 v2 そのものの改訂 (architecture/harness-hub-design-system.md の所有)","各画面の情報構造・機能追加 (feat-card-list-shell の担当)","Markdown のカードブロック記法 (feat-card-block-authoring の担当)","公開 API・DB schema・認可判定・Cloudflare deploy unit の変更","テストの実行 (P06 が所有)","既存の callout/icon/token 実装 (packages/ui/src/icons/index.tsx, packages/ui/src/components/Markdown.tsx, packages/ui/src/tokens/tokens.ts) の変更"]
acceptance: ["scripts/lint-ui-text-emoji.py が packages/ui/src 配下の文言・callout ラベル・空状態文言中の絵文字を検出し、検出時に非ゼロ終了する",".github/workflows/ci.yml の static-gates 相当ジョブに絵文字 lint が fail-closed で組み込まれている (lint 未実行では CI が成功しない)","一覧・カードの状態表現 (Badge.tsx 等) が packages/ui/src/tokens/tokens.ts の semantic token のみで色付けされ、token 外の色指定が残っていない","P04 のテストスタブが実装に対して green で通過する"]
...
phase_ref: "P05"
file_path: "tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p05.md"
template_id: "task"
...
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-semantic-emphasis-icons/sys-emphasis-icons-p05.md","confidence":0.9}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"HarnessHub-xo7n.5","linked_at":"2026-08-14T03:40:00Z","sync_state":"linked"}
...
# 実装 — 絵文字 lint script・CI fail-closed 組込み・一覧/カード状態表現の token 統一・可視ラベル併置の実装

> task projection (P05 / parent: feat-semantic-emphasis-icons)。実装要件の正本は下記の content-addressed published task spec であり、このファイルは実行入口だけを保持する。
...
- task spec: `.dev-graph/plans/generations/feature-package-feat-semantic-emphasis-icons/b10daedf4d24a112c73943e2cecac213a193042aaa0e6e43518a2314a64cfbdf/task-specs/phase-05-implementation.md`
- package digest: `sha256:b10daedf4d24a112c73943e2cecac213a193042aaa0e6e43518a2314a64cfbdf`
- task spec SHA-256: `sha256:8265d3c76f3c660cd770bef5cd15dc8d3de03e57969ce95d1fd350a0db4af79c`
