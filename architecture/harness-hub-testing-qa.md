...
- 2026-08-11 監査 fork 台帳 schema 1.2 (`HarnessHub-uypz`): per-call `tool_use_id` / whole-response digest / `verdict_state` の fail-closed 照合。製品 runtime 非変更。正本は [uypz 受領書](../docs/features/feat-dev-pipeline-improvement/uypz-audit-fork-schema12-spec-reflection-receipt.md)。
