...
認証済み業務画面の右下に常設ボタンを置く。画面遷移なし。投稿者入力は本文と注釈のみ。
画面キャプチャは D9 `modern-screenshot` の DOM 再描画で、許可ダイアログを出さない。
widget 自身は撮影対象から外す。実装は未着手。正本は `system-spec/frontend.md` /
`system-spec/ui-ux.md` と
