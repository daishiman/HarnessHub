...
- [ ] 各 task spec の `## テスト戦略` が `スコープ外` と `Verification and evidence` の間に1件だけ存在し、4項目 (`テストレベル選定`/`カバレッジ目標`/`層別方針`/`保守性制約`) がこの順序で非空である (契約 version は台帳未登録ゆえ `1.2.0` に解決され、欠落は C12 が fail-closed で拒否する)
