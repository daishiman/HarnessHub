...
def _base():
    ...
    build_live_trial_shape.py が先に file path から読み込んでいるため、通常は
    sys.modules の実体を再利用する。単体で import された場合だけ自前で読み込む。
    """
    module = sys.modules.get("build_live_trial_fixture")
    if module is not None:
        return module
    path = Path(__file__).resolve().parents[1] / "build_live_trial_fixture.py"
...
def _write_json(path: Path, document: dict) -> bytes:
    """base の graph/config と同じ整形規則 (sort_keys + indent 2 + 末尾改行) で書く。"""
...
def _retarget(node: dict, kind: str, file_path: str) -> dict:
    """base の task_node を別 artifact_kind へ振り替える (file_path 由来の値も揃える)。
...
def _architecture_node(base) -> dict:
    ...
    node = base.task_node(ARCHITECTURE_ID, "live-trial fixture の参照アーキテクチャ", ARCHITECTURE_SLUG, [])
...
def _feature_node(base) -> dict:
    """macro feature ノード。progress 集約の親になる。"""
    node = base.task_node(FEATURE_ID, "live-trial fixture の可視化対象 feature", FEATURE_SLUG, [])
...
def _package_task_node(base, phase: str, source_digest: str) -> dict:
    ...
    node = base.task_node(node_id, f"{phase}: {PHASE_TITLES[phase]}", slug, depends_on)
    ...
    if node["status"] == "done":
        ...
        node["completion_evidence"] = {
            "completed_at": base.FIXED_TS,
            "evidence_refs": [node["file_path"]],
            "policy": "manual",
            "reconciled_at": base.FIXED_TS,
            "source": "manual",
            "status": "done",
        }
    return node
...
def _feature_package_document(node_ids: dict[str, str]) -> dict:
    ...
    return {
        ...
        "generated_by": "build_live_trial_shape.py --shape render",
        "phases": [
...
def build(out: Path) -> None:
    """base fixture 生成済みの out へ、C05 scenario 固有の artifact を追加する。"""
    base = _base()
    ...
    new_nodes = [_architecture_node(base), _feature_node(base)]
    new_nodes.extend(_package_task_node(base, phase, source_digest) for phase in PHASES)
    for node in new_nodes:
        (out / node["file_path"]).write_text(base.markdown_for(node), encoding="utf-8")

    # 3) base graph へ追記する。register-package.py が 1 回の登録で revision を +1 するのに倣い、
