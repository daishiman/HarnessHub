...
##### 確定内容 qa-216 (対応セル: web, desktop-windows, desktop-macos)
...
- 原則: デプロイパイプライン (`plugins/system-spec-harness/skills/ref-system-design-knowledge/references/continuous-delivery.md#中核概念`)
  ...
  - トレードオフ:
    - selector 規則と gate 台帳の継続保守が必要になる
    - 低 tier の延期分を公開前に回収する運用負担が残る
- 原則: ゲートと改善ループ (`plugins/system-spec-harness/skills/ref-system-design-knowledge/references/test-strategy.md#中核概念`)
  - 採否: `applied`
  - 章固有の根拠: 検査結果を tier-decision、deferred issue、cache provenance として残し、失敗後の回収と同一条件での再検証までを契約化する判断に適用した。
  - トレードオフ:
    - 証跡 artifact と cache の保存費が増える
    - selector や cache が利用不能な場合は fail-closed で速度を失う
- 資するゴール: G1, G4, G5
