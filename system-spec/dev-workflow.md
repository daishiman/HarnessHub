...
**質問**: main 統合で qa-143 の同一 ID に C11/C14/C16 と plugin hook parity の別契約が現れたとき、双方を失わず dev-workflow.web の fail-closed 開発品質契約としてどのように再確定しますか?

**回答**: ユーザーの 2026-08-04 CI 失敗解消・競合解消・最終レビュー・仕様反映・公開指示を承認として、qa-145 の C11/C14/C16・durable evidence 契約と、main から統合する hook entry point parity 契約を qa-146 に統合確定する。

【1. C16/C11/C14 の保全】selected かつ schedulable な node は ready/unmapped/conflicts のいずれかへ被覆し、依存未充足を payload/parity より先に判定する。`artifact_kind=specification` の必須見出し不在は `heading_missing`、空本文等は `placeholder_only_section` として readiness を incomplete にする。conditional template 未解決の task/issue は `HarnessHub-yzv0` の対象とする。stdin preview は `--graph - --repo-root` で未書込み artifact の `artifact_missing` だけを許容し、schema/frontmatter/path containment は fail-closed に保つ。

【2. durable evidence と C02】live-trial criterion がある skill は criteria receipt の唯一の `live_trial_verdict_ref` を current acceptance とし、ref の形状・containment・実在・一意性の不備は最新 run-id へ fallback せず拒否する。過去の run は append-only とし、criteria receipt だけを新しい PASS run へ更新する。C02 registration receipt の直接改変・削除は拒否するが、別 evidence 本文のファイル名言及だけは改変と誤認しない。

【3. plugin hook entry point parity】`package-contract.json` の `entry_points.hooks` を台帳の正本にし、manifest inline hooks と `hooks/hooks.json` の登録和集合、`hooks/` の実体を双方向に検査する。登録済み未宣言は HK-001、登録構成がある plugin の宣言済み未登録は HK-002、残余の実行可能 file/shell script は HK-003 として非0終了にする。相対 command は正規化し、import 専用 support module だけを hooks 残余として許可する。手動運用 script は `scripts/` に置いて自動 hook と混同しない。検査器が 500 行を超えるときは CLI/全体走査と hook 判定を責務分離する。

【4. 境界と検証】変更は repository 内の Dev Graph、plugin 配布、CI、開発品質証跡に限定し、Harness Hub の外部 API、DB schema、認証認可、UI、Cloudflare deploy unit は変更しない。fresh live-trial、criteria evidence、hook parity の正負例、canonical graph と system-spec coverage を検証する。
