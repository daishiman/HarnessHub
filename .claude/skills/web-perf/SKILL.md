---
name: web-perf
description: Chrome DevTools MCP で Web パフォーマンスを分析する。Core Web Vitals の LCP・INP・CLS と補助指標の FCP・TBT・Speed Index を計測し、描画を妨げるリソース、ネットワーク依存の連鎖、レイアウトシフト、キャッシュ、アクセシビリティの不足を洗い出すときに使う。
---
